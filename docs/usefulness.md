...
Alpha.12 supplies the generic `VerifiedArchive` capability, the supported portable UTF-8 profile, and the public `sealr::wheel` evaluator for filenames, metadata, `RECORD`, relocation, generated targets, and distinct identities. The runnable `wheel_admission` and `same_digest_different_tree` examples cover source deletion, plan materialization, identity separation, hostile structure, and lying metadata. The isolated [packaged PyPA adopter conformance kit](../tests/pypa-installer-consumer/README.md) goes through exact installer 1.0.1: it compiles against Cargo's extracted crate, independently verifies canonical evidence, deletes the source, stages bounded member blobs only through `VerifiedArchive`, denies every `.whl` open, performs real filesystem installation, and audits all outputs into the realization identity. It runs both a controlled semantic fixture and the real hash-pinned installer wheel. Alpha.12 native packages also carry the independent evidence verifier and prove the extracted producer-verifier pair across all three release floors. This passes the repository mechanism, evidence, and binary-handoff tests. Stable usefulness remains open because the kit is repository-owned and no external adopter treats the public representation as authority.
...
Alpha.8 demonstrates items 1 and 2 through the public prerelease API. Alpha.12 demonstrates items 4 and 5 in required repository conformance and independently verifies canonical unsigned evidence. A real external adopter for item 3, stable API review, targeted compatibility breadth, and the remaining trust gates still prevent a stable product-level passing claim.
