...
Current main supplies the generic `VerifiedArchive` capability, the supported portable UTF-8 profile, and the public `sealr::wheel` evaluator for filenames, metadata, `RECORD`, relocation, generated targets, and distinct identities. A downstream contract deletes the original Unicode wheel before evaluation and proves deterministic output through the capability alone. The runnable `wheel_admission` example demonstrates admission, source deletion, plan materialization, and realization identity alongside a hostile container refused at admission and a lying `RECORD` denied by the consumer. The public `same_digest_different_tree` example demonstrates that one archive digest and one archive tree can still produce distinct filename-bound wheel identities and a typed refusal. The Alpha.7 laboratory separately preserves the hash-pinned external PyPA installer proof. Canonical receipt v3 evidence is now independently verified against its view and source before the repository statement builder will wrap it. This passes the repository mechanism and evidence tests. Stable usefulness remains open because no external adopter treats the public representation as authority.
...
Alpha.8 demonstrates items 1 and 2 through the public prerelease API and preserves the external research proof relevant to items 3 and 4. Current main demonstrates item 5 and independently verifies canonical unsigned evidence. A real external adopter for item 3, stable API review, targeted compatibility breadth, and the remaining trust gates still prevent a stable product-level passing claim.
