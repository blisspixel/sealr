...
The executable representation slice has landed as a dormant [private semantic record](semantic-record.md). It binds planning IR and completion verification state without changing a shipped path. A feature-gated repository bridge now carries the canonical plan through an immutable sealed memfd, binds it to the restricted worker's exact file-backed snapshot, executes only planned Store and Deflate payload ranges without invoking the structural parser, and returns the canonical completion through a second sealed memfd. The source descriptor remains live through worker exit and reap. The supervisor first treats the completion as an untrusted proposal, then replays the accepted plan against that retained exact source and requires byte-for-byte canonical agreement. This preserves the fail-closed distinction between lifecycle failure and archive semantics without activating public behavior.
