...
`sealr-worker-bootstrap-lab` is a Linux-only executable conformance tool with a trivial non-Linux build path and cross-platform fixed-frame unit tests. It has no dependency on `sealr`, `sealr-cli`, or `sealr-worker-protocol`, and it is not a release asset. Linux-only `rustix` and `landlock` dependencies exercise sequenced-packet descriptor transfer, two-layer authority closure, a direct Landlock floor query plus fixed ABI 3 enforcement, exact truncation flags, parent observation, point-specific abrupt exit, pidfd termination, reap, and checked cleanup ordering. Its raw receive path follows Linux `CMSG_ALIGN` layout on GNU and musl, validates every returned control header, accepts one rights record, and proves installed descriptors close on rejected short, truncated, unknown, malformed, and multiple-rights packets. A dependency-free x86_64 seccomp-BPF module uses `TSYNC`, audit-architecture and x32 checks, a measured deny set, direct `EPERM` probes, and procfs observation to close process-creation and stage-permission-mutation authority before source transfer. The tool proves only those mechanics with synthetic source and stage objects. Product execution waits for the remaining [semantic-ownership gates](decisions/0001-alpha6-semantic-ownership.md), authority-epoch deadline closure, stress evidence, and a packaging design that works for both CLI and library consumers.
