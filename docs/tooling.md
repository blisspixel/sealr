...
`sealr-worker-bootstrap-lab` is a Linux-only executable conformance tool with a trivial non-Linux build path and cross-platform fixed-frame unit tests. It has no dependency on `sealr`, `sealr-cli`, or `sealr-worker-protocol`, and it is not a release asset. Linux-only `rustix` and `landlock` dependencies exercise sequenced-packet descriptor transfer, two-layer authority closure, a direct Landlock floor query plus fixed ABI 3 enforcement, exact truncation flags, parent observation, point-specific abrupt exit, absolute monotonic authority-round deadlines, pidfd termination and reap, and checked cleanup ordering. Its raw receive path follows Linux `CMSG_ALIGN` layout on GNU and musl, validates every returned control header, accepts one rights record, and proves installed descriptors close on rejected short, truncated, unknown, malformed, and multiple-rights packets. Its SHA-256 dependency binds bounded kernel-sealed synthetic plan and completion memfds after independently validating required seals, length, role, digest, and handoff correlation. The nonblocking supervisor polls both the control socket and pidfd under each shared send-and-response deadline; eleven deterministic stalls cover changing authority through post-ack exit. Required conformance cycles its 44-case non-stall matrix 500 times while checking the supervisor child list and descriptor count plus fixture authority before cleanup. A dependency-free x86_64 seccomp-BPF module uses `TSYNC`, audit-architecture and x32 checks, a measured deny set, direct `EPERM` probes, and procfs observation to close process-creation and stage-permission-mutation authority before source transfer. The tool proves only those mechanics with synthetic source, stage, plan, and completion objects. Product execution waits for the remaining [semantic-ownership gates](decisions/0001-alpha6-semantic-ownership.md), real semantic execution and race evidence, and a packaging design that works for both CLI and library consumers.
