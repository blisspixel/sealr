...
The single required `CI` workflow verifies the exact allowlisted library package with `cargo package --locked -p sealr`, compares its complete file list and README and license bytes with the package contract, builds `tests/packaged-consumer` against Cargo's extracted package directory, and then runs that binary inside native Linux package verification against the exact extracted helper manifest. This catches missing packaged files, package-only compilation failures, accidental workspace-only APIs, manifest drift, isolation fallback, and capability regressions before any crates.io publication. An external integration test also compiles against the public crate boundary from outside the library module.
...
`sealr-wheel-lab` is also a non-published workspace tool. Its v1 structural pilot requires the exact production-helper manifest and uses the public fail-closed supervised API with [strict ASCII v2](profiles/zip-strict-ascii-v2.md) to analyze a bounded, digest-pinned corpus without another ZIP parser. Its predecessor-bound [v2 inventory](wheel-compatibility-v2.md) uses the wheel UTF-8 profile and consumes only `VerifiedArchive`. Minimized committed fixtures cover container, `RECORD`, relocation, generated-target, and filename disagreements. A separate hash-pinned PyPA installer bridge receives bounded verified-member blobs after the source wheel is removed and an audit hook denies every wheel open. Raw benign wheels remain outside Git. Required CI performs offline verification of both committed reports and executes the hostile fixtures and external bridge. Re-executing corpus measurement is a deliberate research operation because it requires the ignored local artifact cache.
