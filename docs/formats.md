...
|---|---|---|
| Strict ZIP32 Store and Deflate | Current alpha.2 | Existing preview boundary |
| Canonical ZIP path and tree profile | Phase 0.1 | `ArchiveIR`, outcome axes, profiles, roots, Unicode model, and assurance gates |
| TAR plus PAX and GNU name handling | Phase 1 | ZIP trust gate, canonical consumer, and reusable-tree gates complete; one shared path, quota, materializer, worker, and evidence core |
| gzip and zstd wrappers | Deferred with TAR consumers | Exact stream, window, metadata, and cancellation semantics |
