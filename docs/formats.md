...
| 7z Copy container | In-process preview on main under policy v11 | The executed Gate C first step: raw-header Copy-only structure with zero new dependencies, minimal-integer and dense-covering rules, all CRCs Sealr-verified, `sealrTreeV12`, cross-container content parity, and fail-closed worker refusal |
