...
Status: accepted; shared owning planning seam, dormant records, hostile decoders, isolated sealed inspect execution, supervisor-owned content replay, and immutable original-pass retained-content transfer implemented.
...
Only the private semantic-record experiment is accepted now. Immutable original-pass retained-content transfer has landed inside that repository lab. The content-read backend and public packaging model remain explicit gates.
