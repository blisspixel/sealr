...
Status: accepted and activated behind the explicit supported Linux supervisor; shared owning planning, private records, hostile decoders, a self-bound generic worker adapter, isolated sealed inspect and materialize execution, supervisor-owned content replay, immutable original-pass retained-content transfer, and isolated full and prefix non-retained reads are implemented.
...
The [private semantic-record implementation](../semantic-record.md) is compiled inside the library crate and remains outside the supported type surface. The explicit Linux `apply_supervised` path invokes it internally, while ordinary `apply`, `apply_with_options`, and the CLI remain in-process. It uses independent magic and a canonical bounded binary format. Planning carries exact invocation binding plus complete pending IR in central-directory order. Completion carries exact operation, request, and plan correlation plus one verification state for every planned member. The supervisor derives non-effect completion axes from the first error and verified-prefix state rather than trusting duplicated axes. Source-owning inspect and materialize executors consume an accepted Ready record by value, read only planned payload ranges through the shared bounded verifier, and emit canonical completion bytes without structural reparse.

The checkpoint pins canonical vector digests and covers hostile truncation, trailing data, kind confusion, binding mutation, stale correlation, path and topology state, range overflow, partial-frontier coherence, member-specific failure reachability, hostile labels, and destination-setup merge semantics. Executor evidence adds zero structural parser calls, exact complete and stopped parity for reachable Store and Deflate outcomes, and rejection of ineligible work before payload verification. The authenticated helper uses one generic adapter bound to the plan's actual profile, policy identity, budget, target, consumer, effect, member-sync, target identity, and retention fields. After worker exit and reap, the supervisor replays the plan against its retained exact source and requires byte-for-byte canonical completion and retained-content equality before reconstructing outcome or capability state. Separately bound full-read and prefix-read request kinds create a fresh restricted worker for each caller-bounded non-retained read, transfer no stage or destination authority, preserve the originating inspect or materialize effect, and release no bytes before complete stream integrity, clean exit, and reap agree. Prefix reads retain only the requested prefix while authenticating the discarded tail. Supervised materialization gives the worker only the supervisor-created stage root and sealed plan and leaves exact audit and no-replace publication solely to the supervisor. Targeted faults and 500 repeated writer lifecycles cover quiescence, audit, cleanup, destination races, crashes, and descriptor stability. An immutable 12-case v1 artifact plus 12 additive v2 cases pin a bounded projection. Real-kernel failure evidence and CLI and packaged-consumer activation have landed. Broader format parity, accumulated fuzz history, and external adoption remain open gates.
...
The private semantic-record design and its explicit Linux activation are accepted. Immutable inspect and materialize retained-content transfer, the public worker-backed `VerifiedArchive`, full and prefix content reads, the reaped materializing-writer lifecycle, authenticated child-only helper, fixed release packaging, real-kernel setup-failure evidence, and CLI and packaged-consumer activation have landed. Broader format parity and independent adoption remain explicit gates.
...
Do not add `Deserialize` or a public constructor to `ArchiveIR`. Conversion from a fully validated planning record into crate-private `ArchiveIR` and outcome evidence remains crate-private and associated with the supervisor-owned snapshot. The end-to-end capability review is complete for the explicit Linux supervisor, which may construct `VerifiedArchive` only after source-derived replay and clean worker reap. The materializing-writer lifecycle, authenticated child-only helper, release packaging, and materialization retention parity have landed.
...
The supported Linux boundary uses one fresh worker per later content read. Separate canonical full-read and prefix-read request kinds bind a fresh read operation to the accepted operation, request, plan, exact completion digest, member index, canonical path, caller limit, and originating effect. The supervisor reserves only the bytes it can return. Clones serialize through a shared coordinator, cancellation is sticky, a crash returns no partial bytes, and no idle worker survives between calls or after the last owner. The worker receives no stage or destination and writes the complete member only to a backpressured write-only pipe. The supervisor counts and hashes the complete stream, retains only the requested prefix when applicable, and releases the private buffer only after exact EOF, correlated success, size, CRC32, SHA-256, retained length, clean exit, and reap agree. Retained bytes remain immutable local slices captured during original verification. A later read cannot masquerade as original-pass retention.
...
This decision does not claim that structural parsing is confined, that a complete IR is an independently checkable certificate, or that same-user Linux processes are contained. The isolated full and prefix reader is the hidden backend for `VerifiedArchive` returned by the explicit Linux supervisor, and the helper has a fixed native package contract. Real-kernel containment-floor evidence and CLI and packaged-consumer activation have landed. Broader format parity and external adoption remain open.
