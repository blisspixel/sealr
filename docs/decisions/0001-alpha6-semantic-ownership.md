...
The private semantic-record design and its explicit Linux activation are accepted. Immutable inspect and materialize retained-content transfer, the public worker-backed `VerifiedArchive`, one-shot content reads, the reaped materializing-writer lifecycle, authenticated child-only helper, and fixed release packaging have landed. Real-kernel setup-failure evidence and CLI and packaged-consumer activation remain explicit release gates.
...
This decision does not claim that structural parsing is confined, that a complete IR is an independently checkable certificate, or that same-user Linux processes are contained. The one-shot reader is now the hidden backend for `VerifiedArchive` returned by the explicit Linux supervisor, and the helper has a fixed native package contract. Broader executable parity, real-kernel containment evidence, and consumer activation remain required before the Alpha.6 release claim.
