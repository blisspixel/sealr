...
Only the private semantic-record experiment is accepted now. Immutable original-pass retained-content transfer and the one-shot content-read backend have landed inside that repository lab. Public `VerifiedArchive` integration, the materializing-writer lifecycle, and the public packaging model remain explicit gates.
