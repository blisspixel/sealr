# Threat model (as of 30 August 2026)
...
For path ingest, replacement of the caller pathname after open is in scope. Sealr opens once, copies under the input cap into a random native-private directory, hashes exact copied bytes, checks opened-source length and observable modification state, then rereads the opened source in full and requires its length and SHA-256 to match the copy. This detects persistent drift observed by the second pass without relying only on timestamp resolution. Sealr reopens its own file read-only, removes that filename, and performs all interpretation and verification against the retained unnamed handle. Truncation, cap growth, same-length mutation, path replacement, short reads, and interruption have deterministic regressions. A same-principal actor that mutates the caller-owned file after the reread, or arranges transient mutation that the two passes do not observe, remains outside the current in-process containment claim and needs broader native stress plus a reduced-identity boundary.
...
Member operations must not follow a substituted link or reparse point. Root, an administrator, a principal matching the effective token's default-owner SID, a process running as the same security principal, a process with take-ownership, restore, or filesystem-override capabilities, or a process with debugging or handle-duplication rights is outside the containment promise of an in-process library. The explicit x86_64 Linux worker narrows its own authority for supported ZIP32 payload verification, stage writes, and later reads, but it does not constrain another same-user process. Bringing that actor into scope requires a distinct service identity or equivalent mandatory-access-control boundary. Receipts expose the selected storage, stage ACL, member-resolution, publication, outcome, and cleanup controls, but remain unsigned and therefore are evidence rather than authenticated attestations.

Every worker response is hostile input to the supervisor. [Worker protocol v1](worker-protocol.md) bounds frames, counts, strings, canonical paths and topology, state combinations, capability slots, operation correlation, and representable request resource claims before effect. It does not return a complete `ArchiveIR`, source or policy binding, or later verified-read authority, so it cannot become a public outcome format by itself. The supported Linux path instead uses the private [semantic-record boundary](semantic-record.md). Its supervisor validates the selected complete contract, waits for clean exit and reap, independently replays source-derived results, audits the quiescent stage through retained capabilities, and retains publication authority. Neither codec alone proves that a transport attached the intended handles or that their operating-system rights are minimal.
...
| Fix unique/outlier behaviors | A versioned interpretation specification and executable behavior must agree. Alpha.12 has fifteen preview profile identities: four ZIP32 profiles, one strict ZIP64 profile, raw portable ustar, strict gzip-wrapped portable ustar, restricted raw PAX, restricted raw GNU long-name TAR, two exact gzip compositions, three promoted codec wrappers, and Copy-only 7z. None is yet stable. |
...
Sealr is not an antivirus or package inventory system. It does not claim CRC is a signature and does not run competing parsers during an invocation. Alpha.12 explicitly selects exactly one ZIP32, strict ZIP64, raw or wrapped restricted TAR, or Copy-only 7z path with versioned rules and returns a structured finding at the deterministic refusal point. ZIP64, every TAR selection, and 7z are in process only under their exact policy versions; supervised selection fails closed before source access and without fallback until later semantic records can bind their evidence. A rejected view may be partial and must not be treated as a complete inventory.
