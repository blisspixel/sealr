...
As of 2026-08-26, no scheduled category is eligible or promoted. Coverage-guided fuzzing has one recorded successful scheduled `main` run. Kani and the native 3 GiB resource gate have zero qualifying scheduled runs. Mutation and coverage have zero runs and remain non-promotable. Required CI therefore remains one strict protected authority without importing any unstable scheduled job.
