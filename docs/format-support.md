...
| gzip | exact RFC 1952 wrapper profile | Deflate | Single versus concatenated members and trailing data are separate rules | Internal bounded transform and fuzz infrastructure; not a public format | Required before public promotion | Reuse existing `flate2` |
...
Format-specific evidence is not forced into ZIP field names. ZIP32 keeps `sealr.archive-ir.v1` and `sealrTreeV1` byte-for-byte stable. Portable ustar uses `sealr.archive-ir.tar-ustar.v1`, exact TAR member and covering evidence, and `sealrTreeV2` for layout. Strict ZIP64 uses `sealr.archive-ir.zip64.v1`, ZIP64-native member and covering evidence, and `sealrTreeV3` for layout. All three retain the format-neutral `sealrTreeV1` content encoding.
