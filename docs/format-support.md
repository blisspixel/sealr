...
| 7z Copy container | local raw-header Copy-only interpreter | Copy | Minimal NUMBER encodings, dense covering, all CRCs Sealr-verified, packed headers and every non-Copy coder denied with named remedies | In-process preview on main under policy v11 | Fails closed until a later semantic record | Zero new runtime dependencies; LZMA members and packed headers are the named next Gate C steps on the reviewed `lzma-rust2` boundary |
