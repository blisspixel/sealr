...
| Python wheel admission | Supported preview plus packaged repository conformance landed | The public evaluator consumes only `VerifiedArchive`. An isolated extracted-package project drives exact PyPA installer 1.0.1 through real target effects after source deletion and audits the realization identity. A hash-pinned Poetry 2.4.2 fixture proves one injected update seam, including target-read-only preflight, real uninstall ordering, abort safety, symlink-ancestor denial, and stock parity. Independent adoption remains open. |
...
| Independent evidence verifier and standard authenticated envelopes | Identity and live canonical-evidence verification landed; authenticated verifier policy remains scheduled | The small no-Sealr-dependency tool checks every committed profile and root family plus live canonical v2/v3 evidence, pair and source bindings, axes, and content roots. Standard signature systems remain outside it. |
...
| Strict ZIP64 | Explicit Alpha.10 in-process preview | Policy v3 authorizes a separate strict profile with ZIP64-native IR, covering, and `sealrTreeV3`; ZIP32 does not alias, and the worker refuses it until semantic-record v3. |
| Strict gzip-wrapped portable ustar | Explicit Alpha.10 in-process preview | Policy v4 authorizes one exact RFC 1952 member and portable ustar interpretation over separate immutable source and derived domains, with a full-source transform, three audits, and `sealrTreeV4`; the worker refuses it until a later semantic record. |
...
| Restricted raw GNU long-name | Explicit Alpha.12 in-process preview | Policy v6 authorizes exact old-GNU magic with one bounded `L` carrier per member, carrier provenance, an independent covering replay, and `sealrTreeV6`; the worker refuses it until a later semantic record. |
| Gzip-wrapped restricted PAX and GNU compositions | Explicit Alpha.12 in-process previews | Policy v7 authorizes each frozen raw dialect behind the exact single-member gzip transform with distinct `sealrTreeV7` and `sealrTreeV8` identities and the shared content root; the worker refuses both until a later semantic record. |
| Zstd-wrapped portable ustar | Explicit Alpha.12 in-process preview | Policy v8 authorizes the first promoted codec adapter: one RFC 8878 frame through the reviewed two-package ruzstd boundary with an 8 MiB window ceiling, cross-checked dual header interpretation, `sealrTreeV9`, and the shared content root; the worker refuses it until a later semantic record. |
| Xz-wrapped portable ustar | Explicit Alpha.12 in-process preview | Policy v9 authorizes the second promoted codec adapter: one XZ stream of LZMA2-only blocks through the reviewed one-package lzma-rust2 boundary with an 8 MiB dictionary ceiling, twice-verified checks, a footer-first independent container parse, `sealrTreeV10`, and the shared content root; the worker refuses it until a later semantic record. |
| Bzip2-wrapped portable ustar | Explicit Alpha.12 in-process preview | Policy v10 authorizes the third promoted codec adapter: one bit-aligned bzip2 stream through the reviewed two-package libbz2-rs-sys boundary with format-capped decoder memory, an independent footer shift-scan and block-magic chain-fold replay, `sealrTreeV11`, and the shared content root; the worker refuses it until a later semantic record. |
| Copy-only 7z container | Explicit Alpha.12 in-process preview | Policy v11 authorizes the first Gate C container step: a raw-header Copy-only 7z with zero new dependencies, minimal-integer and dense-covering rules, every container CRC Sealr-verified, `sealrTreeV12`, and the first cross-container shared content root; the worker refuses it until a later semantic record. |
...
## Implemented evolution through Alpha.14
...
The Alpha.5 [worker protocol v1](worker-protocol.md) preparation encodes the selected source, profile, policy, resource limits, and authority slots without serializing the archive. Its reduced result manifest is a bounded, correlated claim for that standalone protocol. Request-bound validation checks the returned profile and representable manifest limits, but version 1 returns neither the source nor policy digest and carries neither a complete `ArchiveIR` nor the independent public outcome axes. It cannot construct `Outcome` or `VerifiedArchive`, preserve later bounded member reads, or support a claim that the supervisor independently verifies archive semantics. The supported Alpha.14 worker path uses the separate private semantic-record boundary described below.

The boundary introduced in Alpha.6 has a production-compiled, crate-private owning planner and [private split-phase semantic records](semantic-record.md), and current main extends it with supervised prefix reads. After successful policy compilation, the planner acquires the exact snapshot, performs detection, parsing, admission, pending IR construction, and covering audit, then returns a terminal result or a non-cloneable Ready value with the snapshot and complete planning context. Ordinary `apply()` consumes Ready directly into its in-process continuation. The supported Linux `apply_supervised` path encodes the same ready plan for an authenticated restricted helper, then accepts completion only after clean reap and exact source-derived replay. Planning records bind the complete invocation and pending source-ordered IR; completion binds one verification vector to the exact request and plan. Source-owning inspect and materialize executors read only planned Store and Deflate payload ranges through the shared bounded verifier without structural reparse. The materialize executor writes only through a supervisor-created stage capability. Later non-retained full and prefix reads bind a fresh operation, accepted plan, exact completion, source-order member index, canonical path, caller limit, request kind, and originating inspect or materialize effect. The worker streams the complete member, and the supervisor releases no full result or retained prefix until exact EOF, correlated success, complete size, CRC32, SHA-256, clean exit, and reap agree. Planning and completion records carry no final receipt, verdict, publication, or cleanup claim; those remain supervisor-owned. A required pinned-kernel QEMU gate proves both public effects fail closed on actual Landlock ABI 2. CLI and general extracted-package-consumer activation, including supervised wheel script-prefix classification, are complete. Broader format parity and global operation replay control remain open. Protocol v2 is not required for this private boundary.
