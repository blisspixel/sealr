...
| Xz-wrapped portable ustar | Explicit current-main in-process preview | Policy v9 authorizes the second promoted codec adapter: one XZ stream of LZMA2-only blocks through the reviewed one-package lzma-rust2 boundary with an 8 MiB dictionary ceiling, twice-verified checks, a footer-first independent container parse, `sealrTreeV10`, and the shared content root; the worker refuses it until a later semantic record. |
| Bzip2-wrapped portable ustar | Explicit current-main in-process preview | Policy v10 authorizes the third promoted codec adapter: one bit-aligned bzip2 stream through the reviewed two-package libbz2-rs-sys boundary with format-capped decoder memory, an independent footer shift-scan and block-magic chain-fold replay, `sealrTreeV11`, and the shared content root; the worker refuses it until a later semantic record. |
| Copy-only 7z container | Explicit current-main in-process preview | Policy v11 authorizes the first Gate C container step: a raw-header Copy-only 7z with zero new dependencies, minimal-integer and dense-covering rules, every container CRC Sealr-verified, `sealrTreeV12`, and the first cross-container shared content root; the worker refuses it until a later semantic record. |
