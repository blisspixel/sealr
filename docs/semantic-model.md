...
Alpha.6 now also has a production-compiled, crate-private owning planner and dormant [private split-phase semantic records](semantic-record.md). After successful policy compilation, the planner acquires the exact snapshot, performs detection, parsing, admission, pending IR construction, and covering audit, then returns a terminal result or a non-cloneable Ready value with the snapshot and complete planning context. Ordinary `apply()` consumes Ready directly into its existing retention, setup, verification, materialization, and outcome continuation. The in-crate conformance harness consumes a separate invocation of the same planner to author record evidence; public `apply()` never traverses the record codec. Planning records bind the complete invocation and pending source-ordered IR, enforce parser-equivalent resource accounting, and reproduce covering evidence against the exact supervisor-owned snapshot. Completion binds one full verification vector to the exact request and plan, then derives interpretation, admission, verification, and completeness from the validated frontier and first error. Source-owning inspect and materialize executors consume accepted Ready records, read only planned Store and Deflate payload ranges through the same bounded verifier as the public path, and emit canonical completion evidence without structural reparse. The materialize executor writes only through a supervisor-created stage capability and captures requested retained bytes through those same verifier calls. The repository-only sealed Linux bridge executes those plans under restriction through an authenticated child-only helper for normal cases. After worker reap, the supervisor replays each accepted plan against the retained exact source, requires byte-for-byte canonical completion and retained-content equality, audits the exact materialized stage, and alone performs no-replace publication. A separate one-shot member-read request binds a fresh operation, accepted plan, exact completion, member index, canonical path, and caller limit. The supervisor gives each fresh worker no stage or destination and releases no output until exact EOF, correlated success, integrity, clean exit, and reap agree. An immutable 12-case v1 artifact and 12 additive v2 cases compare record-owned fields with named public outcomes, plan-native completion, exact backend twins, or a separately labeled supervisor covering reproduction. The planning and completion records deliberately carry no final effect, receipt, verdict, publication, cleanup, or public retained capability. The member-read request supplies only private per-call authority. Broader parity, global operation replay control, public `VerifiedArchive` integration, and real-kernel setup-failure evidence remain separate gates before protocol v2 or public worker mode. Fixed release packaging and inspect and materialize retention parity are complete.
