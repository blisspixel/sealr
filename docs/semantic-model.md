...
Alpha.6 now also has a dormant [private split-phase semantic record](semantic-record.md). Planning binds the complete invocation and pending source-ordered IR, enforces parser-equivalent resource accounting, and reproduces covering evidence against the exact supervisor-owned snapshot. Completion binds one full verification vector to the exact request and plan, then derives interpretation, admission, verification, and completeness from the validated frontier and first error. A source-owning, non-cloneable inspect executor consumes each accepted Ready plan value, reads only its planned payload ranges through the same bounded Store and Deflate verifier as the public path, and emits canonical completion frames without structural reparse. An immutable 12-case v1 artifact and 12 additive v2 cases compare record-owned fields with named public outcomes, plan-native completion, exact backend twins, or a separately labeled supervisor covering reproduction. Ready cases use the current post-covering test capture; a real shared production plan seam remains the next step. The record deliberately carries no final effect, receipt, verdict, publication, cleanup, retained bytes, or later read authority. Broader parity, global operation replay control, transport and process isolation, content authority, retained-content transfer, isolated non-retained reads, lifecycle, and packaging remain separate gates before protocol v2 or public worker mode.
