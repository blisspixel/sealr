...
> This page separates the architecture implemented on current main from the target semantic architecture. Platform-general projection and process isolation, acceleration, stable lock semantics, and an expanded shipping crate graph are not current features. The explicit x86_64 Linux reduced-authority boundary is current, but it does not confine structural planning. See [semantic-model.md](semantic-model.md) for the normative target.

## Implemented on current main

Sealr is an eleven-package Rust workspace. The library and CLI are the release-facing components; nine packages remain repository-only protocol, conformance, bootstrap, release-support, lifecycle, evidence, or compatibility tools:
...
2. Compile policy authorization for the explicit `ArchiveSelection`, then invoke exactly one of the twelve supported container and codec selections. Selection does not come from a filename, and a failed parse is never retried through another format. Policy v3 through v11 authorize their exact post-ZIP32 profiles; the ZIP32 default never aliases to another selection.
...
Current support has twelve explicit selections: ZIP32, strict ZIP64, raw portable ustar, restricted raw PAX, restricted raw GNU long-name TAR, gzip-wrapped ustar, gzip-wrapped PAX, gzip-wrapped GNU long-name TAR, zstd-wrapped ustar, XZ-wrapped ustar, bzip2-wrapped ustar, and Copy-only 7z. Each uses a separately named profile and layout identity, and none widens the ZIP32 default. Wrapped formats retain the caller source as domain 0 and one bounded decoded container as domain 1, bind them through one exact transform, and independently audit both domains. The authenticated worker supports only its existing ZIP32 semantic-record v2 boundary and rejects every other selection without fallback until later records can represent their evidence. Encryption, unsupported codecs or dialect features, spanned archives, recovery parsing, recursive nested extraction, links, devices, and unsupported structures fail closed. The [README](../README.md) is authoritative for the complete support and limitation list.
