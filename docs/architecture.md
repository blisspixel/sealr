...
Alpha.6 first proves a Linux authority bootstrap: a trusted supervisor creates the private stage and immutable snapshot, authenticates a distinct child-only helper, passes it only bounded source and optional stage capabilities, installs restrictions, observes readiness, then exits and reaps the child. This tests executable identity, descriptor transfer, closure, `no_new_privs`, and runtime-probed Landlock before an incomplete result format shapes the public API. The lab now carries canonical operation-bound planning, completion, retained-content, and member-read request records through bounded kernel-sealed memfds with exact length, seal, digest, descriptor, lifecycle, and rejection evidence. Its feature-gated [semantic-record](semantic-record.md) bridge validates a full plan against the restricted worker's exact file-backed snapshot, executes planned Store and Deflate payload ranges without structural reparse, captures supervisor-selected bytes during that pass, and retains source authority through supervisor observation and reap. The supervisor decodes both outputs first as untrusted proposals, then replays the accepted plan against the retained exact source descriptor and requires byte-for-byte canonical agreement. A separate one-shot read boundary gives each caller-bounded non-retained read a fresh worker with no stage or destination authority and releases no bytes until exact output, correlated success, integrity, clean exit, and reap agree. The private materialize route gives the worker only a supervisor-created stage root and sealed plan, requires clean exit and exact reap before validation, source replay, and exact stage audit, then allows only the supervisor to publish with no replacement. Public capability integration, runtime activation, real-kernel setup failure, and release packaging remain separate. Equivalent credible isolation boundaries for macOS and Windows require separate work.
