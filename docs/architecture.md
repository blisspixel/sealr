...
Current main also compiles and tests a dormant crate-private [semantic-record experiment](semantic-record.md). Its independent bounded format carries an exact invocation-bound pending IR in planning order and a plan-digest-bound full verification vector in completion order. The adapter reconstructs ready IR only after structural, range, path, parser-equivalent quota, profile, finding, phase, and correlation checks, plus covering, LFH and CDH variable-length geometry, name, and extra-header reproduction against the exact supervisor-owned snapshot, then derives non-effect completion axes. No production call path invokes it, and it does not construct public outcomes or verified capabilities.
