...
Current main also contains a separate repository-only Linux authority-bootstrap lab. It proves a private same-binary process boundary with `SOCK_SEQPACKET`, raw exhaustive ancillary-header validation, immediate ownership of received `SCM_RIGHTS` descriptors, inherited-descriptor closure, fixed Landlock ABI 3 enforcement, an x86_64 seccomp-BPF no-descendant and permission-mutation deny set before source transfer, procfs descriptor and filter observation, and pidfd-backed reap. It has no archive-parser dependency, does not use operation protocol v1, is absent from release archives, and does not alter the in-process product trust boundary.
