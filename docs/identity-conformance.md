...
- the separate `sealr-identity-verifier` workspace tool consumes those recorded facts without depending on the `sealr` crate. It checks every committed identity family through `sealrTreeV12` and independently reconstructs each profile, layout, and content preimage. Its live mode also checks exact canonical view v2 and receipt v3 bytes, their shared claims, an optional observed source, registered interpretation and known default-policy identities, effect consistency, and the format-neutral content root.

The canonical artifacts are the [ZIP32 v1 conformance manifest](../crates/sealr/tests/conformance/identity-v1.json), the [ZIP64 v1 conformance manifest](../crates/sealr/tests/conformance/zip64-identity-v1.json), the [portable ustar profile vector](../crates/sealr/tests/conformance/tar-ustar-portable-v1.json), the [TAR layout v2 vector](../crates/sealr/tests/conformance/tar-layout-v2.json), the [gzip-TAR TreeV4 manifest](../crates/sealr/tests/conformance/tar-gzip-identity-v1.json), the [restricted PAX profile vector](../crates/sealr/tests/conformance/tar-pax-profile-v1.json), the [PAX TreeV5 manifest](../crates/sealr/tests/conformance/tar-pax-identity-v1.json), the [GNU long-name profile vector](../crates/sealr/tests/conformance/tar-gnu-longname-profile-v1.json), the [GNU TreeV6 manifest](../crates/sealr/tests/conformance/tar-gnu-longname-identity-v1.json), the [gzip-PAX TreeV7 manifest](../crates/sealr/tests/conformance/tar-gzip-pax-identity-v1.json), the [gzip-GNU TreeV8 manifest](../crates/sealr/tests/conformance/tar-gzip-gnu-longname-identity-v1.json), the [zstd TreeV9 manifest](../crates/sealr/tests/conformance/tar-zstd-identity-v1.json), the [xz TreeV10 manifest](../crates/sealr/tests/conformance/tar-xz-identity-v1.json), the [bzip2 TreeV11 manifest](../crates/sealr/tests/conformance/tar-bzip2-identity-v1.json), the [7z TreeV12 manifest](../crates/sealr/tests/conformance/sevenz-copy-identity-v1.json), the [canonical evidence manifest](../crates/sealr/tests/conformance/evidence-v1.json), the [independent verifier](../tools/identity-verifier), and the production [ZIP32](../crates/sealr/tests/golden_identity.rs), [ZIP64](../crates/sealr/tests/zip64_public_api.rs), [raw TAR](../crates/sealr/tests/tar_public_api.rs), [gzip-TAR](../crates/sealr/tests/tar_gzip_public_api.rs), [PAX](../crates/sealr/tests/tar_pax_public_api.rs), [GNU long-name](../crates/sealr/tests/tar_gnu_longname_public_api.rs), [gzip-PAX](../crates/sealr/tests/tar_gzip_pax_public_api.rs), [gzip-GNU](../crates/sealr/tests/tar_gzip_gnu_longname_public_api.rs), [zstd](../crates/sealr/tests/tar_zstd_public_api.rs), [xz](../crates/sealr/tests/tar_xz_public_api.rs), [bzip2](../crates/sealr/tests/tar_bzip2_public_api.rs), and [7z](../crates/sealr/tests/sevenz_public_api.rs) tests.
...
The verifier does not inflate payloads, implement a general archive-discovery parser, prove SHA-256, or authenticate a signer. For committed format manifests it parses only the exact bounded fixture languages and finite evidence. For live evidence it does not reconstruct the format-specific layout root or recompute member hashes from compressed source payloads. Production verification supplies those claims; the standalone tool checks their canonical binding and independently reconstructs the format-neutral content root.

## Canonical evidence contract

`sealr.evidence-conformance.v1` contains five exact view v2 and receipt v3 pairs: admitted inspect, admitted committed materialization, structural rejection, source unavailable, and admitted setup failure. The verifier rejects duplicate JSON properties, unknown fields, non-RFC-8785 bytes, trailing whitespace, pair drift, observed-source mismatch, default-policy digest drift, impossible outcome states, malformed findings, member identity mutations, and content-root mismatch.
...
## Bzip2-wrapped ustar `sealrTreeV11`

The bzip2 layout uses the label `sealr.tree.layout.tar-bzip2-ustar.v1`. Its body binds the bzip2 transform identifiers, both domain identities, the level, header range, total payload and padding bits, every scan-verified block bit offset and CRC, the combined CRC, and the complete inner portable-ustar layout body — bit-granular geometry, because bzip2 blocks are not byte-aligned. Its manifest carries pinned deterministic producer cases at levels 9 and 1 over one committed derived TAR plus a separate three-block multi-block section over its own committed derived TAR (replayed under a documented widened expansion ratio, since the identity roots are policy-independent), and the verifier independently replays the container grammar — footer shift-scan with a unique-match requirement, full block-magic scan, per-block CRC and randomised-flag extraction, and the combined-CRC chain fold — re-hashes single-block derived bytes with a self-contained bzip2-variant CRC32, and reconstructs `sealrTreeV11`, the raw `sealrTreeV2` relationship, and the shared `sealrTreeV1` content root without a decompressor.

## Copy-only 7z container `sealrTreeV12`

The 7z layout uses the label `sealr.tree.layout.7z-copy.v1`. Its body binds the profile digest, the source identity, the dense covering ranges (signature, contiguous pack streams, raw next header), every folder and substream record with declared CRCs, name and dummy geometry, and each member's destination meaning with its evidence-only container facts. Its manifest carries three measured 7-Zip 26.02 raw-header producer cases — single file, directory-plus-file, and the full empty-stream/empty-file matrix over two Copy folders — each embedding its complete serialized IR, plus a pinned cross-container assertion: the single-file case's `sealrTreeV1` content root equals the TAR family's shared root, the first parity proof across containers rather than wrappers. The verifier independently re-parses the complete container grammar — signature and both header CRCs, minimal variable-length integers, MSB-first bit vectors, folders, substream tiling, the empty matrix, and exact UTF-16 name consumption — verifies every declared CRC32 and member hash itself, and reconstructs `sealrTreeV12` and the shared content root without linking Sealr.
...
cargo run --locked -p sealr-identity-verifier -- crates/sealr/tests/conformance/tar-bzip2-identity-v1.json
cargo run --locked -p sealr-identity-verifier -- crates/sealr/tests/conformance/sevenz-copy-identity-v1.json
cargo run --locked -p sealr-identity-verifier -- crates/sealr/tests/conformance/evidence-v1.json
...
cargo test --locked -p sealr --test tar_bzip2_public_api
cargo test --locked -p sealr --test sevenz_public_api
```

Required CI runs the production comparison, canonical-evidence corpus, verifier tamper tests, and verifier commands. A change to a source fixture, serialized IR, canonical view or receipt, profile bytes, semantic state, finding, range, or root therefore needs one deliberate manifest review.
