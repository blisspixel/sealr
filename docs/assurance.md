...
| Linux authority bootstrap | A repository-only same-binary lab structurally transfers the source only after fixed Landlock ABI 3 enforcement, observes the exact paused-child descriptor set, exercises outside denial and stage-local creation, rejects authority and correlation drift, and kills plus reaps through pidfd on a bounded timeout | Add insufficient-ABI and setup-failure seams, ancillary truncation at the process boundary, crash barriers, no-descendant and permission-mutation controls, repeated native stress, and semantic worker integration |
