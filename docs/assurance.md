...
| Private semantic record | Dormant crate-private planning and completion frames use independent magic, exact invocation and plan binding, a 64 MiB encoded-length cap, hostile validation, exact re-encoding, pinned vectors, a versioned 12-case in-process shadow matrix, focused strict-v2 and private-file parity, a source-owning non-cloneable inspect executor with no additional structural parse, and a dedicated fuzz target with clean exact-main on-demand evidence. A required isolated-child probe pins a 67,041,104-byte plan and measures accepted Complete and last-member Stopped reconstruction. The local Windows Complete sample added 16,752 transient bytes above its 89,486,520-byte logical output; the maximum-label and maximum-detail Stopped sample added 83,367 peak bytes above its 89,486,480-byte logical reconstruction. Stale and late-invalid controls perform zero IR materializations. A decoded completion remains an untrusted worker proposal; binding does not prove file-content computation. | Expand parity beyond the bounded projection; close pre-parser authority controls; add a real plan-only seam, sealed plan and completion transport, isolated execution, independent exact-byte content authority, retained-content transfer, isolated later reads, and process lifecycle integration |
