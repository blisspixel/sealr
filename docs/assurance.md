...
| Fuzz | `protocol_decoders` exercises arbitrary start and result frames plus input-directed mutations of valid frames under pinned bounds; its first campaign completed 18,277,565 units in 601 seconds at 503 MiB peak RSS with no reproducer. `semantic_records` exercises arbitrary planning and completion bytes, mutations of every canonical record kind, repeated-decode stability, Ready-plan equality with production pending IR, and stale correlation from four digest-pinned seeds. Its first exact-main on-demand campaign completed 194,476 units in 601 seconds at 323 executions per second and 512 MiB peak RSS with no reproducer. The same run revalidated `protocol_decoders` for 11,199,176 units at 18,634 executions per second and 507 MiB peak RSS. Required verification binds the complete Cargo manifest, parsed targets, lock checksum, and registry-rooted crates.io fuzz engine while refusing Cargo configuration, then compares the complete scheduled workflow, including its weekly trigger, permissions, concurrency, exact shell programs, and failure-artifact fields. Executable negative fixtures reject inert TOML remapping, local, patched, or vendored fuzz-engine substitution, manual-only drift, weakening, inactive or appended commands, inert evidence, and raw or quoted duplicate last-wins arguments. | Add targets for interpretation, canonical paths, covering, codecs, and inspect-only `apply()`; complete the first scheduled-event run; accumulate scheduled history |
...
| Private semantic record | Dormant crate-private planning and completion frames use independent magic, exact invocation and plan binding, a 64 MiB encoded-length cap checked before buffer-growth requests and copies, supervisor-bound path-depth limits before owned component allocation, member-name-derived normalization reservation bounds, checked hostile ranges, supervisor-snapshot covering reproduction, source-checked represented LFH, CDH, descriptor, name, and extra-field evidence, signature exclusion over comments and stored descriptor payloads, member-kind agreement with source attributes, parser-oracle metadata accounting, phase-specific finding validation, pending source-ordered IR, a full verification vector, exact re-encoding, pinned vectors, deterministic regressions, a dedicated fuzz target, and one clean exact-main on-demand campaign | Add broader in-process shadow parity, validated-plan execution without structural reparse, sealed immutable transport, retained-content authority, isolated later reads, and process lifecycle integration |
