...
| Public API and package | External-crate fixtures exercise evidence types, `VerifiedArchive`, bounded retention, portable UTF-8 profile selection, and `sealr::wheel` after source deletion; two separate Cargo consumers run against the extracted package; the PyPA consumer independently verifies canonical evidence, installs a controlled fixture and the exact installer 1.0.1 wheel from staged capability reads, audits all outputs, and proves negative gates; Linux package smoke authenticates the extracted helper and exercises supervised inspect, materialize, retained and one-shot reads, exact reap, and stage cleanup; scheduled compiler-accurate discovery compares current Rust-visible API with the package produced from the exact Alpha.11 commit through SHA-256-authenticated cargo-semver-checks 0.49.0, with three deliberate pre-freeze lint classes pinned to seven exact warnings | Remove the known-warning debt at a later release baseline, then start ten distinct successful scheduled `main` runs before promotion review; separately review behavioral and evidence-format compatibility before a stable release |
...
| Scheduled discovery | A weekly bounded workflow records cargo-mutants 27.1.0 outcomes for the three scalar kernels, cargo-llvm-cov 0.9.0 summary JSON for the `sealr` all-features tests, and cargo-semver-checks 0.49.0 results against the exact packaged Alpha.11 baseline. Reports are retained for 30 days. Mutation and coverage remain discovery-only; API compatibility also remains non-promotable while its exact known-warning file is nonempty. | Review missed mutants and blind branches as leads; remove API compatibility debt before starting promotion history; never report any result as a correctness or security score |
