...
| Public API and package | External-crate fixtures exercise evidence types, `VerifiedArchive`, bounded retention, portable UTF-8 profile selection, and `sealr::wheel` after source deletion; a separate Cargo consumer runs against the extracted packaged crate and exercises wheel evaluation; Linux package smoke authenticates the extracted helper and exercises supervised inspect, materialize, retained and one-shot reads, exact reap, and stage cleanup | Add semantic compatibility checks after publication |
| Corpus | All 5,927 pinned ZipDiff constructions, local generated adversarial fixtures, and three predecessor-bound reports over 20 exact PyPI wheels, including the Alpha.8 public supported-preview evaluator | Expand producer and feature diversity; target benign Unicode, `.data`, descriptors, codec boundaries, source races, and wheel semantics |
