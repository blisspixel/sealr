...
| Fuzz | `protocol_decoders` exercises arbitrary start and result frames plus input-directed mutations of valid frames under pinned tool, seed, input, time, timeout, RSS, job, and output bounds. The first campaign completed 18,277,565 units in 601 seconds, averaged 30,411 executions per second, reached 503 MiB peak RSS, and produced no reproducer. | Add targets for interpretation, canonical paths, covering, codecs, and inspect-only `apply()`; accumulate scheduled history |
...
| Private semantic record | Dormant crate-private planning and completion frames use independent magic, exact invocation and plan binding, a 64 MiB hard cap, pre-reservation count checks, checked hostile ranges, supervisor-snapshot covering reproduction, source-checked LFH and CDH variable-length geometry, names, and extra headers, parser-oracle metadata accounting, pending source-ordered IR, a full verification vector, derived non-effect axes, exact re-encoding, pinned vector digests, and deterministic truncation, correlation, mutation, phase, topology, and setup-merge regressions | Add a dedicated fuzz target, broader in-process shadow parity, sealed immutable transport, retained-content authority, isolated later reads, and process lifecycle integration |
