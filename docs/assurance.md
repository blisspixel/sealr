...
| Differential | The ZipDiff expectation gate binds strict rejection and a documented valid-control allowlist. The committed `semantic-shadow-v1` gate pins 12 strict-v1 memory-backed cases and compares production `apply()` with decoded semantic records at the real planning and completion boundaries, including exact IR where present, axes, frontier, ordered record-owned findings, and correlation identities. | Expand semantic-record parity across strict-v2, private-file, ignored-extra, topology, covering-terminal, and remaining quota cases; compare major consumers on well-formed profile inputs and track disagreement frontiers |
...
| Fuzz | `protocol_decoders` exercises arbitrary start and result frames plus input-directed mutations of valid frames under pinned bounds. `semantic_records` exercises arbitrary planning and completion bytes, mutations of every canonical record kind, repeated-decode stability, Ready-plan equality with production pending IR, and stale correlation from four digest-pinned seeds. On-demand run [32640986271](https://github.com/blisspixel/sealr/actions/runs/32640986271) passed against executable `main` commit `20f9afc0476dacb13099848e0324c03559b14492`. `semantic_records` completed 256,813 units in 601 seconds at 427 executions per second and 508 MiB peak RSS, reaching coverage 2,447, feature coverage 5,285, and corpus 420/6,370 bytes, with no crash or reproducer. `protocol_decoders` completed 13,896,832 units in 601 seconds at 23,122 executions per second and 510 MiB peak RSS, reaching coverage 857, feature coverage 1,882, and corpus 305/4,538 bytes, with no crash or reproducer. Required verification binds the complete Cargo manifest, parsed targets, lock checksum, and registry-rooted crates.io fuzz engine while refusing Cargo configuration, then compares the complete scheduled workflow, including its weekly trigger, permissions, concurrency, exact shell programs, and failure-artifact fields. Executable negative fixtures reject inert TOML remapping, local, patched, or vendored fuzz-engine substitution, manual-only drift, weakening, inactive or appended commands, inert evidence, and raw or quoted duplicate last-wins arguments. The event was `workflow_dispatch`; the first scheduled-event run and accumulated scheduled history remain pending. | Add targets for interpretation, canonical paths, covering, codecs, and inspect-only `apply()`; complete the first scheduled-event run; accumulate scheduled history |
...
| Linux authority bootstrap | A repository-only same-binary lab transfers the source only after a direct ABI-floor query and fixed Landlock ABI 3 enforcement, independently probes child-entry descriptor closure, observes exact paused-child descriptors, distinguishes short data from `MSG_TRUNC` and `MSG_CTRUNC` at the source boundary, exercises two deterministic restriction failures and 17 abrupt-exit barriers, and proves bounded reap before checked cleanup | Add authority-epoch stalls, raw unknown-ancillary validation, no-descendant and permission-mutation controls, repeated native stress, and semantic worker integration |
| Private semantic record | Dormant crate-private planning and completion frames use independent magic, exact invocation and plan binding, a 64 MiB encoded-length cap checked before buffer-growth requests and copies, supervisor-bound path-depth limits before owned component allocation, input-sized fallible or allocation-free path, topology, covering, extra-field, and reconstruction validation, checked hostile ranges, supervisor-snapshot covering reproduction without cloned diagnostic labels, source-checked represented LFH, CDH, descriptor, name, and extra-field evidence, signature exclusion over comments and stored descriptor payloads, member-kind agreement with source attributes, parser-oracle metadata accounting, phase-specific and member-reachable finding validation, pending source-ordered IR, a full verification vector, borrow-only completion validation, zero IR materializations for direct encode and invalid decode, exactly one fallible IR materialization for accepted decode, moved findings, exact re-encoding, pinned vectors, deterministic allocation and ownership regressions, a versioned 12-case in-process shadow matrix, a source-owning non-cloneable inspect executor that consumes each validated plan value and reads only planned payload ranges through the shared bounded verifier with no additional structural parse, exact resource-boundary controls, focused strict-v2 and private-file parity, and a dedicated fuzz target with clean exact-main on-demand evidence | Expand parity beyond the bounded projection; add an isolated allocator or process peak-live measurement, sealed plan transport and isolated execution, retained-content authority, isolated later reads, and process lifecycle integration |
