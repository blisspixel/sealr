...
> This page separates current evidence from the target assurance program. Alpha.4 through Alpha.6 establish the deterministic suite, strict cross-platform CI, cargo-deny policy, pinned ZipDiff gate, finite-domain properties, immutable snapshot, semantic-record shadow evidence, reduced-authority Linux boundary, native lifecycle oracle, fuzzing, and Kani spine. Alpha.7 adds the exact repository-only wheel profile, bounded evaluator, hostile fixtures, predecessor-bound inventory, and non-reopening bridge research. Alpha.8 adds the fourth independently verified profile vector, exhaustive portable UTF-8 flag and extra-field domains, public source-deletion consumer test, exact wheel consumer identities, and predecessor-bound public-surface replay over 20 pinned wheels. Alpha.9 adds independent portable ustar profile and layout verification, three external producer fixtures, a covering oracle, and a dedicated bounded TAR fuzz target. Alpha.10 adds independently reconstructed strict ZIP64 and two-domain gzip-TAR identities, public bounded fuzz targets, and exact dependency budgets. Alpha.11 adds a restricted raw POSIX PAX language, independently reconstructed extension-state replay, TreeV5 identity, producer fixtures, and its own bounded fuzz campaign without adding dependencies. Alpha.12 adds independent live canonical-evidence verification, five committed v2/v3 evidence cases, native verifier delivery, and an extracted-package PyPA installer 1.0.1 conformance kit over controlled and real pinned wheels. Alpha.13 adds full-integrity supervised prefix reads, the packaged public-API-only `WheelSource` handoff, and one exact Poetry 2.4.2 repository fixture with update ordering, abort, stock-output, and realization-identity evidence. Alpha.15 adds explicit Deflate completion regressions and a 24-artifact Unicode and streaming wheel producer matrix, including twelve complete supervised installer handoffs and eighteen exact negative cases. Accumulated scheduled history remains pending. Broader ecosystem sampling, topology fuzz targets, authenticated-envelope verification, independent adoption, and an external audit remain future work.
...
| Public API and package | External-crate fixtures exercise evidence types, `VerifiedArchive`, bounded retention, portable UTF-8 profile selection, and `sealr::wheel` after source deletion; two separate Cargo consumers run against the extracted package; the PyPA consumer independently verifies canonical evidence, installs a controlled fixture and the exact installer 1.0.1 wheel from staged capability reads, audits all outputs, and proves negative gates; Linux package smoke authenticates the extracted helper and exercises supervised inspect, materialize, retained, full, and prefix reads, exact reap, and stage cleanup; the general extracted-package consumer evaluates a supervised `.data/scripts` wheel through prefix classification; scheduled compiler-accurate discovery compares current Rust-visible API with the package produced from the exact Alpha.12 commit through SHA-256-authenticated cargo-semver-checks 0.49.0, with zero expected warnings | Accumulate ten distinct successful scheduled `main` runs before promotion review; separately review behavioral and evidence-format compatibility before a stable release |
| Corpus | All 5,927 pinned ZipDiff constructions, local generated adversarial fixtures, immutable predecessor reports, and the current 300-artifact stratified wheel inventory with 280 admissions and 20 individually investigated denials | Expand producer and feature diversity; target benign Unicode and descriptor-bearing wheels, codec boundaries, source races, and wheel semantics |
...
| Fuzz | `protocol_decoders` exercises arbitrary start and result frames plus input-directed mutations of valid frames under pinned bounds. `semantic_records` exercises arbitrary planning and completion bytes, mutations of every fuzzed canonical record kind, repeated-decode stability, Ready-plan equality with production pending IR, and stale correlation from four digest-pinned seeds. `tar_ustar_portable_v1` adds a 4 MiB self-cap, untouched raw bytes, checksum-repaired canonical header mutations, exact quota-frontier replays, source-geometry assertions, and inspect-only public application. Exact-main on-demand run [32938266865](https://github.com/blisspixel/sealr/actions/runs/32938266865) and scheduled-event run [32708222003](https://github.com/blisspixel/sealr/actions/runs/32708222003) passed the earlier two-target domain without crashes or reproducers. Those historical measurements remain evidence for that exact domain but do not count toward the new TAR-inclusive promotion sequence, which starts at zero. Required verification binds the complete Cargo manifest, every parsed target, lock checksum, and registry-rooted crates.io fuzz engine while refusing Cargo configuration, then compares the complete scheduled workflow, including its weekly trigger, permissions, concurrency, exact shell programs, and failure-artifact fields. Executable negative fixtures reject inert TOML remapping, local, patched, or vendored fuzz-engine substitution, manual-only drift, weakening, inactive or appended commands, inert evidence, and duplicate last-wins arguments. Both member-read request kinds have deterministic hostile coverage but are not yet included in `semantic_records`. | Add targets for ZIP interpretation, canonical paths, codecs, and both member-read request kinds; accumulate fresh scheduled history across every current job, including the raw-dialect and gzip-composition campaigns |
| Executable specification | A no-Sealr-dependency verifier reconstructs every committed profile and root family through `sealrTreeV12`; five canonical evidence cases cover admitted inspect, committed materialization, structural rejection, unavailable source, and failed effect; live verification checks exact RFC 8785 bytes, pair and source bindings, registered interpretations, known default policies, axes, effects, and the format-neutral content root | Add independent bounded models for paths and lifecycle transitions; independently reconstruct live format-specific layout claims; add authenticated-envelope verification only with an explicit verifier policy |
...
| Private semantic record | Crate-private planning, completion, retained-content, full-read, and prefix-read request frames use independent magic, exact invocation and plan binding, a 64 MiB encoded-length cap, hostile validation, exact re-encoding, pinned vectors, 24 manifest-pinned bounded shadow observations, source-owning inspect and materialize executors with no additional structural parse, and a dedicated fuzz target. The supported Linux supervisor carries those records through authenticated sealed worker handoffs, retains the exact source through observation and reap, treats worker output as a proposal, and accepts it only after exact source-derived replay. Read validation binds canonical order, operation, plan, completion, kind, caller bound, complete size, CRC32, SHA-256, and retained length. The public worker-backed `VerifiedArchive` keeps retained bytes local and uses one fresh restricted worker for each non-retained full or prefix read, including capabilities returned after supervised materialization. The materialize binding requires an opaque target digest; the worker receives only the source, sealed plan, and stage root, while post-reap audit and no-replace publication remain supervisor-owned. A required isolated-child probe pins a 67,041,104-byte plan and bounds transient requested heap for Complete and Stopped reconstruction. | Expand parity beyond the bounded projection; fuzz both member-read request kinds; add near-ceiling read and retained-transfer measurements; accumulate scheduled fuzz history |
| Generic worker semantics | One self-bound private adapter validates and executes the actual sealed-plan profile, policy identity, budget, target, consumer, effect, member-sync, target identity, and retention for inspect and materialize. The helper preserves complete and stopped verification outcomes, validates retained content without duplicating its bytes, and binds full and prefix reads to the exact plan and completion. The supervisor reconstructs owned outcome and retention state only from exact source-derived replay. Public fail-closed inspect, materialize, and worker-backed `VerifiedArchive` reads use this adapter. Focused evidence covers a custom policy, strict ASCII v1, exact custom ceilings, one-path retention, operation and effect drift, a bad-CRC stopped frontier, pre-worker destination setup failure, committed publication, retained reads, full reads, and prefix reads. | Expand end-to-end parity across supported public requests and lifecycle failures |
...
| Scheduled discovery | A weekly bounded workflow records cargo-mutants 27.1.0 outcomes for the three scalar kernels, cargo-llvm-cov 0.9.0 summary JSON for the `sealr` all-features tests, and cargo-semver-checks 0.49.0 results against the exact packaged Alpha.12 baseline. Reports are retained for 30 days. Mutation and coverage remain discovery-only; API compatibility is promotable from zero warning debt and begins a new scheduled history at zero. | Review missed mutants and blind branches as leads; accumulate the clean API history without weakening its gate; never report any result as a correctness or security score |
...
- Keep the protocol, semantic-record, and portable ustar fuzz targets, their separate seed manifests, and the scheduled campaigns reproducible.
