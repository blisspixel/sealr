...
| Private semantic record | Dormant crate-private planning and completion frames use independent magic, exact invocation and plan binding, a 64 MiB encoded-length cap checked before buffer-growth requests and copies, supervisor-bound path-depth limits before owned component allocation, input-sized fallible or allocation-free path, topology, covering, extra-field, and reconstruction validation, checked hostile ranges, supervisor-snapshot covering reproduction without cloned diagnostic labels, source-checked represented LFH, CDH, descriptor, name, and extra-field evidence, signature exclusion over comments and stored descriptor payloads, member-kind agreement with source attributes, parser-oracle metadata accounting, phase-specific finding validation, pending source-ordered IR, a full verification vector, borrow-only completion validation, zero IR materializations for direct encode and invalid decode, exactly one fallible IR materialization for accepted decode, moved findings, exact re-encoding, pinned vectors, deterministic allocation and ownership regressions, a versioned 12-case in-process shadow matrix, and a dedicated fuzz target with clean exact-main on-demand evidence | Expand parity beyond the pinned strict-ASCII-v1 borrowed-source projection; add an isolated allocator or process peak-live measurement, validated-plan execution without structural reparse, sealed immutable transport, retained-content authority, isolated later reads, and process lifecycle integration |
