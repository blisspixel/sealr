...
| Linux authority bootstrap | A repository-only same-binary lab transfers the source only after a direct ABI-floor query, fixed Landlock ABI 3 enforcement, and an x86_64 seccomp-BPF deny set. It directly probes no-descendant, execution, namespace, permission, ownership, xattr, and `ioctl` denial; observes filter mode and exact paused-child descriptors; parses every raw control header; rejects kernel-generated unknown ancillary, malformed layouts, and multiple rights records; distinguishes short data from `MSG_TRUNC` and `MSG_CTRUNC`; and proves descriptor closure on rejected descriptor-bearing packets. Canonical semantic plan, completion, and retained-content records cross through bounded memfds with required write, grow, shrink, and seal seals, independent length and SHA-256 verification, exact binding, four malformed-plan rejections, Store plus Deflate execution without structural reparse, and 30 original-pass retained fixture bytes. After worker reap, the supervisor replays the accepted plan against its retained exact source and requires byte-for-byte agreement for both outputs. Twenty-two abrupt-exit barriers and eleven changing-authority stalls require pidfd termination, proved reap, phase-appropriate state, and checked post-reap cleanup. A required 500-iteration native campaign repeats all 44 non-stall cases at least 11 times and checks child, descriptor, source, sentinel, reap, and cleanup state after every iteration. | Add real kernel setup-failure evidence, independently varied race stress, isolated later reads, and the remaining semantic lifecycle integration |
| Private semantic record | Dormant crate-private planning, completion, and retained-content frames use independent magic, exact invocation and plan binding, a 64 MiB encoded-length cap, hostile validation, exact re-encoding, pinned vectors, 24 manifest-pinned bounded shadow observations across v1 and v2, a source-owning non-cloneable inspect executor with no additional structural parse, and a dedicated fuzz target with clean exact-main on-demand evidence. A feature-gated repository bridge carries the full plan, completion, and supervisor-selected original-pass retained bytes through the sealed Linux worker boundary while retaining the exact source descriptor through observation and reap. Retention validation covers canonical status order, exact completion binding, size, CRC32, SHA-256, every truncation, content mutation, 64 paths, and a 63 MiB content ceiling without copying decoded member bytes. Proposal validation deliberately does not claim source computation. A forged canonical file-digest regression is accepted as a proposal, then rejected when the supervisor replays the accepted plan against the retained source and requires exact canonical equality. A required isolated-child probe pins a 67,041,104-byte plan and measures accepted Complete and last-member Stopped reconstruction. The local Windows Complete sample added 16,752 transient bytes above its 89,486,520-byte logical output; the maximum-label and maximum-detail Stopped sample added 83,367 peak bytes above its 89,486,480-byte logical reconstruction. Stale and late-invalid controls perform zero IR materializations. | Expand parity beyond the bounded projection; add isolated later reads, a retained-transfer resource measurement, and the remaining process lifecycle integration |
