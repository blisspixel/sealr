...
This is the target safety specification. [README.md](../README.md#security-limitations) and [ROADMAP.md](../ROADMAP.md) record current implementation status. Options described as future policy surfaces are not accepted by the Alpha.6 CLI.
...
7. Decode according to the selected format profile, then jail one canonical string. Compatibility v1 requires valid UTF-8 when ZIP bit 11 is set and accepts only ASCII when it is clear. Strict ASCII v2 denies bit 11 and every non-ASCII member name. CP437 transcoding and Unicode normalization remain unimplemented.
...
Current Alpha.6 behavior: **do not create them.** ZIP external attributes that describe a special file type are rejected, and file/directory attribute disagreements are rejected.
