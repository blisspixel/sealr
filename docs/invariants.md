...
Publishing a final destination is all-or-reject. Materialization stages into a same-volume directory and renames it to a previously absent destination only after every member passes. Unix stages are private by verified mode and ownership. Windows stages on supported local NTFS parents are created with a protected DACL owned by and granting only the effective token user, with inheritance to descendants. A normal failure never publishes the requested destination, attempts cleanup and one retry, and records whether the stage was removed or remains after both attempts fail.
