...
The Alpha.6 [private semantic-record implementation](semantic-record.md) is a separate internal format with independent `SEALRSEM` magic. The supported Linux supervisor uses it without revising, extending, or reinterpreting protocol v1. Deterministic tests reject cross-decoding between semantic planning and completion records; protocol v1 golden and fuzz surfaces remain unchanged.
...
Alpha.6 uses a separate authenticated Linux worker boundary without changing protocol v1. Normal cases authenticate a distinct child-only helper's exact bytes, sealed executable identity, private hello, and running process before bootstrap; deliberate fault cases alone use the lab executable. Bounded kernel-sealed memfds carry canonical operation-bound planning, plan-bound completion, completion-bound retained-content, and one-shot member-read request records. The private executor binds a source-owning Ready plan to the exact worker descriptor, verifies planned Store and Deflate ranges without structural reparse, and retains handoff authority through observation and reap. The supervisor treats both outputs as untrusted proposals, replays the accepted plan against its retained exact source, and requires byte-for-byte canonical agreement. Each caller-bounded non-retained read spawns a fresh restricted worker with no stage or destination capability. Its bootstrap preserves whether the accepted plan originated from inspect or materialize, and no bytes return before exact pipe EOF, correlated result, integrity, clean exit, and reap agree. Supervised materialization passes only a supervisor-created stage root, requires clean exit and exact reap before result validation, replay, or audit, and reserves no-replace publication for the supervisor. Public package smoke preserves IR on destination setup failure and covers effect mapping, retained borrows, bounded reads, exact helper packaging, cleanup, and reap. A required pinned-kernel QEMU gate proves public inspect and materialize fail closed on actual Landlock ABI 2 before source transfer.

The authenticated helper's semantic execution is now fixture-independent within the private record boundary. One self-bound generic adapter validates the sealed plan's actual profile, policy identity, budget, target, consumer, effect, member-sync, target identity, and retention, then executes inspect, materialize, or one-shot read work without reconstructing lab defaults. It preserves canonically stopped verification as semantic output. The supervisor reconstructs owned private outcome and retention state only after its exact-source replay matches completion and retained content byte for byte. This adapter still does not use or revise protocol v1.
