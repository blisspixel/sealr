...
The separate authenticated Linux worker boundary introduced in Alpha.6 and extended on current main does not change protocol v1. Normal cases authenticate a distinct child-only helper's exact bytes, sealed executable identity, private hello, helper feature generation, and running process before bootstrap; deliberate fault cases alone use the lab executable. Bounded kernel-sealed memfds carry canonical operation-bound planning, plan-bound completion, completion-bound retained-content, and distinct full-read or prefix-read request records. The private executor binds a source-owning Ready plan to the exact worker descriptor, verifies planned Store and Deflate ranges without structural reparse, and retains handoff authority through observation and reap. The supervisor treats both outputs as untrusted proposals, replays the accepted plan against its retained exact source, and requires byte-for-byte canonical agreement. Each caller-bounded non-retained read spawns a fresh restricted worker with no stage or destination capability. Its bootstrap preserves whether the accepted plan originated from inspect or materialize and carries an explicit prefix flag when required. The worker streams the complete member. The supervisor retains only the requested prefix for prefix reads, and no bytes return before exact pipe EOF, correlated result, complete size, CRC32, SHA-256, clean exit, and reap agree. Helper feature generation 2 makes the additive request kind incompatible with older helpers before source transfer while bootstrap ABI 1 and operation protocol v1 remain unchanged. Supervised materialization passes only a supervisor-created stage root, requires clean exit and exact reap before result validation, replay, or audit, and reserves no-replace publication for the supervisor. Public package smoke preserves IR on destination setup failure and covers effect mapping, retained borrows, full and prefix reads, exact helper packaging, cleanup, and reap. A required pinned-kernel QEMU gate proves public inspect and materialize fail closed on actual Landlock ABI 2 before source transfer.

The authenticated helper's semantic execution is now fixture-independent within the private record boundary. One self-bound generic adapter validates the sealed plan's actual profile, policy identity, budget, target, consumer, effect, member-sync, target identity, and retention, then executes inspect, materialize, or full or prefix read work without reconstructing lab defaults. It preserves canonically stopped verification as semantic output. The supervisor reconstructs owned private outcome and retention state only after its exact-source replay matches completion and retained content byte for byte. This adapter still does not use or revise protocol v1.
...
The same workflow contains a separate `semantic_records` job with its own manifest, corpus, dictionary, and artifact boundary. That Alpha.6 target reaches a hidden private planning and completion codec entry only when its nondefault fuzz feature is enabled. It does not revise protocol v1 and does not yet fuzz either private member-read request kind. Both kinds have deterministic hostile validation coverage. Clean exact-main on-demand and first scheduled-event evidence are recorded in the [assurance report](assurance.md#current-evidence); accumulated scheduled history remains pending.
