...
The first Alpha.6 slices have landed as a separate repository-only Linux authority and semantic lab. Normal cases authenticate a distinct child-only helper's exact bytes, sealed executable identity, private hello, and running process before bootstrap; deliberate fault cases alone use the lab executable. The lab tests descriptor transfer and identity, pre-exec and child-entry closure, `no_new_privs`, fixed Landlock ABI 3 ordering before source transfer, pidfd-backed termination and reap, and checked post-reap fixture cleanup without treating v1 as a runtime outcome format. Bounded kernel-sealed memfds carry canonical operation-bound planning, plan-bound completion, completion-bound retained-content, and one-shot member-read request records, proving transport seals, length, SHA-256, correlation, live descriptors, failures, and lifecycle without using protocol v1. The feature-gated private executor binds a source-owning Ready plan to the exact worker descriptor, verifies planned Store and Deflate ranges without structural reparse, captures supervisor-selected bytes during those verifier calls, and retains the source and handoff descriptors through supervisor observation and reap. The supervisor decodes both outputs first as untrusted proposals, replays the accepted plan against its retained exact source, and requires byte-for-byte canonical agreement. A separate caller-bounded read capability spawns a fresh restricted worker per call, passes no stage or destination capability, and returns no bytes until exact pipe EOF, correlated result, integrity, clean exit, and reap agree. The private materialize route passes only a supervisor-created stage root, requires clean exit and exact reap before result validation, replay, or audit, and reserves no-replace publication for the supervisor. This private route remains separate from protocol v1. Future end-to-end tests must preserve IR on destination setup failure, distinguish worker and supervisor facts, and cover public effect mapping, retained borrows, bounded reads, fixed release packaging, and real kernel setup failure before archive execution can shape public state.
