...
This is supported prerelease evaluation, not stable wheel installation. Benign `.data` and `.data/headers` shapes now appear in the 300-artifact inventory, and Core Metadata 2.1 through 2.6 has an explicit pinned policy. The packaged PyPA installer 1.0.1 conformance kit and copyable handoff exercise real target effects after source deletion. A stable API still requires the exact Poetry fixture, targeted benign Unicode and descriptor-bearing artifacts, long-running cross-platform evidence, broader public API review, independent adoption, and the remaining production-readiness gates in the roadmap.
