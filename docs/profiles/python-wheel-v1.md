...
The Alpha.7 laboratory pins PyPA `installer` 0.7.0 by version and wheel SHA-256 as historical evidence. Current required conformance pins exact `installer` 1.0.1. Rust stages bounded blobs only through `VerifiedArchive::read_member`, deletes the original fixture wheel before Python starts, and passes no source-wheel path or ZIP descriptor to the external process. The Python adapter receives only bounded verified-member and plan descriptors, installs an audit hook before importing installer, rejects every `.whl` open, validates `RECORD`, proves repeatable `WheelSource` iteration, and compares installer writes, relocations, shebang rewrites, generated wrappers, and final `RECORD` placement with the Rust plan. The resulting target outputs receive a separate realization identity.

The supported Linux supervisor now also represents bounded prefix reads. A distinct sealed request and worker frame flag bind script classification to the exact accepted source, plan, completion, member, and cap. The worker streams the complete member while the supervisor retains only the prefix and independently checks complete size, CRC32, and SHA-256 before release. The extracted-package consumer evaluates a supervised `.data/scripts` fixture, so wheel evaluation and the strongest Linux backend compose without fallback. The next repository sequence is a compact copyable public-API-only `WheelSource` handoff, an exact Poetry 2.4.2 fixture that consumes it before uninstall or destination effects, and only then independent external adoption.
