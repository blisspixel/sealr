...
The current [v5 stratified inventory](../wheel-compatibility-v5.md) expands bounded measurement to 300 exact artifacts: 280 admitted and 20 denied. It measures twelve admitted `scripts` schemes, one `headers` scheme, and three `data` schemes. It still contains no Unicode member paths, and descriptor-bearing benign artifacts are not reported as a distinct measured cohort. Those two gaps, plus a separate ratio-boundary study, are the next corpus targets so benign compatibility evidence cannot silently weaken the interpretation profile or default adversarial budget.
...
The Alpha.7 laboratory pins PyPA `installer` 0.7.0 by version and wheel SHA-256 as historical evidence. Current required conformance pins exact `installer` 1.0.1 and consumes the Python bridge from the packaged [copyable `WheelSource` handoff](../../crates/sealr/examples/pypa_installer_handoff/README.md). Rust stages bounded blobs only through `VerifiedArchive::read_member`, deletes the original fixture wheel before the post-admission installer bridge starts, and passes no source-wheel path or ZIP descriptor to the external process. The exact plan remains in Rust. Python receives only a reduced SHA-256-bound manifest and bounded verified blobs, while the canonical receipt and manifest digests arrive as separate command arguments. The adapter installs an audit hook before importing installer, rejects every `.whl` open, validates `RECORD`, and proves repeatable `WheelSource` iteration. Rust compares exact installed paths, relocations, final `RECORD` placement, and executable modes with the plan, then binds every observed hash and size, including rewritten scripts and generated wrappers, into the separate realization identity.

The supported Linux supervisor also represents bounded prefix reads. A distinct sealed request and worker frame flag bind script classification to the exact accepted source, plan, completion, member, and cap. The worker streams the complete member while the supervisor retains only the prefix and independently checks complete size, CRC32, and SHA-256 before release. The extracted-package consumer evaluates a supervised `.data/scripts` fixture, so wheel evaluation and the strongest Linux backend compose without fallback. The copyable handoff builds from Cargo's extracted `.crate` with no internal Sealr feature and runs both supervised inspect and materialize origins against the helper manifest and verifier extracted from the Linux native package. The exact Poetry 2.4.2 fixture now consumes that handoff after Poetry's lock validation, emits PREPARED before real update uninstall or target writes, denies post-PREPARED wheel opens in the host adapter and installer bridge, and requires stock-output plus realization-identity parity. Independent external adoption is next.
...
Alpha.8 met these gates through the portable profile vector, exhaustive flag and extra-field checks, the public capability-only evaluator, downstream source-deletion regression, hostile fixtures inherited from the Alpha.7 laboratory, distinct identity domains, and the predecessor-bound v3 inventory. Alpha.8 release promotion required the final surface to pass Required CI on Linux, macOS, and Windows.

This is supported prerelease evaluation, not stable wheel installation. Benign `.data` and `.data/headers` shapes now appear in the 300-artifact inventory, and Core Metadata 2.1 through 2.6 has an explicit pinned policy. The packaged PyPA installer 1.0.1 conformance kit, copyable handoff, and exact Poetry 2.4.2 fixture exercise real target effects after source deletion. A stable API still requires targeted benign Unicode and descriptor-bearing artifacts, long-running cross-platform evidence, broader public API review, independent adoption, and the remaining production-readiness gates in the roadmap.
