...
Status: implemented as dormant crate-private Alpha.6 code and exercised by the repository-only Linux worker lab. No production `apply`, CLI, receipt, default-feature, supported API, public worker mode, or release path invokes it. Outside crate tests, only the feature-gated repository lab and the unsupported hidden driver enabled explicitly by the fuzz workspace reach it.

This experiment makes the split-phase handoff in the [semantic-ownership decision](decisions/0001-alpha6-semantic-ownership.md) executable without defining operation protocol v2 or changing public behavior. It establishes bounded representations and hostile decoders for semantic proposals, one-shot read authority, and a private materialization request. The Linux repository lab supplies process-isolated evidence for its exact inspect, read, and writer lifecycle cases, but this record module does not establish a public verified capability or production containment claim.
...
The worker completion remains an untrusted proposal. The supervisor receives its sealed descriptors while the child is alive but does not decode or validate them. It sends the exit acknowledgement, requires a successful exit and exact pidfd-backed reap, then replays the accepted plan against the retained source into a sink and requires byte-for-byte canonical completion equality. Only a complete replay yields an opaque authorized stage manifest. The supervisor audits the retained root for private root security, stable stage-name identity, exact expected paths and kinds, no reparse points, single-link regular files on Linux, exact sizes, and streamed SHA-256. An audited stage alone can reach the supervisor's existing no-replace publication primitive.

If termination and reap cannot be proved, the active writer wrapper deliberately abandons the stage owner instead of running recursive cleanup through `Drop`. Once reap is proved, failure may abort the stage. Targeted Linux evidence covers mutation after reap, destination appearance after audit, cleanup failure, crash after writes, crash after completion sealing, crash after result observation, crash after exit acknowledgement, and stalls before execution and during exit. A separate 500-iteration campaign alternates publication and hostile writer cases with child and descriptor baseline checks after every iteration. This remains repository-only evidence and does not map worker errors into public effect axes, construct `VerifiedArchive`, or activate a runtime path.
...
The first bounded matrix expansion, shared owning planning seam, and one-shot read backend are complete. Continued parity growth beyond these named fixtures remains assurance work, not an activation blocker by itself. Runtime activation still requires:

1. real-kernel setup-failure evidence beyond the deterministic authority-gate injections;
2. public outcome and `VerifiedArchive` integration, including retention parity for materialization;
3. an authenticated child-only helper packaging and discovery model that works for both library and CLI consumers;
4. accumulated clean scheduled fuzz history, with every reproducible failure promoted to a deterministic regression.

The isolated bridge has landed as Linux-only, repository-only inspect, read, and materialize evidence. The supervisor retains the expected invocation, accepted plan, retention request, deadlines, lifecycle, source descriptor, and all public semantic merging. After restriction readiness, the worker receives the read-only snapshot and sealed canonical plan. Record validation binds the plan to the exact file-backed snapshot and may read represented structural ranges without invoking the ZIP parser; after plan acceptance, the executor reads only recorded Store or Deflate payload ranges. Both fixture members are supervisor-selected for retention and captured as output sinks of their original verifier calls. The worker returns separately sealed canonical completion and retained-content bundles while retaining its source and handoff descriptors through supervisor observation, then exits cleanly and is reaped. The supervisor checks blob seals and length, independently recomputes each digest from the exact kernel-sealed bytes, and decodes both records against its retained plan first as untrusted proposals. Correlation, seals, canonical decoding, and successful worker execution do not themselves prove the proposed file-content SHA-256 values. The supervisor therefore replays the accepted plan against its retained exact source descriptor after reap and requires both canonical outputs to match the proposals byte for byte. A regression supplies a fully canonical completion with forged file SHA-256: proposal validation accepts it, while the source-derived authority step rejects it. Retention regressions reject every truncation, content mutation, and a request above the transfer ceiling. This replay is independent of worker output but deliberately uses the same bounded verifier implementation. One-shot non-retained reads then bind fresh process authority to the exact accepted completion and use only one write-only output pipe. For materialization, the supervisor alone creates the stage and retains the destination parent and final name; the worker receives only the stage root, exact source, and sealed plan. After clean exit and reap, the supervisor requires source-derived completion equality, audits every staged object, and alone performs no-replace publication. The lab does not translate any output into public interpretation, admission, verification, `ArchiveIR`, or `VerifiedArchive` state. It proves isolated plan consumption, content authority, original-pass retained-content transfer, bounded non-retained reads, writer quiescence, stage audit, and publication ownership for its fixtures. It does not activate a public worker, construct `VerifiedArchive`, claim structural-parser confinement, close helper packaging or receipt integration, or provide macOS or Windows isolation.
