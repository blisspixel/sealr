...
Focused evidence covers a private-file Ready value after removal of the caller path, a direct one-over archive-cap ingest failure before parsing, exact plan-owned context, zero payload reads during planning, and plan-native completion without a structural parse. The fuzz context also consumes actual pending IR from this seam; it no longer clones completed IR and resets verification fields to synthesize planning state. This is an in-process ownership and drift-control result, not parser confinement, sealed transport, content authority, or a resource-envelope measurement.
...
6. a helper packaging and discovery model that works for both library and CLI consumers;
7. the first scheduled-event campaign and accumulated clean scheduled history, with every reproducible failure promoted to a deterministic regression.
