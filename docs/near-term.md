...
1. **Close dormant semantic assurance.** Correct verification-time source I/O to `Indeterminate + Admitted + Partial` before a zero-difference matrix can canonize the shared outcome bug. Make planning and completion validation allocation-fallible, eliminate redundant full-IR cloning during completion decode, and add a near-limit memory regression. Then add a manifest-pinned shadow matrix spanning ready, terminal, complete, stopped, setup-failure, Store, Deflate, descriptor, quota, and codec outcomes. Compare exact IR, ordered findings, axes, verified frontier, and request and plan correlation. Preserve any fuzz reproducer as a deterministic regression. **Campaign evidence landed:** the [first exact-main on-demand Linux AddressSanitizer run](https://github.com/blisspixel/sealr/actions/runs/32634689922) completed 194,476 units in 601 seconds at 512 MiB peak RSS without a crash or reproducer. Correct source-I/O axes, typed allocation failure, zero differences across the broader pinned shadow matrix, and scheduled-event history remain open.
