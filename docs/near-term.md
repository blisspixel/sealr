...
> Status: active plan after the Alpha.11 restricted raw POSIX PAX increment, the restricted GNU long-name profile, the gzip-wrapped PAX and GNU compositions, and the first three promoted codecs (zstd-, xz-, and bzip2-wrapped ustar) on current main. This page turns the long-range [roadmap](../ROADMAP.md) into release-sized work and records completed gates where they constrain the next increment.
...
3. The restricted language admits exactly one stream of one to 4096 LZMA2-only blocks with an 8 MiB pre-allocation dictionary ceiling, checks limited to CRC32/CRC64/SHA-256 and verified twice with `None` denied, declared block sizes both-or-neither and verified, reserved bits zero, and stream padding, concatenation, and trailing data denied with exact full-source consumption.
4. Sealr's independent footer-and-index-first container parse is cross-checked against the decoder's streaming interpretation — including the index tiling, backward size, and reserved bits the upstream decoder does not enforce — and the ready-boundary composite audit re-verifies every block check over the derived snapshot with Sealr's own CRC32, CRC64, and SHA-256 before any destination stage exists.
...
3. The restricted language admits exactly one stream of one to 65,536 blocks at levels 1-9 with the bzip1 container, randomized blocks, empty streams, nonzero footer padding, concatenation, and trailing data denied, and exact full-source consumption.
4. The bit-aligned container forced a new dual-parse design, validated against real producer output before freezing: Sealr independently recovers the footer by a unique-match eight-shift scan, scans the full stream for every 48-bit block magic, extracts every stored block CRC, and requires the chain fold to reproduce the footer's combined CRC exactly; single-block streams are additionally re-hashed end to end with Sealr's own bzip2-variant CRC32 at the ready boundary; decoder memory is format-capped at ~3.7 MB by the level digit before decoding.
...
Build the local bounded 7z interpreter with Copy first under [Gate C](codec-dependency-gates.md#gate-c-complex-container-engine), then reuse the reviewed LZMA layer. Copy-first separates container correctness from decoder complexity and keeps packed headers, coder graphs, solid routing, encryption, and volume behavior explicitly gated.
...
| Current main | GNU long-name TAR, gzip compositions, and the zstd, xz, and bzip2 codecs | Separate GNU `L`-only raw language under policy v6 with `sealrTreeV6`; distinct gzip compositions under policy v7 with `sealrTreeV7` and `sealrTreeV8`; and three executed Gate B codec promotions — zstd under policy v8 with `sealrTreeV9`, xz under policy v9 with `sealrTreeV10`, and bzip2 under policy v10 with `sealrTreeV11` |
| Next | 7z structure | A local bounded Copy-first interpreter through Gate C before the reviewed LZMA layer |
