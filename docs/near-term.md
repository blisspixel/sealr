...
Together, these increments freeze what the worker may mean, how read output is withheld on failure, when staged effects become stable enough for supervisor audit and publication, and which exact helper artifact may cross the release boundary. The real insufficient-kernel proof and exact packaged-consumer activation are now required CI. The next increment closes the accumulated assurance ledger and cuts the Alpha.6 release candidate.
