...
1. **Downstream usefulness.** The wheel consumer evaluates, plans, and realizes installs from the `VerifiedArchive` capability alone and never reopens the archive. The runnable `wheel_admission` and `same_digest_different_tree` examples, byte-exact canonical evidence, independent verifier, and verified statement builder close the repository proof. The exit condition is now one external publisher, registry, build backend, or installer that binds this capability and evidence as its authoritative decision.
