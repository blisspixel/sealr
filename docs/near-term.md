...
Authority closure and the remaining semantic-assurance work can proceed in parallel. The bounded shadow-parity prerequisite, shared owning planner, plan-native inspect executor, sealed transport, measured isolated plan consumption, supervisor-owned exact-byte replay, immutable original-pass retained-content transfer, and one-shot isolated non-retained reads are now clean. The next bridge must execute materializing Store and Deflate work under the restricted boundary, prove that the writer and every possible descendant are gone before audit, validate the exact quiescent stage, and leave publication solely to the supervisor. This is next because a correct read boundary cannot authorize effects, and auditing while writable authority survives would make the staged-tree claim unstable.
...
Together, these increments freeze what the worker may mean, what authority it may hold for inspect and later reads, and how a failed one-shot read withholds output. The next increment must close the distinct writable-stage lifecycle before any public activation.
...
**Current status: shared owning planner, dormant records, hostile decoders, source-binding hardening, input-sized allocation fallibility, single-materialization completion reconstruction, a required near-limit peak-live probe, an immutable 12-case v1 shadow baseline plus 12 additive v2 cases, a source-owning inspect executor, a sealed isolated Linux bridge, supervisor-owned exact-byte replay, immutable original-pass retained-content transfer, one-shot isolated non-retained reads, and a dedicated fuzz surface with clean exact-main evidence landed.** The v2 additions declare apply, backend, or supervisor-reproduction ownership for each observation and cover mixed strict-v2, same-byte profile behavior, exact memory/private-file frames, topology, covering, and quota boundaries. The repository bridge validates the full record against the exact worker source and executes planned Store and Deflate ranges without structural reparse. Selected bytes are captured in that pass and transferred in a canonical operation-, plan-, and completion-bound sealed bundle. After reap, the supervisor re-executes the accepted plan against its retained exact source and requires completion and retention bundles to match byte for byte. A separate read request binds each later call to that exact authority and releases no bytes before complete validation and clean reap. This establishes bounded isolated plan consumption, source-derived content authority, retained transfer, and isolated later-read lifecycle evidence plus a reproducible search surface, not broad profile or policy parity, complete `VerifiedArchive`, materializing-writer lifecycle, helper packaging, or runtime-worker parity.
