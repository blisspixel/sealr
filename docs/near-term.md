...
Authority closure and the remaining semantic-assurance work can proceed in parallel. The bounded shadow-parity prerequisite, shared owning planner, plan-native inspect executor, sealed transport, measured isolated plan consumption, supervisor-owned exact-byte replay, immutable original-pass retained-content transfer, one-shot isolated non-retained reads, and the private materializing-writer lifecycle are now clean. The writer receives only the exact source, sealed materialization plan, and supervisor-created stage root after restriction readiness. The supervisor accepts no completion authority until clean exit and exact reap, then replays the retained source, audits the exact stage, and alone performs no-replace publication. The next bridge must integrate this lifecycle with the public capability contract without changing semantic axes, later-read behavior, or platform fallbacks, then settle helper packaging and real-kernel setup-failure evidence. This is next because private conformance does not make a shipped consumer use the boundary.
...
4. **Prove isolated non-retained read behavior.** **Landed in the repository-only Linux lab:** a canonical member-read request binds a fresh read operation to the accepted operation, request, plan, exact completion digest, source-order member index, canonical path, and caller limit. The supervisor validates the request and reserves the exact verified size before spawn. Each call creates one fresh restricted worker with the read-only source and sealed plan, completion, and request records, but no stage or destination capability. The worker writes only the authorized planned Store or Deflate range through a backpressured write-only pipe. The supervisor releases no bytes until exact EOF, correlated result, size, CRC32, SHA-256, clean exit, and reap agree. Capability clones share immutable authority and a one-slot coordinator. Evidence covers exact and one-under limits without spawn, pre-spawn and queued cancellation without spawn, active cancellation at a deterministic probe stall, a post-result crash with no returned bytes, next-call recovery, 64 alternating one-shot reads with no surviving child, and last-owner descriptor restoration. It does not construct public `VerifiedArchive`, prove every cancellation epoch, or activate a runtime worker.

5. **Prove the private materializing-writer lifecycle.** **Landed in the repository-only Linux lab:** the supervisor creates the production stage before spawn and retains the destination parent, random stage name, final name, cleanup, and publication authority. The restricted worker receives only a read-only descriptor for that stage root, the exact source, and a sealed materialization plan. A separate plan-native executor shares component-handle creation, Store and Deflate verification, flush, and member-sync behavior with public in-process materialization. The supervisor observes result descriptors while the child is alive but validates them only after clean exit and exact pidfd-backed reap. It replays the plan against its retained source, requires exact canonical completion equality, audits root security, exact paths and kinds, single-link files, sizes, and SHA-256, and alone invokes no-replace publication. The measured seccomp set denies descendants, permission mutation, rename, link, unlink, symlink, device creation, mount, truncate, and new socket paths. Targeted evidence covers post-reap stage mutation, destination races, cleanup failure, four writer crash barriers, and two writer stalls. A separate 500-iteration campaign alternates publication and hostile writer cases while requiring no child or descriptor leak after every iteration. This does not activate public `apply`, CLI, `VerifiedArchive`, protocol v1, or release packaging.

Together, these increments freeze what the worker may mean, how read output is withheld on failure, and when staged effects become stable enough for supervisor audit and publication. The next increment must merge the private lifecycle into the public capability boundary and package the helper before activation.
...
The next semantic sequence is dependency-ordered. The additive v2 matrix, shared owning plan seam, x86_64 no-descendant and permission-mutation controls, raw ancillary gate, authority-epoch absolute deadlines, 500-iteration bootstrap stress, sealed handoff mechanics, Linux-only semantic inspect bridge, supervisor-owned exact-byte replay, immutable original-pass retained-content transfer, and one-shot isolated non-retained reads are complete without changing the frozen v1 protocol bytes or public behavior. The worker gets no destination, receipt, or publication authority, and no proposal shapes public state. Next close the materializing-writer lifecycle by proving quiescence, auditing the exact stage after reap, and keeping no-replace publication supervisor-owned. Public activation still waits for that effect boundary and helper packaging.
