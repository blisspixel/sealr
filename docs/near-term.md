...
1. **Close dormant semantic assurance.** Make planning and completion validation allocation-fallible, eliminate redundant full-IR cloning during completion decode, and add a near-limit memory regression. Then add a manifest-pinned shadow matrix spanning ready, terminal, complete, stopped, setup-failure, Store, Deflate, descriptor, quota, and codec outcomes. Compare exact IR, ordered findings, axes, verified frontier, and request and plan correlation. Preserve any fuzz reproducer as a deterministic regression. **Source-I/O axes fix landed:** verification-time source I/O now produces `Indeterminate + Admitted + Partial`, with direct outcome and stopped-record regressions preventing a zero-difference matrix from canonizing the prior shared bug. **Campaign evidence landed:** the [first exact-main on-demand Linux AddressSanitizer run](https://github.com/blisspixel/sealr/actions/runs/32634689922) completed 194,476 units in 601 seconds at 512 MiB peak RSS without a crash or reproducer. Typed allocation failure, zero differences across the broader pinned shadow matrix, and scheduled-event history remain open.
...
**Current status: dormant record, hostile decoder, source-binding hardening, dedicated fuzz surface, and one clean exact-main on-demand campaign landed.** This establishes parity for the named deterministic record, source-binding, and codec cases plus a reproducible search boundary, not scheduled stability, broader shadow parity, complete `VerifiedArchive`, or runtime-worker parity.
...
6. Pin record vectors for ready planning, terminal admission, terminal covering evidence, partial completion, complete completion, malformed structure, and record mutation. **Expanded deterministic slice landed:** canonical plan and completion digests, every truncation, trailing bytes, cross-kind decode, structured binding mutation, stale correlation, impossible frontiers, source-order preservation, hostile finding labels, phase allowlists, range overflow, parser-oracle metadata-cap boundaries, represented fixed-field and descriptor source binding, 257-component and normalization parity, expected-binding depth rejection before allocation, pre-growth encoded-length limits, and setup-failure merge are covered. A separate semantic-record libFuzzer target now exercises arbitrary bytes, input-directed mutations of runtime-generated canonical frames, decode stability, exact Ready-plan IR parity with production, correlation, and every valid record kind. Four committed driver-input seeds plus a dictionary are digest-pinned. Required verification uses parsed Cargo metadata to bind each target and hidden driver, then binds the complete scheduled workflow, including its weekly trigger and both jobs. Negative fixtures reject inert TOML remapping, manual-only drift, weakening, inactive or appended commands, inert artifact evidence, and raw or quoted duplicate last-wins resource arguments. The first exact-main on-demand Linux AddressSanitizer campaign is clean; the first scheduled-event run, accumulated history, and broader shadow parity remain open. Worker crash belongs to supervisor lifecycle evidence, alongside verification and effect failure, writer quiescence, stage audit, cleanup, publication, clone and drop behavior, retained borrows, and bounded non-retained reads.
