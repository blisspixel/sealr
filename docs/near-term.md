...
Remaining bootstrap closure is broader repeated fault stress plus real kernel setup-failure evidence that deterministic injections cannot supply. Current crash barriers prove unexpected self-exit handling, and the nine epoch stalls prove supervisor-initiated termination across every current authority round and changing stage state. The measured seccomp deny set closes no-descendant and stage-permission-mutation authority in the x86_64 lab, but it is not a complete syscall allowlist or a network-containment claim.
