...
Authority closure and the remaining semantic-assurance work can proceed in parallel. The bounded shadow-parity prerequisite, the shared owning planner, and the first plan-native inspect executor are now clean. The next bridge must preserve those boundaries while moving validated-plan consumption into measured isolated execution.
...
**Current status: shared owning planner, dormant record, hostile decoder, source-binding hardening, input-sized allocation fallibility, single-materialization completion reconstruction, a required near-limit peak-live probe, an immutable 12-case v1 shadow baseline plus 12 additive v2 cases, a source-owning inspect executor, and a dedicated fuzz surface with clean exact-main on-demand evidence landed.** The v2 additions declare apply, backend, or supervisor-reproduction ownership for each observation and cover mixed strict-v2, same-byte profile behavior, exact memory/private-file frames, topology, covering, and quota boundaries. This establishes a bounded in-process planning and execution boundary plus a reproducible search surface, not scheduled stability, broad profile or policy parity, complete `VerifiedArchive`, transport safety, isolation, or runtime-worker parity.
...
The next semantic sequence is dependency-ordered. The additive v2 matrix, shared owning plan seam, x86_64 no-descendant and permission-mutation controls, raw ancillary gate, authority-epoch absolute deadlines, 500-iteration bootstrap stress, and sealed synthetic handoff mechanics are complete without changing the frozen v1 bytes or public behavior. Next replace the synthetic payload with the full expected binding plus real semantic plan and completion across the Linux-only inspect bridge. The worker gets no destination, stage, retention, later-read, receipt, or publication authority, and the supervisor treats every completion as a proposal rather than verified content. Public activation still waits for separate content-authority, lifecycle, later-read, and helper-packaging gates.
...
2. Run at least 500 bounded Linux worker race and fault iterations. **Bootstrap fault campaign landed:** required Linux conformance cycles a closed 44-case matrix 500 times, with every case executed at least 11 times and per-iteration child, descriptor, retained-authority, and cleanup checks. Worker archive-execution races and separate repeated in-process materializer stress on native Linux, macOS, and Windows remain later gates.
