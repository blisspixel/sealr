...
```text
Primary       semantic contract [done] -> private snapshot [done] -> supervised Linux worker [release gates]
...
Authority closure and the remaining semantic-assurance work can proceed in parallel. The bounded shadow-parity prerequisite, shared owning planner, plan-native inspect and materialize executors, sealed transport, measured isolated plan consumption, supervisor-owned exact-byte replay, immutable retained-content transfer, one-shot isolated non-retained reads, reaped materializing-writer lifecycle, authenticated child-only normal execution, fixed release packaging, public fail-closed supervisor, and hidden worker-backed `VerifiedArchive` backend are now implemented. The next bridge activates that same explicit boundary in the CLI, wheel laboratory, and extracted-package consumer without changing semantic axes, later-read behavior, or platform fallbacks. Real-kernel setup-failure evidence remains a release gate because deterministic injections cannot prove behavior on an actually insufficient Landlock kernel.
...
**Current status: authority, authenticated-helper, lifecycle, and fixed-package conformance slices landed.** This remains a non-published tool, not a runtime path in `sealr` or `sealr-cli`; the next native release candidate is nevertheless required to carry and verify the helper artifact on Linux.
...
7. Keep the library, CLI, wheel laboratory, and packaged consumer on one semantic boundary. A CLI-only reduced manifest is not sufficient. Retained-content, later-read, lifecycle, packaging, and public library gates are closed; CLI, wheel-laboratory, and packaged-consumer activation remain before Alpha.6 release. Protocol v2 remains unnecessary for this private record boundary.

The semantic sequence remains dependency-ordered. The additive v2 matrix, shared owning plan seam, x86_64 no-descendant and permission-mutation controls, raw ancillary gate, authority-epoch absolute deadlines, 500-iteration bootstrap stress, sealed handoff mechanics, source replay, immutable retention, one-shot reads, the reaped materializing-writer lifecycle, authenticated child-only execution, the self-bound adapter, the [fixed helper package](helper-packaging.md), and public supervised inspect and materialize are complete without changing the frozen v1 protocol bytes. CLI, wheel-laboratory, and packaged-consumer activation follow on that same boundary. Real-kernel setup-failure evidence remains a release gate.
