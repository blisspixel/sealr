...
**Current status: first dormant record and hostile-decoder slice landed.** It establishes covered semantic-evidence parity, not complete `VerifiedArchive` or runtime-worker parity.
...
6. Pin record vectors for ready planning, terminal admission, terminal covering evidence, partial completion, complete completion, malformed structure, record mutation, and worker crash. **Initial deterministic slice landed:** canonical plan and completion digests, every truncation, trailing bytes, cross-kind decode, structured binding mutation, stale correlation, impossible frontiers, source-order preservation, hostile finding labels, range overflow, parser-oracle metadata-cap boundaries with omitted records and correlated geometry fabrication rejected, source-backed covering and header-field success and failure, and setup-failure merge are covered. Dedicated fuzzing, broader shadow parity, real worker crash transfer, and supervisor lifecycle vectors remain open. Pin separate supervisor-merge vectors for verification and effect failure, writer quiescence, stage audit, cleanup, publication, clone and drop behavior, retained borrows, and bounded non-retained reads.
