...
The next semantic sequence is dependency-ordered. The additive v2 matrix, shared owning plan seam, x86_64 no-descendant and permission-mutation controls, and raw ancillary gate are complete without changing the frozen v1 bytes or public behavior. The bootstrap must now close authority-epoch absolute deadlines and repeated stress. Only then carry the full expected binding plus sealed plan and completion blobs across a Linux-only inspect bridge. The worker gets no destination, stage, retention, later-read, receipt, or publication authority, and the supervisor treats every completion as a proposal rather than verified content. Public activation still waits for separate content-authority, lifecycle, later-read, and helper-packaging gates.
