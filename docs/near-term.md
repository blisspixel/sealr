...
1. **Close dormant semantic assurance.** Add a manifest-pinned shadow matrix spanning ready, terminal, complete, stopped, setup-failure, Store, Deflate, descriptor, quota, and codec outcomes. Compare exact IR, ordered findings, axes, verified frontier, and request and plan correlation. Run the existing bounded Linux AddressSanitizer job and preserve any reproducer as a deterministic regression. Exit evidence is zero differences across the pinned matrix and one clean exact-commit 600-second campaign.
...
**Current status: dormant record, hostile decoder, source-binding hardening, and dedicated fuzz surface landed.** This establishes parity for the named deterministic record, source-binding, and codec cases plus a reproducible search boundary, not broader shadow parity, complete `VerifiedArchive`, runtime-worker parity, or a completed semantic-record fuzz campaign.
