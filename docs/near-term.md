...
1. **Close dormant semantic assurance.** **Allocation, ownership, and bounded shadow-parity hardening landed:** input-sized path, topology, covering, extra-field, planning-canonicality, and completion-reconstruction work now uses fallible or allocation-free validation; direct completion encoding materializes no IR, invalid decode materializes none, valid decode materializes exactly one fallibly reconstructed IR, and findings move instead of cloning. A deterministic failpoint walk covers every completion-reconstruction reservation without mutating the accepted plan. A record above 64 KiB proves an exact logical reconstruction budget, one-byte-under typed failure, and one materialization at the exact budget. Verification-time source I/O now produces `Indeterminate + Admitted + Partial`. A versioned 12-case manifest now pins the profile, policy digest, requested effect, retention state, exact pending and final IR, ordered record-owned findings, semantic axes, verified frontier, source, request and plan identities, and canonical frame hashes for Store, Deflate, descriptor, terminal structure and admission, CRC, declared-size, codec, source-I/O, and setup-failure outcomes. The pending IR comes from the real post-covering, pre-setup and pre-verification boundary. Clean exact-main on-demand campaign evidence is in the [assurance ledger](assurance.md#current-evidence). An isolated allocator or process peak-live measurement, parity expansion across profiles and source backends, the first scheduled-event run, and accumulated scheduled history remain open.
