...
Authority closure and the remaining semantic-assurance work can proceed in parallel. The bounded shadow-parity prerequisite and the first plan-native inspect executor are now clean. The next bridge must preserve that executor boundary while moving it into measured isolated execution.

1. **Close dormant semantic assurance.** **Allocation, ownership, bounded shadow-parity, and near-limit completion measurement landed:** input-sized validation is fallible or allocation-free; invalid completion decode materializes no IR; accepted decode materializes one fallibly reconstructed IR; findings move instead of cloning; and a deterministic failpoint walk covers every reconstruction reservation. A required isolated-child probe pins a 67,041,104-byte plan and enforces that completion reconstruction adds less than 1 MiB transient requested heap above its one logical output. A versioned 12-case manifest pins exact pending and final IR, axes, findings, frontiers, and frame identities at the real planning boundary. Clean exact-main on-demand fuzz evidence is in the [assurance ledger](assurance.md#current-evidence). Parity expansion across profiles and source backends, the first scheduled-event run, and accumulated scheduled history remain open.
...
3. **Extract a crate-private inspect-only validated-plan executor.** **Landed:** the non-cloneable executor consumes each accepted Ready inspect plan value bound to its exact snapshot, rejects materialization and every present retention plan before payload reads, and reuses one shared Store and Deflate verifier across `apply`, later verified-member reads, and plan execution. Instrumentation proves exactly one structural parse in planning and no additional parse during execution, structural-range poison remains unread, and every execution read stays within a planned payload range, including ignored-extra geometry. Complete and stopped parity covers Store, Deflate, CRC with a trailing Pending member, both declared-size-lie boundaries, codec failures, and injected source I/O before and after a verified prefix. Member, ratio, and total-budget cases pass at the exact cap and remain planning-terminal one byte or one ratio unit below it; they cannot be forged as execution failures. Focused strict-v2 and private-file cases supplement the unchanged 12-case manifest. Destination setup still precedes independently bad payload verification. Destination effects, publication, retained-content transfer, later reads, worker transport, and public activation remain out of scope.
...
**Current status: dormant record, hostile decoder, source-binding hardening, input-sized allocation fallibility, single-materialization completion reconstruction, a required near-limit peak-live probe, a versioned 12-case in-process shadow matrix, a source-owning inspect executor, and a dedicated fuzz surface with clean exact-main on-demand evidence landed.** Executor completion frames have exact parity for the manifest-pinned strict-ASCII-v1 borrowed-source projection, with focused strict-v2, private-file, and additional stopped-frontier regressions. This establishes a bounded in-process execution boundary and reproducible search surface, not scheduled stability, broad profile or policy parity, complete `VerifiedArchive`, transport safety, isolation, or runtime-worker parity.
...
The next semantic sequence is dependency-ordered. First add a new pinned v2 matrix with mixed strict-v2, a byte-identical memory/private-file parity twin, ignored-extra, supervisor-reproduced IR-bearing covering evidence, dot-dot, interleaved topology, total-quota, and ratio-quota cases while preserving the v1 manifest bytes and digest. In parallel, finish the bootstrap's pre-parser controls. Then extract one real crate-private plan-only seam shared by in-process `apply` and a repository lab. Only then carry the full expected binding plus sealed plan and completion blobs across a Linux-only inspect bridge. The worker gets no destination, stage, retention, later-read, receipt, or publication authority, and the supervisor treats every completion as a proposal rather than verified content. Public activation still waits for the separate content-authority, lifecycle, later-read, and helper-packaging gates.
