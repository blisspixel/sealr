...
> Status: active plan after the Alpha.11 restricted raw POSIX PAX increment, the restricted GNU long-name profile, the gzip-wrapped PAX and GNU compositions, the first three promoted codecs (zstd-, xz-, and bzip2-wrapped ustar), and the first Gate C container step (Copy-only 7z) on current main. This page turns the long-range [roadmap](../ROADMAP.md) into release-sized work and records completed gates where they constrain the next increment.
...
3. Stock producers compress the header itself; `kEncodedHeader` is a named Unsupported shape whose finding carries the producer remedies (`-mhc=off`, `set_encoded_header_mode(False)`), and packed-header admission is deferred to the LZMA member review because it introduces a metadata-decode architectural decision.
4. Every container CRC32 — start header, next header, pack, folder, and substream digests — is the plain zlib CRC32 and is verified by Sealr itself; member payload CRC lies are attributed precisely at member verification; the empty-stream/empty-file matrix is authoritative for file-versus-directory meaning and any attribute disagreement is malformed rather than silently resolved.
