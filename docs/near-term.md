...
2. **Close the pre-parser Linux authority gate.** **No-descendant and permission-mutation controls landed:** the x86_64 bootstrap now installs an architecture-checked `TSYNC` seccomp-BPF deny set after Landlock and before source transfer, directly probes representative denial, and exposes filter readiness for supervisor observation. Next add per-epoch stall deadlines, raw unknown-ancillary rejection, and repeated hostile fault stress. Remaining exit evidence is bounded termination and reap for every stalled epoch plus 500 iterations with no surviving descendant, outside write, leaked descriptor, destination replacement, or cleanup before reap.
...
The next semantic sequence is dependency-ordered. The additive v2 matrix, shared owning plan seam, and x86_64 no-descendant and permission-mutation controls are complete without changing the frozen v1 bytes or public behavior. The bootstrap must now close raw ancillary parsing, authority-epoch deadlines, and repeated stress. Only then carry the full expected binding plus sealed plan and completion blobs across a Linux-only inspect bridge. The worker gets no destination, stage, retention, later-read, receipt, or publication authority, and the supervisor treats every completion as a proposal rather than verified content. Public activation still waits for separate content-authority, lifecycle, later-read, and helper-packaging gates.
