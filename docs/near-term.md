...
2. Run at least 500 bounded Linux worker race and fault iterations. **Bootstrap fault campaign landed:** required Linux conformance cycles a closed 36-case matrix 500 times, with every case executed at least 13 times and per-iteration child, descriptor, retained-authority, and cleanup checks. Worker archive-execution races and separate repeated in-process materializer stress on native Linux, macOS, and Windows remain later gates.
