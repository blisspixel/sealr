...
Together, these increments freeze what the worker may mean, how read output is withheld on failure, when staged effects become stable enough for supervisor audit and publication, and which exact helper artifact may cross the release boundary. The real insufficient-kernel proof is now required CI. The next increment activates the explicit supervisor in the CLI and packaged consumers.
