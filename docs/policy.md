...
| `max_derived_archive_bytes` | Policy v4 through v9 only. Bounds the immutable decoded TAR while a gzip, zstd, or xz wrapper is consumed. It is separate from the original source, member, and total extracted-byte caps. Policies v5 through v9 preserve the field even when a raw dialect is selected so compiled controls do not drift. |
| `max_files` | Checked against both format-declared counts where present and the number of members actually parsed. A false low ZIP EOCD count cannot authorize member-vector growth beyond the cap. Policy v2 through v9 cap the configured value at `u32::MAX` because their multi-format identity encodings bind the count as `u32`. Policy v1 preserves the Alpha.8 constructor language; ZIP32 itself bounds actual member counts below that encoding limit. |
...
| `max_metadata_bytes` | Bounds format-specific structural metadata. For ZIP this includes the central directory, EOCD comment, and referenced local name and extra bytes. For raw ustar it bounds admitted headers before trailing zero record padding is scanned. For every gzip-, zstd-, or xz-wrapped selection it cumulatively bounds the wrapper's structural bytes (headers, trailers, and for xz the block headers, padding, checks, index, and footer) plus the inner dialect's structural metadata. For raw PAX it additionally includes every extension payload and its zero padding; for raw GNU long-name it includes every carrier header and payload. |
...
- `max_dict_bytes`: reserved for future dictionary and window policy. The shipped zstd profile uses a fixed 8 MiB window ceiling and denies dictionaries outright; the shipped xz profile uses a fixed 8 MiB LZMA2 dictionary ceiling.
