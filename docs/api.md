...
This page distinguishes the implemented preview contract from the target semantic API. Alpha.11 is the latest published release; current main additionally implements the restricted raw GNU long-name profile under policy v6, the gzip-wrapped restricted PAX and GNU long-name compositions under policy v7, the zstd-wrapped portable ustar profile under policy v8, the xz-wrapped portable ustar profile under policy v9, the bzip2-wrapped portable ustar profile under policy v10, and the Copy-only 7z container under policy v11. The implemented surface includes explicitly selected portable ustar, strict ZIP64 under policy v3, strict gzip-wrapped portable ustar under policy v4, and restricted raw POSIX PAX under policy v5, all without changing the compatibility `apply()` behavior or ZIP32 default. Current callers must pin to the implemented section. The target is specified further in [semantic-model.md](semantic-model.md).
