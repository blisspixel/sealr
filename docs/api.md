...
pub struct Request<'a> {
    ...
    pub policy: &'a Policy,          // sealr.policy.v1 through sealr.policy.v11
...
`TarBzip2InterpretationProfile::UstarPortableV1` selects `sealr.profile.tar-bzip2.ustar-portable.v1`, whose canonical profile digest is `f6711c0c98cff6e3a2c6b266d159413ef891c202b4898b4e1665081dce0f29ee`. Its IR uses `sealr.archive-ir.tar-bzip2-ustar.v1` and `ArchiveFormat::TarBzip2Ustar`, serialized as `tar-bzip2-ustar`. Wrapper evidence records the level, header range, total payload and padding bits, every scan-verified block bit offset and CRC, the combined CRC, and derived length and SHA-256 — bit offsets rather than byte ranges, because the container is bit-aligned; member payload ranges refer only to the derived domain. Sealr's independent bit-level container replay — footer shift-scan, block-magic scan, and combined-CRC chain fold — is cross-checked against the decoder, and the ready boundary re-hashes single-block derived snapshots with Sealr's own bzip2-variant CRC32. Layout identity is `sealrTreeV11`; verified files retain the format-neutral `sealrTreeV1` content identity. Policy v10 with digest `eada8150e14c0f05dcb25b6c9a90b87d3821fbb5f754192aceaea6d942e9f374` authorizes the selection. The authenticated worker refuses it without fallback.

`SevenZInterpretationProfile::CopyPortableV1` selects `sealr.profile.7z.copy-portable.v1`, whose canonical profile digest is `7b6604ad59b5aecf9ebdfa42d7d48d3df663813798992741dd6d74ea56f60b75`. Its IR uses `sealr.archive-ir.7z-copy.v1` and `ArchiveFormat::SevenZCopy`, serialized as `7z-copy`. This is a single-domain container profile: no transform, member payloads at exact original-source ranges, container evidence recording the dense covering, every folder and substream with declared CRCs, and evidence-only attributes and times; every container CRC32 is verified by Sealr itself, and member CRC lies are caught at member verification with precise attribution. Layout identity is `sealrTreeV12`; content identity remains the format-neutral `sealrTreeV1` — shared with the TAR family over the same member set, the first cross-container parity. Policy v11 with digest `afa0aeb04ceca00706b31dfd250216a87f2af0ada6e98d3815873de0d15172fc` authorizes the selection. The authenticated worker refuses it without fallback.

The public `sealr::wheel` module is a pure supported-preview consumer. `evaluate_wheel(outer_filename, verified_archive, limits)` accepts only `PortableUtf8V1`, validates the exact outer filename, `WHEEL`, `METADATA`, `RECORD`, entry points, relocations, generated targets, and bounded plan inspection, then returns one non-exhaustive `WheelEvaluation`: `Admitted`, `Denied`, `Unsupported`, or `InfrastructureFailure`. It receives no source path and uses only `VerifiedArchive` member reads. Infrastructure failures preserve a typed `WheelInfrastructureErrorKind`. `WheelArtifactIR`, `WheelIdentities`, and nested evidence are non-exhaustive prerelease records. `WheelInstallPlan` is evaluator-owned and externally read-only through accessors. `realize_identity` validates its caller-supplied target model, installer policy, canonical output records, complete plan coverage, and unchanged copy bytes before binding them, but does not perform or independently observe installation. It does not replace the caller's duty to reject filesystem outputs outside the plan or validate executable modes. Callers construct each realized-file claim through `RealizedOutput::new`, the struct's only external constructor. The runnable `wheel_admission` example demonstrates the complete consumer loop against a deleted source.

### Copyable PyPA `WheelSource` handoff

The packaged [PyPA handoff example](../crates/sealr/examples/pypa_installer_handoff/README.md) demonstrates the supported public sequence without adding another Sealr API:

1. `LinuxWorker::load_from_manifest` authenticates the helper from the native Linux package.
2. `apply_supervised` admits a file-backed wheel under `ZipInterpretationProfile::PortableUtf8V1` with no fallback.
3. `Outcome::canonical_evidence` emits exact view v2 and receipt v3 bytes, which the separately packaged verifier checks against the live source.
4. `evaluate_wheel` consumes the worker-backed `VerifiedArchive`; Rust retains the returned `WheelInstallPlan` and identities.
5. `VerifiedArchive::read_member` supplies capped member bytes for staging before the explicitly consumed source is deleted.
6. Python receives only a reduced SHA-256-bound manifest and opaque staged blobs. The manifest and canonical receipt digests are separate command arguments, not values trusted solely because they appear inside the manifest. Python never receives the source wheel path or bytes.
7. Rust enumerates the installed roots, rejects links, requires the exact plan target set plus the generated final `RECORD`, validates executable modes, constructs `RealizedOutput` values, and then calls `realize_identity`.

The reduced manifest carries only the bridge and installer policy identifiers, canonical receipt, artifact, and install-plan identities, distribution and wheel-directory names, target information, and bounded member records. It does not serialize the full `WheelArtifactIR` or `WheelInstallPlan` into Python. The first target model is Linux POSIX with `/usr/bin/python3`, five separate scheme roots, no bytecode, no overwrite, a 16 MiB per-member cap, and a 64 MiB aggregate staging cap.
...
```json
{
  ...
  "view_digest": { "sha256": "..." },
  "tool": { "name": "sealr", "version": "0.1.0-alpha.14" },
...
- `interpretation` binds the selected profile identifier and the SHA-256 of its canonical container, encoding, metadata, and name rules. `apply()` selects `sealr.profile.zip.strict-ascii.v1`; `StrictAsciiV2` selects the [closed ASCII v2 contract](profiles/zip-strict-ascii-v2.md); `PortableUtf8V1` selects the [supported Unicode contract](profiles/zip-portable-utf8-v1.md); `WheelUtf8V1` preserves the separately named Alpha.7 research language; `Zip64StrictAsciiV1` selects the [strict ZIP64 contract](profiles/zip64-strict-ascii-v1.md); `TarInterpretationProfile::UstarPortableV1` selects the [portable ustar contract](profiles/tar-ustar-portable-v1.md); `TarGzipInterpretationProfile::UstarPortableV1` selects the [gzip-wrapped ustar contract](profiles/tar-gzip-ustar-portable-v1.md); `TarPaxInterpretationProfile::PortableV1` selects the [restricted PAX contract](profiles/tar-pax-portable-v1.md); `TarGnuLongNameInterpretationProfile::PortableV1` selects the [restricted GNU long-name contract](profiles/tar-gnu-longname-portable-v1.md); `TarGzipInterpretationProfile::PaxPortableV1` and `GnuLongNamePortableV1` select the [gzip-wrapped PAX](profiles/tar-gzip-pax-portable-v1.md) and [gzip-wrapped GNU long-name](profiles/tar-gzip-gnu-longname-portable-v1.md) composition contracts; `TarZstdInterpretationProfile::UstarPortableV1` selects the [zstd-wrapped ustar contract](profiles/tar-zstd-ustar-portable-v1.md); `TarXzInterpretationProfile::UstarPortableV1` selects the [xz-wrapped ustar contract](profiles/tar-xz-ustar-portable-v1.md); `TarBzip2InterpretationProfile::UstarPortableV1` selects the [bzip2-wrapped ustar contract](profiles/tar-bzip2-ustar-portable-v1.md); and `SevenZInterpretationProfile::CopyPortableV1` selects the [Copy-only 7z container contract](profiles/7z-copy-portable-v1.md). Profile selection affects admission and interpretation identity and must be authorized by policy.
- `layout` is `sealrTreeV1` for ZIP32, `sealrTreeV2` for portable ustar, `sealrTreeV3` for ZIP64, `sealrTreeV4` for gzip-wrapped ustar, `sealrTreeV5` for restricted raw PAX, `sealrTreeV6` for restricted raw GNU long-name TAR, `sealrTreeV7` and `sealrTreeV8` for the gzip-wrapped PAX and GNU compositions, `sealrTreeV9` through `sealrTreeV11` for the zstd, XZ, and bzip2 ustar compositions, and `sealrTreeV12` for Copy-only 7z. Each encoding binds canonical paths, kinds, raw names, declared sizes, complete source covering, format-specific member evidence, and normalization actions. Stateful TAR profiles additionally bind ordered extension records or carriers, effective values, and provenance. It is present once an `ArchiveIR` exists. It is `{ "status": "unavailable" }` when planning never produced a tree.
...
- `read_member_prefix(canonical_path, max_prefix_bytes)` returns at most the requested leading bytes only after the complete member size, CRC32, and SHA-256 have been verified;
- the supervised backend caps returned full or prefix bytes at 63 MiB, so a large verified member can still be classified through a small prefix while a request that would retain more reports `LimitExceeded` before worker spawn;
