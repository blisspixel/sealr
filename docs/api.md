...
This page distinguishes the implemented preview contract from the target semantic API. Alpha.11 is the latest published release; current main additionally implements the restricted raw GNU long-name profile under policy v6, the gzip-wrapped restricted PAX and GNU long-name compositions under policy v7, and the zstd-wrapped portable ustar profile under policy v8. The implemented surface includes explicitly selected portable ustar, strict ZIP64 under policy v3, strict gzip-wrapped portable ustar under policy v4, and restricted raw POSIX PAX under policy v5, all without changing the compatibility `apply()` behavior or ZIP32 default. Current callers must pin to the implemented section. The target is specified further in [semantic-model.md](semantic-model.md).
...
pub struct Request<'a> {
    ...
    pub policy: &'a Policy,          // sealr.policy.v1 through sealr.policy.v8
