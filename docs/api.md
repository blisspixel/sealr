...
pub struct Request<'a> {
    ...
    pub policy: &'a Policy,          // sealr.policy.v1 through sealr.policy.v4
...
`ApplyOptions` carries one explicit `ArchiveSelection`. The current variants are `Zip(ZipInterpretationProfile)`, `TarUstar(TarInterpretationProfile)`, and `TarGzipUstar(TarGzipInterpretationProfile)`. `archive_selection()` and `archive_format()` expose the exact choice; the compatibility `interpretation_profile()` accessor remains ZIP-shaped for existing callers, so new multi-format code should use the format-specific accessors. Filename suffixes never select or retry a parser. A selected format must also be authorized by the policy before source ingestion. `Policy::default_v1()` authorizes ZIP32 only; `Policy::default_v2()` adds portable ustar; `Policy::default_v3()` adds ZIP64; and `Policy::default_v4()` adds gzip-wrapped portable ustar with a mandatory derived-archive cap. `apply()` remains strict ASCII ZIP32 v1 and does not alias any newer profile.
