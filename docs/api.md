...
This page distinguishes the implemented preview contract from the target semantic API. Alpha.11 is the latest published release. It includes explicitly selected portable ustar, strict ZIP64 under policy v3, strict gzip-wrapped portable ustar under policy v4, and restricted raw POSIX PAX under policy v5 without changing the compatibility `apply()` behavior or ZIP32 default. Current callers must pin to the implemented section. The target is specified further in [semantic-model.md](semantic-model.md).
...
pub struct Request<'a> {
    ...
    pub policy: &'a Policy,          // sealr.policy.v1 through sealr.policy.v5
