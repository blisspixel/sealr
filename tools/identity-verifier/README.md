...
- For bzip2-wrapped ustar it reconstructs the composed `sealr.profile.tar-bzip2.ustar-portable.v1` digest and the `sealr.transform.bzip2.bzip2fmt-single-stream.v1` transform and decoder-parameter digests, replays the restricted bit-aligned bzip2 container grammar over each case's source bytes (fixed `BZh` header and level digit, unique-shift footer recovery with zero padding bits, a full 48-bit block-magic scan whose offsets must match the evidence exactly, per-block CRC and `randomised`-flag extraction, and a combined-CRC chain fold that must reproduce the footer), binds the committed derived TAR bytes through length, SHA-256, and — for single-block streams — an independently implemented bzip2-variant CRC-32 (MSB-first 0x04C11DB7) over the entire derived bytes, re-parses both the shared and the multiblock derived TARs under the portable-ustar language, and reproduces the wrapped `sealrTreeV11`, raw `sealrTreeV2`, and `sealrTreeV1` roots for the two single-block cases and the three-block multiblock section.
- For Copy-only 7z it reconstructs the `sealr.profile.7z.copy-portable.v1` digest, re-parses each case's source bytes as a restricted raw-header Copy-only container (signature and version, both header CRC32s, minimal variable-length integers, MSB-first bit vectors with zero padding, Copy-only single-coder folders whose unpack sizes equal their pack sizes, the dense signature/pack-streams/next-header covering, the substream last-implied-size and known-CRC-exclusion rules, and the empty-stream/empty-file matrix with directory-attribute agreement), cross-checks every folder, substream, and member against the embedded IR including re-hashed per-member CRC32 and SHA-256 payload facts, reproduces each `sealrTreeV12` layout preimage byte for byte, and verifies the first cross-container parity pin: the single-file case's `sealrTreeV1` content root equals the TAR family's shared root.
