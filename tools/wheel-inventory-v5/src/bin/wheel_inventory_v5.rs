...
fn render(report: &Report) -> String {
    ...
    writeln!(out, "- `wheel.header-duplicate`: fourteen artifacts carry a duplicated `Generator` field. The cffi and matplotlib pair from the v4 pilot are joined in the stratified v5 corpus by cbor2, ddtrace, duckdb, h5py, mmh3, pymssql, pyroaring, simplejson, statsmodels, thrift, xgboost, and zopfli, almost all cp310 macOS artifacts whose build pipelines emit a second generator line. The supported model still denies duplicated headers because it has not defined ordered or merged generator semantics; this is a consumer-compatibility gap, not a container disagreement, and it is the largest denial cluster in this corpus.").unwrap();
...
- `zip.extra`: the protobuf Windows artifact carries extra field `0x0001` (ZIP64 extended information) in a local header, which `sealr.profile.zip.portable-utf8.v1` denies. This remains the only container-stage denial in the corpus and would require the strict ZIP64 profile lineage to interpret.\n").unwrap();
    ...
    .unwrap();
    writeln!(out, "- The `scripts` scheme is measured on twelve admitted artifacts: awscli, dill, dulwich, jmespath, maturin, ninja, numba, patchelf, pywin32, ruff, ty, and uv. This extends the native-CLI observation beyond the two flipped artifacts, but the stratified corpus is not a population-prevalence sample.
