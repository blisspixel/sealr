...
use sealr_wheel_lab::{evaluate_wheel, InstallScheme, WheelEvaluation, WheelLimits};
use serde::{Deserialize, Serialize};
use sha2::{Digest, Sha256};

const REPORT_SCHEMA: &str = "sealr.wheel-compatibility-report.v2";
const ANALYZER: &str = "sealr-wheel-inventory.v2";
...
fn main() {
    if let Err(error) = run() {
        eprintln!("wheel inventory v2: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), AnyError> {
    ...
    match args.next().as_deref() {
        ...
        Some("verify") => {
            ...
            println!("wheel compatibility inventory v2 is internally consistent");
        }
        _ => return Err(usage()),
    }
    Ok(())
...
fn analyze(manifest_path: &Path, cache: &Path) -> Result<(Vec<u8>, Vec<u8>), AnyError> {
                ...
                WheelEvaluation::Admitted {
                    ...
                    for facts in wheel.member_facts {
                        *artifact
                            ...
                            .or_default() += 1;
                        artifact.executable_members +=
                            u64::from(facts.pypa_installer_0_7_executable);
                    }
                }
    ...
    let report = Report {
        ...
        predecessor_report_sha256: hex_sha256(&fs::read("tests/corpus/wheels/report.json")?),
        manifest_sha256: hex_sha256(&raw_manifest),
        query_date: manifest.query_date,
        selection_method: manifest.selection_method,
        analyzer: ANALYZER.into(),
        interpretation_profile: ZIP_WHEEL_UTF8_V1.into(),
        interpretation_profile_sha256: sealr::zip_wheel_utf8_v1_digest(),
...
fn render(report: &Report) -> String {
    ...
    writeln!(out, "# Wheel compatibility inventory v2\n").unwrap();
    writeln!(out, "> Status: non-shipping research evidence over the pinned 20-wheel pilot. This is not a PyPI prevalence estimate or a support claim.\n").unwrap();
    ...
    writeln!(out, "- `wheel.header-duplicate`: the cffi artifact contains two `Generator` fields. The research model denies this because it has not defined ordered or merged generator semantics; this is a consumer-compatibility gap, not a container disagreement.").unwrap();
    writeln!(out, "- `wheel.metadata-version-unsupported`: Hatchling and wheel declare Core Metadata 2.5. The pinned research snapshot implements 2.1 through 2.4, so both are unsupported rather than denied.").unwrap();
    writeln!(out, "- `quota.ratio`: SciPy reaches the existing default 100:1 expansion ceiling on three test-data members before wheel evaluation. The laboratory does not weaken the adversarial resource policy to improve corpus acceptance.\n").unwrap();
    ...
    writeln!(
        out,
        "- PyPA installer 0.7.0 executable members: `{}`\n",
...
fn verify(manifest: &Path, report_path: &Path, markdown_path: &Path) -> Result<(), AnyError> {
    ...
    if report.schema != REPORT_SCHEMA
        ...
        || report.interpretation_profile != ZIP_WHEEL_UTF8_V1
        || report.interpretation_profile_sha256 != sealr::zip_wheel_utf8_v1_digest()
        || report.predecessor_report_sha256
            != hex_sha256(&fs::read("tests/corpus/wheels/report.json")?)
    {
...
fn stage(value: sealr_wheel_lab::EvaluationStage) -> String {
...
fn usage() -> AnyError {
    "usage: sealr-wheel-inventory-v2 analyze <manifest> <cache> <report> <markdown> | verify <manifest> <report> <markdown>".into()
}
