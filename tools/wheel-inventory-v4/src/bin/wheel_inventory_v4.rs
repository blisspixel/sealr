//! Successor-bound inventory for the widened wheel-consumer snapshot.
//!
//! This binary remains separate from the frozen `wheel-lab` analyzers so the
//! Alpha.7 and Alpha.8 reports stay executable under their exact historical
//! profiles. It measures the supported `sealr::wheel` evaluator under the
//! `pypa-wheel-core-metadata-2026-08-28` specification snapshot and binds the
//! immutable v3 report so every outcome delta is attributable.
...
const REPORT_SCHEMA: &str = "sealr.wheel-compatibility-report.v4";
const ANALYZER: &str = "sealr-wheel-inventory.v4";
const PREDECESSOR_REPORT: &str = "tests/corpus/wheels/report-v3.json";
...
/// The subset of the immutable v3 report needed to attribute outcome deltas.
#[derive(Deserialize)]
...
/// One artifact shared with the v3 corpus whose outcome or finding codes
/// changed under the widened snapshot.
...
fn main() {
    if let Err(error) = run() {
        eprintln!("wheel inventory v4: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), AnyError> {
    ...
    match args.next().as_deref() {
        ...
        Some("verify") => {
            ...
            println!("wheel compatibility inventory v4 is internally consistent");
        }
        _ => return Err(usage()),
    }
    Ok(())
...
fn load_predecessor() -> Result<(String, BTreeMap<String, PredecessorArtifact>), AnyError> {
    ...
    if report.schema != "sealr.wheel-compatibility-report.v3" {
        return invalid("predecessor report schema is not v3");
    }
...
fn render(report: &Report) -> String {
    ...
    writeln!(out, "# Wheel compatibility inventory v4\n").unwrap();
    writeln!(out, "> Status: supported-preview measurement of the widened Core Metadata 2.1-2.6 snapshot over a 64-artifact stratified corpus. This is not a PyPI prevalence estimate or a claim of general wheel compatibility.\n").unwrap();
    ...
    writeln!(out, "- `wheel.header-duplicate`: the cffi and matplotlib artifacts each contain two `Generator` fields, cffi from its historical toolchain and matplotlib from its meson-python build. The supported model denies duplicated headers because it has not defined ordered or merged generator semantics; this is a consumer-compatibility gap, not a container disagreement.").unwrap();
    writeln!(out, "- `quota.ratio`: SciPy reaches the existing default 100:1 expansion ceiling on three NIST ANOVA test-data members before wheel evaluation, exactly as in v3. The portable profile does not weaken the adversarial resource policy to improve corpus acceptance.").unwrap();
    writeln!(out, "- `wheel.script-aggregate-limit`: uv and ruff each ship one multi-megabyte native executable under `.data/scripts`. Script-scheme members are inspected for launcher transforms, and their full declared sizes count against the checked 16 MiB plan-inspection aggregate before any read, so a single oversized script member exceeds the cap and the artifact is denied. Raising or scoping this budget is a policy decision for a future limits revision, not a silent widening.").unwrap();
    writeln!(out, "- `zip.extra`: the protobuf Windows artifact carries extra field `0x0001` (ZIP64 extended information) in a local header, which `sealr.profile.zip.portable-utf8.v1` denies. This is the only container-stage denial among the 44 additions and would require the strict ZIP64 profile lineage to interpret.\n").unwrap();
    ...
    .unwrap();
    writeln!(out, "- Benign `.data` payloads, absent from the v3 pilot, are now measured on four admitted artifacts: jupyterlab and notebook relocate the `data` scheme, and ninja and pywin32 relocate the `scripts` scheme.").unwrap();
    writeln!(out, "- No measured artifact declares Core Metadata 2.6 yet; that half of the widening is currently exercised only by unit fixtures.").unwrap();
...
fn verify(manifest: &Path, report_path: &Path, markdown_path: &Path) -> Result<(), AnyError> {
    ...
    if report.predecessor_delta != predecessor_delta(&predecessor, &report.artifacts) {
        return invalid("inventory predecessor delta does not match the bound v3 report");
    }
...
fn usage() -> AnyError {
    "usage: wheel_inventory_v4 analyze <manifest> <cache> <report> <markdown> | verify <manifest> <report> <markdown>".into()
}
