...
There is no production-ready or stable supported version. The latest published release is Alpha.11, and current main remains an alpha preview. The current limitations are listed in [README.md](README.md). The complete pinned ZipDiff construction corpus is enforced in CI. Portable UTF-8 ZIP paths, raw portable ustar, strict ZIP64 through the explicit `Zip64StrictAsciiV1` selection authorized by policy v3, strict single-member gzip-wrapped portable ustar through its explicit policy v4 selection, and a restricted raw POSIX PAX language through its explicit policy v5 selection are supported preview behavior. None of the newer profiles widens, detects, retries, or aliases through an older default. The gzip-TAR profile retains the exact compressed source and one bounded immutable decoded TAR, independently audits both coverings and their exact transform binding, and creates no destination stage before that composite authority passes. The PAX profile independently audits the source covering and canonical extension-state replay before TreeV5 identity or materialization. Authenticated worker execution rejects ZIP64, gzip-wrapped ustar, and PAX without fallback until a later semantic record can represent their evidence. Production containment, legacy CP437, signed receipts, broader TAR dialects, additional compression wrappers, and the remaining major-format families are not complete.
