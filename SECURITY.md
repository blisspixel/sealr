...
There is no production-ready or stable supported version. The latest published release is Alpha.9, and current main remains an alpha preview. The current limitations are listed in [README.md](README.md). The complete pinned ZipDiff construction corpus is enforced in CI. Portable UTF-8 ZIP paths and raw portable ustar are supported preview behavior. Current main also exposes strict ZIP64 through the explicit `Zip64StrictAsciiV1` selection authorized by policy v3 and strict single-member gzip-wrapped portable ustar through its explicit selection authorized by policy v4. Neither widens, detects, retries, or aliases through the ZIP32 default. The gzip-TAR profile retains the exact compressed source and one bounded immutable decoded TAR, independently audits both coverings and their exact transform binding, and creates no destination stage before that composite authority passes. Authenticated worker execution rejects ZIP64 and gzip-wrapped ustar without fallback until a later semantic record can represent their evidence. Production containment, legacy CP437, signed receipts, rich TAR dialects, additional compression wrappers, and the remaining major-format families are not complete.
