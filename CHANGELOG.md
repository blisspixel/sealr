...
- Captured the matrix's pending IR at the real post-covering, pre-setup and pre-verification boundary. The test-only hook is explicitly scoped, so unrelated unit-test applies do not clone or retain an IR.
...
- Made executable semantic shadow cases obtain their completion frames from the source-owning plan executor. The canonical 12-case manifest and its digest remain unchanged.
