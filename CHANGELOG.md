...
- Added a bounded kernel-sealed blob transport to the Linux worker lab. `SLRBLOB1` memfds require write, grow, shrink, and seal seals; bind an envelope role and exact length; independently recompute SHA-256; and reject missing seals, length drift, role confusion, digest disagreement, and trailing bytes. Synthetic planning payloads bind the operation and source identity, synthetic completions bind the operation, planning digest, and result, and procfs observation proves the exact live source, plan, and completion descriptors. This closes transport mechanics, not semantic-record execution.
