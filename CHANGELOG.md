...
- Kept the semantic-record experiment outside every `apply`, CLI, default-feature, and shipped runtime path. An unsupported hidden byte-slice driver is exported only when the nondefault fuzz feature is explicitly enabled. The experiment derives final non-effect axes from validated findings and member state, while effect, cleanup, audit, publication, verdict, receipt, retained content, later reads, and `VerifiedArchive` construction remain supervisor-owned or future capability work.
