...
- Added a required 500-iteration native Linux worker stress campaign over a closed 36-case matrix. Every success, descriptor rejection, protocol rejection, restriction failure, raw transport rejection, and abrupt-exit case runs at least 13 times. Each iteration requires no surviving child PID, no supervisor descriptor-count growth, exact source identity and bytes, an unchanged outside sentinel, proved reap, checked cleanup, and an absent fixture root.
