...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, applies a fixed Landlock floor and x86_64 seccomp no-descendant and permission-mutation controls before source transfer, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. The shared crate-private planner now moves the exact snapshot, pending IR, planning findings, profile, policy identity, and compiled controls into ordinary `apply()` or the in-crate conformance harness without cloning or record round-trips. The [private semantic-record](docs/semantic-record.md) slice has exact invocation and source binding, hostile bounded decode, a plan-native Store and Deflate executor without structural reparse, a required three-platform near-limit completion probe, deterministic fuzz infrastructure, and 24 named v1 plus v2 shadow observations with explicit v2 oracle ownership. This is still a dormant bounded projection, and a correlated completion remains an untrusted proposal that can carry an arbitrary file-content SHA-256. The next dependencies are raw ancillary rejection, authority-epoch deadlines, and repeated native stress, followed by a sealed repository-only inspect bridge. Scheduled history, independent content authority, retained-content transfer, isolated later reads, helper packaging, writer quiescence, stage audit, and supervisor-owned publication remain separate gates.
