...
| BZip2 (12) | older ZIP toolchains | Planned adapter; the reviewed `libbz2-rs-sys` boundary already ships in the TAR wrapper |
| LZMA (14) | 7-Zip ZIP files | Planned adapter |
| XZ (95) | newer ZIP toolchains | Planned adapter; the reviewed `lzma-rust2` boundary already ships in the TAR wrapper |
...
| restricted raw GNU long-name | Released in Alpha.12 as an explicit policy v6 in-process preview with zero new runtime dependencies |
...
| gzip-wrapped restricted PAX | Released in Alpha.12 as an explicit policy v7 composition of the frozen wrapper and PAX languages with `sealrTreeV7` |
| gzip-wrapped restricted GNU long-name | Released in Alpha.12 as an explicit policy v7 composition of the frozen wrapper and GNU languages with `sealrTreeV8` |
| zstd-wrapped portable ustar | Released in Alpha.12 as an explicit policy v8 in-process preview; the first Gate B codec promotion adds exactly `ruzstd` and `twox-hash` |
| bzip2-wrapped portable ustar | Released in Alpha.12 as an explicit policy v10 in-process preview; the third Gate B codec promotion adds exactly `bzip2` and `libbz2-rs-sys` |
| Copy-only 7z container | Released in Alpha.12 as an explicit policy v11 in-process preview; the first Gate C structure step adds zero dependencies and proves cross-container content parity |
| xz-wrapped portable ustar | Released in Alpha.12 as an explicit policy v9 in-process preview; the second Gate B codec promotion adds exactly `lzma-rust2` |
...
Further codec breadth follows the usefulness and review gates. The shipped zstd, XZ/LZMA2, and bzip2 TAR wrappers already demonstrate the promotion path; adding more codecs now would multiply trusted code before independent adoption, Unicode-path measurement, and review are done.

## Executive decision

The latest published milestone is **Alpha.13: supervised wheel handoff and exact Poetry 2.4.2 fixture**. It releases full-integrity supervised prefix reads, a packaged public-API-only `WheelSource` handoff, and one exact repository-owned Poetry update seam while retaining the project-wide alpha status. It is a GitHub-only prerelease and does not publish a crate to crates.io.
...
Format breadth is an explicit second lane, not a substitute for a dependent. Alpha.9 proved the container-neutral core with portable raw ustar and no new runtime dependency. Alpha.10 proved the original-snapshot structural path with explicit ZIP64 and the exact two-domain transform path with gzip-wrapped portable ustar, also without a new dependency. Alpha.11 added restricted raw PAX as a separate language with fixed state, provenance, independent covering replay, and `sealrTreeV5`, again without a new dependency. Alpha.12 released the narrow GNU long-name-only profile, the gzip compositions, the separately promoted zstd, XZ/LZMA2, and bzip2 wrappers, and the Copy-first 7z structure. With twelve explicit selections proven, parser breadth is deliberately frozen. Alpha.13 releases supervised prefix parity, the copyable public-API-only wheel handoff, and the exact Poetry 2.4.2 repository fixture. The active sequence is independent adoption, measured compatibility, stable API and identity review, and independent security review. The parked 7z LZMA member and packed-header steps on branch `feature/7z-lzma-portable-v1` resume afterward. A local 7z interpreter started with Copy before reusing the reviewed LZMA layer, exactly as planned. The cpio, ar/deb, CAB, RPM, and RAR5 programs remain separately gated. ISO 9660, UDF, and other filesystem images belong to a separate program. Targeted wheel feature coverage, stable identity and API review, authenticated recovery, durability, and scheduled history continue because they measure the first real supported consumer.
...
The [usefulness test](docs/usefulness.md) is the product gate: same bytes and policy produce one tree or no tree on Linux, macOS, and Windows, and the next tool consumes that admitted tree instead of opening the ZIP again. The repository wheel evaluator, installer conformance kit, [copyable packaged handoff](crates/sealr/examples/pypa_installer_handoff/README.md), and [exact Poetry fixture](tests/poetry-consumer/README.md) now prove that mechanism locally. The remaining product exit is a separately maintained consumer that treats the capability and independently verified evidence as authoritative. CI protects the boundary through the corpus, lockfile, cargo-deny, native materialization, packaged handoff, and exact integration fixtures.
...
- a current 300-artifact wheel inventory with 280 admissions and 20 individually investigated denials, measured scripts, headers, and data schemes, and supervised script-prefix parity that authenticates complete member streams without fallback;
...
- a pinned weekly assurance workflow with three exact full-domain scalar Kani harnesses, bounded mutation, source-coverage, and compiler-accurate public API compatibility discovery, retained reports, explicit nonclaims, and a machine-checked ten-run promotion ledger that keeps six evidence categories separate;
...
- the `v0.1.0-alpha.12` canonical-evidence and independent-verification preview line, with the native verifier, packaged PyPA conformance, strict policy documents, explicit CLI operations, seven additional format selections, fourteen bounded release fuzz jobs, and the source full-read integrity correction.
- the `v0.1.0-alpha.13` supervised-wheel-handoff preview line, with full-integrity supervised prefix reads, the packaged public-API-only `WheelSource` handoff, and the exact Poetry 2.4.2 repository fixture.
...
3. **Alpha.6 supervised Linux worker: released and extended.** The authority bootstrap closes inherited authority, applies a fixed Landlock floor and x86_64 seccomp controls before source transfer, validates every raw ancillary header while immediately owning received descriptors, enforces supervisor-owned absolute monotonic deadlines, exercises bounded lifecycle failures, and proves reap before checked cleanup. Eleven deterministic stalls cover changing authority from pre-bootstrap receive through post-ack exit. A required 500-iteration Linux campaign cycles the complete 44-case non-stall matrix while proving no child survives and no supervisor descriptor count grows. Bounded kernel-sealed plan, completion, retained-content, full-read, and prefix-read request records prove required memfd seals, exact length, independently recomputed SHA-256, operation and source binding, live descriptor identity, cross-kind rejection, malformed-plan rejection, and lifecycle behavior. The shared crate-private planner moves the exact snapshot, pending IR, planning findings, profile, policy identity, and compiled controls into ordinary `apply()` or the worker path without cloning or record round-trips. The semantic-record slice has exact invocation and source binding, hostile bounded decode, plan-native Store and Deflate execution without structural reparse, immutable original-pass retention, a required three-platform near-limit completion probe, deterministic fuzz infrastructure, and 24 named v1 plus v2 shadow observations with explicit v2 oracle ownership. The public supervisor covers inspect, materialize, full reads, and bounded prefix reads through the generic adapter. Prefix reads stream and independently authenticate the complete member while retaining only the requested bytes. The helper feature handshake fails incompatible helpers before source transfer. Materialization retains destination and publication authority in the supervisor, waits for clean exit and exact reap, validates completion through retained-source replay, audits the exact root, and alone performs no-replace publication. A required pinned-kernel QEMU gate proves both public operations fail closed on actual Landlock ABI 2. The CLI, wheel laboratory, and packaged consumer use the same fixed-manifest boundary without fallback.
4. **Continuous assurance lane: governance active.** Pure range and quota models, property tests, three scalar Kani harnesses, fuzzing, native race stress, targeted mutation discovery, source-coverage discovery, and compiler-accurate public API compatibility now run under explicit bounds and nonclaims. The six-category promotion ledger binds the API check to exact packaged Alpha.12 commit `9d762a596624415b8499b774707d018cf128dbd6`, requires zero expected warnings, makes the clean category promotable, and starts its qualifying history from zero. Stable scheduled history remains an accumulating observation, not a reason to weaken or duplicate required CI. Step 8 continues with path and lifecycle models and new targets driven by concrete gaps.
5. **Parallel wheel laboratory: research implementation complete.** The first reproducible benign [compatibility pilot](docs/wheel-compatibility-pilot.md) remains immutable. A separate [v2 semantic inventory](docs/wheel-compatibility-v2.md) binds that predecessor and evaluates the exact bytes under an exhaustive wheel UTF-8 profile through `VerifiedArchive` only. The pure bounded evaluator validates filenames, wheel and core metadata, `RECORD`, entry points, relocation, generated targets, and distinct identity domains. Minimized hostile fixtures pin every implemented decision boundary. The Alpha.7 tag retains the hash-pinned PyPA installer 0.7.0 bridge as historical research. Current required conformance uses the public evaluator and exact installer 1.0.1 from an isolated extracted-package project. The laboratory remains non-published research, not supported wheel admission.
...
12. **Canonical evidence verification: released in Alpha.12.** `Outcome::canonical_evidence()` and CLI `--canonical` emit byte-exact RFC 8785 `sealr.view.v2` and `sealr.receipt.v3` documents. The no-Sealr-dependency verifier checks those bytes, pair bindings, observed source, every registered interpretation profile, known default-policy identities, outcome and effect consistency, and the format-neutral content root. Five committed evidence cases and hostile mutations run in required CI. The statement builder accepts v3 only with its verified view and actual source, then embeds the original receipt token in an unsigned in-toto Statement v1.
13. **Native canonical-evidence verifier delivery: released in Alpha.12.** Every native package contains the no-Sealr-dependency `sealr-identity-verifier` beside the CLI. The shared package verifier runs only the extracted pair on all three release floors, accepts admitted and coherent rejected canonical evidence, binds the observed source, and rejects mutations to either document, the source, or the pair. Exact archive shape, modes, help, version, misuse, dependency closure, and release workflow wiring are required gates. This removes the checkout and Rust-toolchain barrier for the next adopter; it does not authenticate its own release archive, complete external adoption, or widen the evidence claim to live layout reconstruction.
14. **Supervised wheel-consumer parity: released in Alpha.13.** Worker-backed `VerifiedArchive::read_member_prefix` now binds a distinct private request to the exact source, accepted plan, originating effect, completion, member, and caller prefix cap. The worker streams the complete member, and the supervisor retains only the prefix while independently checking complete size, CRC32, and SHA-256 before clean exit, reap, and release. Store, Deflate, zero, exact, oversized, clone, cancellation, timeout, crash, discarded-tail corruption, recovery, repeat, and descriptor-stability evidence cover the boundary. The extracted-package consumer evaluates a supervised `.data/scripts` wheel without fallback.
15. **Packaged downstream handoff: released in Alpha.13.** The [compact copyable example](crates/sealr/examples/pypa_installer_handoff/README.md) uses the public `sealr` crate with no internal feature or workspace dependency and pins PyPA installer 1.0.1 exactly. Rust authenticates the native packaged worker, requires independent canonical-evidence verification, retains the exact plan, and stages only bounded capability reads. Python receives a reduced SHA-256-bound manifest and opaque blobs, with the canonical receipt and manifest digests supplied out of band, and never receives the wheel path or bytes. Rust rejects links, missing or extra outputs, and executable-mode drift before deriving the realization identity. Required CI builds the example inside the extracted `.crate`, builds its copied standalone form against that same extraction, and runs a mixed Store-and-Deflate fixture through supervised inspect and materialize origins with exact identity parity. The real pinned installer is a third end-to-end run. Detailed fixtures and oracles remain in repository tests, which consume the exact packaged Python bridge.
16. **Poetry 2.4.2 repository fixture: released in Alpha.13.** Exact Poetry 2.4.2 and its Ubuntu 24.04 x86_64 CPython 3.12 wheel closure are hash-pinned. Poetry performs lock-hash validation first. PREPARED follows supervised admission, independent evidence verification, evaluation, bounded staging, private-source deletion, exact target-interpreter validation, and target-read-only preflight while the old distribution remains installed. Poetry update uninstall and all target writes occur only afterward. A digest-bound permit resumes the packaged handoff, the host adapter and installer bridge deny `.whl` opens after PREPARED, the 13 realized outputs match the pinned realization identity, and the complete non-bytecode target snapshot matches the stock control.
17. **External usefulness exit: next.** A separately maintained publisher, registry, build backend, or installer must treat the capability and independently verified evidence as authoritative.

Raw portable ustar, restricted raw PAX, the narrow GNU long-name-only raw profile, strict gzip-wrapped portable ustar, the gzip-wrapped PAX and GNU compositions, format-native archive and member evidence, domain-qualified retained payload plans, one validated `VerifiedArchive` construction authority, immutable original and derived snapshots, exact transform graphs, and the explicit strict ZIP64 in-process path are landed. The zstd wrapper is promoted through the executed ruzstd Gate B review under policy v8, the xz wrapper through the executed lzma-rust2 Gate B review under policy v9, the bzip2 wrapper through the executed bzip2/libbz2-rs-sys Gate B review under policy v10, and the Copy-first 7z structure through the executed Gate C first step under policy v11. Native evidence-verifier delivery, supervised wheel prefix parity, the packaged `WheelSource` handoff, and the exact Poetry 2.4.2 fixture close the repository-owned binary, evaluator-composition, copyability, and injected-update gaps. The immediate sequence is external usefulness, measured compatibility, stable review, and independent review in the [near-term execution plan](docs/near-term.md). The parked 7z LZMA/LZMA2 member and packed-header step over that structure with the reviewed lzma-rust2 layer resumes afterward. cpio, ar/deb, CAB, RPM, and RAR5 follow only through measured gates. ISO 9660, UDF, and filesystem images remain separate. ZIP64 and TAR worker-record parity, targeted wheel evidence, authenticated recovery, durability, stable identities, and avoided-work performance continue in parallel.
...
3. Follow the [accepted private-record and explicit public hybrid decision](docs/decisions/0001-alpha6-semantic-ownership.md). Preserve IR on destination setup failure and the single-interpretation property. **Shared planning, generic self-bound worker execution, exact-byte authority, immutable inspect and materialize retention, full and prefix reads, reaped writer lifecycle, and explicit public activation have landed:** ordinary `apply()` consumes one non-cloneable owning planning result through its in-process continuation. Linux `apply_supervised` encodes that same ready plan, transfers it through a sealed memfd, validates it against the restricted worker's exact file-backed snapshot, and maps only source-authorized results into the public outcome and capability contract. Later non-retained full and prefix reads use separately bound request kinds and a fresh worker with no stage or destination authority, preserve the originating effect binding, authenticate the complete member stream, and release only the authorized output after clean exit and reap. Materialization gives the worker only the supervisor-created stage, source, and sealed plan; exact completion and retention validation, retained-source replay, stage audit, cleanup, and no-replace publication remain supervisor-owned after clean exit and reap. A required isolated-child probe measures completion reconstruction near the record cap without claiming planning, transport, retained-transfer, isolated-read, or RSS bounds. Ordinary calls do not unexpectedly sandbox their caller because the supervised path is explicit and fails closed.
...
7. Use targeted mutation testing and coverage reports to find weak assertions and blind branches. Add compiler-accurate public API comparison against an exact packaged release baseline. Do not use a global coverage percentage as an assurance claim. **Scheduled discovery landed:** exact tool versions, source and function scope, API baseline, authenticated tool artifact, expected-warning state, timeouts, report retention, failure semantics, and nonclaims are machine checked. Mutation and coverage are explicitly non-promotable scores. API compatibility is promotable from the exact clean Alpha.12 baseline, starts history at zero, and still requires ten clean scheduled `main` runs plus explicit review.
...
- **Historical research retained:** the Alpha.7 inventory records exact PyPA installer 0.7.0 semantics;
- **Repository conformance landed:** an isolated project compiles only against Cargo's extracted `sealr` package and drives exact PyPA installer 1.0.1 through the public portable profile, wheel evaluator, and realization identity;
- **Repository conformance landed:** canonical evidence is independently verified, the original wheel is deleted before the post-admission installer bridge starts, bounded member blobs come only from `VerifiedArchive`, every `.whl` open is denied, and both a controlled fixture and the real hash-pinned installer wheel complete through fresh filesystem roots;
- **Packaged handoff landed:** the public-API-only copied example keeps the exact plan in Rust and gives Python only a reduced digest-bound manifest and bounded member blobs, with the canonical receipt and manifest digests supplied out of band;
- **Required package proof landed:** the copied project resolves against Cargo's extracted `.crate`, enables no internal Sealr feature, uses the helper manifest and verifier from the extracted Linux native package, and exercises both supervised inspect and materialize origins;
- **Required denial gates landed:** lying `RECORD`, traversal, closed-schema, out-of-band receipt, raw-manifest, and staged-member tampering fail before installation effects; implemented post-install audits reject links, output-set drift, and executable-mode drift before identity acceptance, while exact controlled and real-distribution identity pins detect semantic drift;
- **Poetry repository fixture landed:** exact Poetry 2.4.2 and its Ubuntu 24.04 x86_64 CPython 3.12 wheel closure are hash-pinned; one isolated serial wheel update consumes the packaged handoff through a repository-owned injected executor;
- **Poetry phase boundary landed:** Poetry's lock validation occurs first; PREPARED follows Sealr admission, independent evidence verification, evaluation, bounded staging, private-source deletion, exact target-interpreter validation, and target-read-only preflight while the old distribution remains installed. Poetry update uninstall and all target writes occur only afterward, with post-PREPARED `.whl` opens denied in the host adapter and installer bridge;
- **Poetry nonclaim:** this does not establish a public Poetry extension API, general Poetry compatibility, sdist, VCS, path, or build support, resolver or repository trust, rollback, transactionality, concurrent target mutation safety, general existing-environment overwrite safety, external adoption, or production readiness;
- **Nonclaim:** this is a repository-owned contract for one Linux POSIX target model, not proof that a separately maintained project has adopted Sealr;
...
- GNU long-name parsing is released in Alpha.12 as a separate dialect profile rather than widening portable ustar or mixing PAX and GNU state, and each frozen raw dialect composes separately with the exact gzip transform under its own layout identity;
