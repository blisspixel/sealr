...
Do not add TAR, 7z, MCP, language bindings, accelerators, extra ZIP methods, a desktop CLI, or archive-decision signing first. Those are not substitutes for a dependent. The current implementation has a strict but incomplete ZIP32 parser, an executable ZipDiff gate, component-bound output, a private snapshot, and a dormant split-phase semantic contract with an in-process plan-native inspect executor. Near-limit completion heap measurement has landed. The next work is broader semantic parity, pre-parser authority closure, a real plan-only seam, sealed isolated plan execution, and the remaining worker content gates, followed by canonical Unicode paths and a real consuming boundary. Layered adversarial assurance continues with every increment.
...
- a Linux authority-bootstrap lab with sequenced-packet descriptor transfer, pre-exec closure plus independently probed child-entry descriptor closure, a runtime-queried Landlock ABI 3 floor before source transfer, process-boundary malformed-message evidence, bounded lifecycle failures, pidfd termination and reap, and checked cleanup;
- a dormant crate-private split-phase semantic-record experiment with independent magic, bounded hostile decode, exact supervisor correlation and source binding, input-sized fallible validation allocations, single-materialization completion reconstruction, a required near-limit isolated-child heap probe, moved findings, pinned record vectors, a versioned 12-case shadow matrix against the current in-process semantics, and a source-owning inspect executor that consumes accepted Ready plans through shared bounded payload verification without structural reparse; a dedicated deterministic-seed target has clean exact-main on-demand Linux AddressSanitizer evidence, while scheduled-event history remains pending.
...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, installs `no_new_privs` and a fixed Landlock ABI 3 policy before source transfer, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. The [private semantic-record](docs/semantic-record.md) slice makes the split-phase decision executable with exact invocation binding, source-backed structural and fixed-field checks, parser-equivalent metadata accounting, phase-specific finding validation, input-sized fallible validation allocations, source-ordered pending IR, a plan-digest-bound completion vector, one fallible completion IR materialization, moved findings, hostile decoding, and a versioned 12-case in-process shadow matrix that includes setup failure. Its source-owning inspect executor consumes accepted Ready plans without structural reparse and emits canonical completion frames through the shared Store and Deflate verifier. A required three-platform probe now pins a plan 67,760 bytes below the private 64 MiB cap and enforces that accepted completion reconstruction retains one logical output with less than 1 MiB transient requested heap above it. A dedicated deterministic-seed fuzz target has parsed Cargo-target and complete scheduled-workflow verification. The experiment remains dormant. Its decoded completion is only a bound proposal: correlation cannot prove file-content computation, and a file proposal can carry an arbitrary SHA-256. Scheduled fuzz history, parity beyond the bounded projection, pre-parser authority controls, a real plan-only seam, sealed isolated plan execution, independent content authority, retained-content transfer, isolated later reads, helper packaging, writer quiescence, stage audit, permission-mutation control, and supervisor-owned publication remain separate gates.
...
1. Prove a nonsemantic Linux authority bootstrap first. **Repository conformance slice landed:** before exec, a same-binary lab child marks unrelated inherited descriptors close-on-exec; at child entry it retains the sequenced-packet control channel plus inert `/dev/null` output streams and repeats closure through `close_range`. It then receives and validates optional private-stage authority, hard-requires `no_new_privs` plus a fixed Landlock ABI 3 policy, and receives the read-only synthetic source only after restriction readiness. The parent independently observes the exact descriptor roles and identities before and after source transfer, drives outside-denial and stage-local probes, authorizes exit, and uses a pidfd-backed termination and reap deadline before checked cleanup. It interprets no archive and changes no public or shipped execution path.
...
3. Follow the [accepted private experiment and provisional public hybrid decision](docs/decisions/0001-alpha6-semantic-ownership.md): prototype private split-phase planning and completion records, then separately resolve retained-content transfer, isolated non-retained reads, lifecycle, and helper packaging before operation protocol v2. Preserve the current IR-on-destination-setup-failure behavior and the single-interpretation property. **Dormant in-process inspect and peak-live slices landed:** an accepted Ready plan owns its revalidated snapshot, permits neither materialization nor retention, reads only planned payload ranges, and emits the canonical completion without structural reparse. A required isolated-child probe measures completion reconstruction near the record cap without claiming planning, transport, worker, or RSS bounds. This is not isolated execution or an independently checkable certificate, and a library call must not unexpectedly sandbox its caller.
...
6. Prevent worker descendants from retaining writable stage authority. A minimal measured control may deny process and thread creation before the first archive read; a broader syscall allowlist still waits for the real measured surface.
...
Why this follows the snapshot slice: the Windows DACL removes inherited DACL grants to other principals but does not reduce a compromised parser's ambient authority. Windows descendants still receive the creating token's default owner, and a principal matching that SID remains outside this milestone. Protocol v1 binds retained snapshot and stage capability slots to a bounded reduced manifest, but it carries neither a complete `ArchiveIR` nor independent public outcome axes. A repository-only inspect lab may cross a process boundary once the exact plan seam and pre-parser controls close; the bridge itself must prove sealed transport and lifecycle. It cannot activate runtime or public behavior. Runtime activation still requires retained-content authority, isolated later reads, and helper packaging. Landlock confines pathname operations by the worker, but it does not revoke pre-opened descriptors or stop a forked descendant from retaining the writable stage. Descriptor closure and writer quiescence are therefore separate gates. Same-principal containment requires a distinct service identity or equivalent mandatory-access-control boundary and remains outside this milestone.
