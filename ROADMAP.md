...
Do not add TAR, 7z, MCP, language bindings, accelerators, extra ZIP methods, a desktop CLI, or archive-decision signing first. Those are not substitutes for a dependent. The current implementation has a strict but incomplete ZIP32 parser, an executable ZipDiff gate, component-bound output, a private snapshot, a shared owning plan seam, and a dormant split-phase semantic contract with plan-native inspect and materialize executors. Near-limit completion heap measurement, pre-parser no-descendant controls, raw ancillary validation, authority-epoch absolute deadlines, bounded kernel-sealed handoffs, repository-only Store-and-Deflate execution, supervisor-owned exact-byte replay, immutable inspect and materialize retained-content transfer, one-shot isolated non-retained reads, clean writer reap, exact stage audit, supervisor-only no-replace publication, an authenticated child-only helper, fixed Linux helper packaging, a self-bound generic semantic adapter, shared authenticated transport primitives, and separate 500-iteration bootstrap and writer campaigns have landed as private evidence. The helper now consumes the actual sealed-plan semantics and preserves stopped archive outcomes, while the library and helper use one frame, descriptor, sealed-blob, and helper-authentication implementation. The next work is an explicit fail-closed public supervisor path that maps source-authorized completion into the existing outcome axes and installs a hidden worker-backed `VerifiedArchive` backend. Real-kernel setup-failure evidence completes the reduced-authority release gate. Canonical Unicode paths and a real consuming boundary follow. Broader semantic parity continues as an assurance lane driven by observed gaps. Layered adversarial assurance continues with every increment.
...
- a production-compiled, crate-private owning planner shared by ordinary `apply()` and the in-crate conformance harness, plus a dormant split-phase semantic-record experiment with independent magic, bounded hostile decode, exact supervisor correlation and source binding, input-sized fallible validation allocations, single-materialization completion reconstruction, a required near-limit isolated-child heap probe, moved findings, pinned record vectors, an immutable 12-case v1 shadow baseline plus 12 additive v2 cases with explicit oracle ownership, a source-owning inspect executor, immutable original-pass retained-content transfer, a self-bound generic inspect and materialize worker adapter, source-authorized stopped completions, and a fresh restricted worker for each caller-bounded non-retained read; ordinary `apply()` never traverses the record or worker paths, and a dedicated deterministic-seed target has clean exact-main Linux AddressSanitizer evidence.
