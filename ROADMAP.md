...
- a production-compiled, crate-private owning planner shared by ordinary `apply()` and the in-crate conformance harness, plus a dormant split-phase semantic-record experiment with independent magic, bounded hostile decode, exact supervisor correlation and source binding, input-sized fallible validation allocations, single-materialization completion reconstruction, a required near-limit isolated-child heap probe, moved findings, pinned record vectors, an immutable 12-case v1 shadow baseline plus 12 additive v2 cases with explicit oracle ownership, and a source-owning inspect executor that consumes accepted Ready plans through shared bounded payload verification without structural reparse; ordinary `apply()` never traverses the record codec, and a dedicated deterministic-seed target has clean exact-main on-demand Linux AddressSanitizer evidence while scheduled-event history remains pending.
...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, applies a fixed Landlock floor and x86_64 seccomp no-descendant and permission-mutation controls before source transfer, validates every raw ancillary header while immediately owning received descriptors, enforces supervisor-owned absolute monotonic deadlines across all four authority rounds, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. Eleven deterministic stalls cover changing authority from pre-bootstrap receive through post-ack exit. A required 500-iteration Linux campaign cycles the complete 44-case non-stall matrix while proving no child survives and no supervisor descriptor count grows. Bounded kernel-sealed synthetic plan and completion payloads now prove required memfd seals, exact length, independently recomputed SHA-256, operation and source binding, plan and result binding, live descriptor identity, malformed-plan rejection, and lifecycle behavior. The shared crate-private planner moves the exact snapshot, pending IR, planning findings, profile, policy identity, and compiled controls into ordinary `apply()` or the in-crate conformance harness without cloning or record round-trips. The [private semantic-record](docs/semantic-record.md) slice has exact invocation and source binding, hostile bounded decode, a plan-native Store and Deflate executor without structural reparse, a required three-platform near-limit completion probe, deterministic fuzz infrastructure, and 24 named v1 plus v2 shadow observations with explicit v2 oracle ownership. This is still a dormant bounded projection, and a correlated completion remains an untrusted proposal that can carry an arbitrary file-content SHA-256. The next dependency is replacing the synthetic payloads with a validated semantic plan and completion in the repository-only inspect bridge. Scheduled history, independent content authority, retained-content transfer, isolated later reads, helper packaging, writer quiescence, stage audit, and supervisor-owned publication remain separate gates.
...
Why this follows the snapshot slice: the Windows DACL removes inherited DACL grants to other principals but does not reduce a compromised parser's ambient authority. Windows descendants still receive the creating token's default owner, and a principal matching that SID remains outside this milestone. Protocol v1 binds retained snapshot and stage capability slots to a bounded reduced manifest, but it carries neither a complete `ArchiveIR` nor independent public outcome axes. The exact in-process plan seam, raw ancillary gate, authority-epoch deadlines, repeated bootstrap stress, and sealed synthetic-blob lifecycle have landed. The next repository-only inspect lab must carry the real validated semantic plan and completion through that boundary without structural reparse. It cannot activate runtime or public behavior. Runtime activation still requires retained-content authority, isolated later reads, and helper packaging. Landlock confines pathname operations by the worker but does not revoke pre-opened descriptors. The bootstrap seccomp deny set prevents a new process or thread from retaining stage authority; descriptor closure, worker exit, reap, and writer quiescence remain separate gates. Same-principal containment requires a distinct service identity or equivalent mandatory-access-control boundary and remains outside this milestone.
