...
| gzip | Internal bounded transform and fuzz infrastructure; no public format selection yet |
...
The latest published milestone is **Alpha.9: portable ustar and the multi-format core**. Current main adds strict ZIP64 as an explicit in-process preview under policy v3 while retaining the project-wide alpha status.
...
9. **Stable distribution mechanics: executable before the stable claim.** Only the `sealr` crate is crates.io-allowlisted; exact package files, README, license, MSRV, registry, extracted consumer, and public API fixture are required checks. Native workflows use exact Ubuntu 24.04, macOS 15 arm64, and Windows Server 2022 runners and assert the architecture, ABI, kernel or deployment contract before testing and packaging. The [distribution contract](docs/distribution-contract.md) keeps source and native promises separate. The remaining 1.0 blockers are trust-gate completion, supported consumer promotion, stable assurance history, API and schema freeze, and independent review.

Raw portable ustar, format-native archive and member evidence, explicit retained payload plans, one non-cloneable `ReadyArchive` execution authority, domain-zero raw snapshots, the bounded private derived-snapshot constructor, and the explicit strict ZIP64 in-process path are landed. The immediate sequence is: finish ZIP64 assurance and semantic-record v3 worker parity; promote gzip-wrapped ustar from internal transform and fuzz infrastructure; add PAX and selected GNU dialects; then promote further wrappers and the 7z, cpio, ar/deb, CAB, RPM, and RAR5 programs only through measured gates. ISO 9660, UDF, and filesystem images remain separate. Targeted wheel evidence, authenticated recovery, durability, stable identities, and avoided-work performance continue in parallel.
