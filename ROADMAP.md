...
| gzip-wrapped portable ustar | Implemented on current main as an explicit policy v4 in-process preview; reuses existing `flate2` with zero new dependencies |
...
The latest published milestone is **Alpha.9: portable ustar and the multi-format core**. Current main adds strict ZIP64 under policy v3 and strict gzip-wrapped portable ustar under policy v4 as explicit in-process previews while retaining the project-wide alpha status.
...
Format breadth is an explicit second lane, not a substitute for a dependent. Alpha.9 proved the container-neutral core with portable raw ustar and no new runtime dependency. Current main proves the original-snapshot structural path with explicit ZIP64 and the exact two-domain transform path with gzip-wrapped portable ustar, also without a new dependency. Next come raw PAX and a narrow GNU long-name profile, followed by gzip composition and separately promoted zstd, XZ, and bzip2 wrappers. The 7z, cpio, ar/deb, CAB, RPM, and RAR5 programs remain separately gated. ISO 9660, UDF, and other filesystem images belong to a separate program. Targeted wheel feature coverage, stable identity and API review, authenticated recovery, durability, and scheduled history continue because they measure the first real supported consumer.
...
8. **Strict ZIP64 public integration: implemented on current main.** `Zip64StrictAsciiV1` is an explicit in-process selection authorized by policy v3, with ZIP64-native IR, covering, and `sealrTreeV3` evidence. `apply()` and the `zip` CLI selection remain ZIP32 and never alias to it. The authenticated worker refuses ZIP64 without fallback until semantic-record v3 can represent the new evidence.
9. **Strict gzip-wrapped portable ustar: implemented on current main.** `TarGzipInterpretationProfile::UstarPortableV1` is an explicit in-process selection authorized by policy v4. It retains the original gzip and one bounded decoded TAR as separate immutable domains, requires one exact full-source transform, applies independent wrapper, TAR, and composite audits before staging, and publishes `sealrTreeV4` layout evidence while preserving the format-neutral content root. It reuses `flate2` and adds no runtime dependency. The authenticated worker refuses the profile without fallback until a later semantic record can bind both domains.
10. **Stable distribution mechanics: executable before the stable claim.** Only the `sealr` crate is crates.io-allowlisted; exact package files, README, license, MSRV, registry, extracted consumer, and public API fixture are required checks. Native workflows use exact Ubuntu 24.04, macOS 15 arm64, and Windows Server 2022 runners and assert the architecture, ABI, kernel or deployment contract before testing and packaging. The [distribution contract](docs/distribution-contract.md) keeps source and native promises separate. The remaining 1.0 blockers are trust-gate completion, supported consumer promotion, stable assurance history, API and schema freeze, and independent review.

Raw portable ustar, strict gzip-wrapped portable ustar, format-native archive and member evidence, domain-qualified retained payload plans, one validated `VerifiedArchive` construction authority, immutable original and derived snapshots, exact transform graphs, and the explicit strict ZIP64 in-process path are landed. The immediate sequence is: close ZIP64 and gzip-TAR assurance and worker-record gaps; add raw PAX and a narrow GNU long-name profile; compose both with gzip; then promote zstd, XZ, and bzip2 wrappers before building 7z on the reviewed LZMA layer. cpio, ar/deb, CAB, RPM, and RAR5 follow only through measured gates. ISO 9660, UDF, and filesystem images remain separate. Targeted wheel evidence, authenticated recovery, durability, stable identities, and avoided-work performance continue in parallel.
