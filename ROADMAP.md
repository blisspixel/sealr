...
The [usefulness test](docs/usefulness.md) is the product gate: same bytes and policy produce one tree or no tree on Linux, macOS, and Windows, and the next tool consumes that admitted tree instead of opening the ZIP again. The repository wheel evaluator, installer conformance kit, and [copyable packaged handoff](crates/sealr/examples/pypa_installer_handoff/README.md) now prove that mechanism locally. The remaining product exit is a separately maintained consumer that treats the capability and independently verified evidence as authoritative. CI protects the boundary through the corpus, lockfile, cargo-deny, native materialization, packaged handoff, and exact integration fixtures.
