...
- a repository-only Linux authority-bootstrap lab with sequenced-packet descriptor transfer, pre-exec closure plus independently probed child-entry descriptor closure, a runtime-queried Landlock ABI 3 floor before source transfer, process-boundary malformed-message evidence, bounded lifecycle failures, pidfd termination and reap, and checked cleanup;
- a dormant crate-private split-phase semantic-record experiment with independent magic, a 64 MiB hard limit, exact request and plan correlation, complete pending structural IR, a plan-ordered completion vector, fallible hostile decode, and pinned record vectors;
...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, installs `no_new_privs` and a fixed Landlock ABI 3 policy before source transfer, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. The first [private semantic-record](docs/semantic-record.md) slice now makes the split-phase decision executable with exact invocation binding, supervisor-snapshot covering reproduction, parser-equivalent metadata accounting, source-ordered pending IR, a plan-digest-bound completion vector, hostile decoding, and covered setup-failure parity. It remains dormant and does not define protocol v2, construct `VerifiedArchive`, or activate a worker. Retained-content authority, isolated later reads, helper packaging, writer quiescence, stage audit, permission-mutation control, and supervisor-owned publication remain separate gates.
