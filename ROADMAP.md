...
- a repository-only Linux authority-bootstrap lab with a private fixed-size protocol, sequenced-packet `SCM_RIGHTS` transfer, `close_range` descriptor closure, fixed Landlock ABI 3 enforcement before source transfer, exact parent-observed descriptor evidence, pidfd termination, and fail-closed native conformance cases;
...
3. Resolve the public-capability model before operation protocol v2: either return a complete bounded IR or certificate that preserves the local verified capability, or define an explicit worker-backed session with bounded reads, retention, lifetime, cancellation, and crash behavior. A library call must not unexpectedly sandbox its caller.
