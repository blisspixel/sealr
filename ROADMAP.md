...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, applies a fixed Landlock floor before source transfer, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. The shared crate-private planner now moves the exact snapshot, pending IR, planning findings, profile, policy identity, and compiled controls into ordinary `apply()` or the in-crate conformance harness without cloning or record round-trips. The [private semantic-record](docs/semantic-record.md) slice has exact invocation and source binding, hostile bounded decode, a plan-native Store and Deflate executor without structural reparse, a required three-platform near-limit completion probe, deterministic fuzz infrastructure, and 24 named v1 plus v2 shadow observations with explicit v2 oracle ownership. This is still a dormant bounded projection, and a correlated completion remains an untrusted proposal that can carry an arbitrary file-content SHA-256. The next dependency is remaining pre-parser authority closure, followed by a sealed repository-only inspect bridge. Scheduled history, independent content authority, retained-content transfer, isolated later reads, helper packaging, writer quiescence, stage audit, permission-mutation control, and supervisor-owned publication remain separate gates.
...
Why this follows the snapshot slice: the Windows DACL removes inherited DACL grants to other principals but does not reduce a compromised parser's ambient authority. Windows descendants still receive the creating token's default owner, and a principal matching that SID remains outside this milestone. Protocol v1 binds retained snapshot and stage capability slots to a bounded reduced manifest, but it carries neither a complete `ArchiveIR` nor independent public outcome axes. The exact in-process plan seam has landed. A repository-only inspect lab may cross a process boundary only after the pre-parser controls close, and the bridge itself must prove sealed transport and lifecycle. It cannot activate runtime or public behavior. Runtime activation still requires retained-content authority, isolated later reads, and helper packaging. Landlock confines pathname operations by the worker, but it does not revoke pre-opened descriptors or stop a forked descendant from retaining the writable stage. Descriptor closure and writer quiescence are therefore separate gates. Same-principal containment requires a distinct service identity or equivalent mandatory-access-control boundary and remains outside this milestone.
