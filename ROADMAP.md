...
The latest published milestone is **Alpha.10: strict ZIP64 and gzip TAR**. It adds strict ZIP64 under policy v3 and strict gzip-wrapped portable ustar under policy v4 as explicit in-process previews while retaining the project-wide alpha status.
...
Format breadth is an explicit second lane, not a substitute for a dependent. Alpha.9 proved the container-neutral core with portable raw ustar and no new runtime dependency. Alpha.10 proves the original-snapshot structural path with explicit ZIP64 and the exact two-domain transform path with gzip-wrapped portable ustar, also without a new dependency. Next come raw PAX and a narrow GNU long-name profile, followed by gzip composition and separately promoted zstd, XZ, and bzip2 wrappers. The 7z, cpio, ar/deb, CAB, RPM, and RAR5 programs remain separately gated. ISO 9660, UDF, and other filesystem images belong to a separate program. Targeted wheel feature coverage, stable identity and API review, authenticated recovery, durability, and scheduled history continue because they measure the first real supported consumer.
