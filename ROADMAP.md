...
The latest published milestone is **Alpha.12: canonical evidence and independent verification**. It ships exact RFC 8785 decision evidence, a no-Sealr-dependency verifier in every native archive, an adopter-ready packaged PyPA path, and seven separately selected restricted format additions while retaining the project-wide alpha status.
...
The [usefulness test](docs/usefulness.md) is the product gate: same bytes and policy produce one tree or no tree on Linux, macOS, and Windows, and the next tool consumes that admitted tree instead of opening the ZIP again. The repository wheel evaluator and installer conformance kit now prove that mechanism locally. The remaining product exit is a separately maintained consumer that treats the capability and independently verified evidence as authoritative. CI protects the boundary through the corpus, lockfile, cargo-deny, native materialization, packaged handoff, and exact integration fixtures.
...
15. **Packaged downstream handoff: next.** Ship a compact copyable `WheelSource` example that depends only on the public crate and exact PyPA installer dependency. Prove the example against the extracted crate and packaged helper while keeping exhaustive fixtures and oracles in repository tests.
16. **Poetry 2.4.2 repository fixture: after handoff.** Exercise the packaged handoff through one exact isolated Poetry integration. Preparation must finish after Poetry's lock-hash validation but before update uninstall or any destination effect. The fixture must preserve Poetry's existing checks, remove the source before Python effects, deny every `.whl` open, and retain the explicit repository-owned nonclaim.
17. **External usefulness exit.** Seek independent adoption only after the preceding repository contract is copyable and complete. A separately maintained publisher, registry, build backend, or installer must treat the capability and independently verified evidence as authoritative.

Raw portable ustar, restricted raw PAX, the narrow GNU long-name-only raw profile, strict gzip-wrapped portable ustar, the gzip-wrapped PAX and GNU compositions, format-native archive and member evidence, domain-qualified retained payload plans, one validated `VerifiedArchive` construction authority, immutable original and derived snapshots, exact transform graphs, and the explicit strict ZIP64 in-process path are landed. The zstd wrapper is promoted through the executed ruzstd Gate B review under policy v8, the xz wrapper through the executed lzma-rust2 Gate B review under policy v9, the bzip2 wrapper through the executed bzip2/libbz2-rs-sys Gate B review under policy v10, and the Copy-first 7z structure through the executed Gate C first step under policy v11. Native evidence-verifier delivery and supervised wheel prefix parity close the remaining repository-owned binary and evaluator-composition gaps. The immediate sequence is the packaged `WheelSource` handoff, exact Poetry 2.4.2 repository fixture, external usefulness exit, measured compatibility, and independent review in the [near-term execution plan](docs/near-term.md). The parked 7z LZMA/LZMA2 member and packed-header step over that structure with the reviewed lzma-rust2 layer resumes afterward. cpio, ar/deb, CAB, RPM, and RAR5 follow only through measured gates. ISO 9660, UDF, and filesystem images remain separate. ZIP64 and TAR worker-record parity, targeted wheel evidence, authenticated recovery, durability, stable identities, and avoided-work performance continue in parallel.
...
- **Adopter conformance landed:** an isolated project compiles only against Cargo's extracted `sealr` package and drives exact PyPA installer 1.0.1 through the public portable profile, wheel evaluator, and realization identity;
- **Adopter conformance landed:** canonical evidence is independently verified, the original wheel is deleted before Python starts, bounded member blobs come only from `VerifiedArchive`, every `.whl` open is denied, and both a controlled fixture and the real hash-pinned installer wheel complete through fresh filesystem roots;
- **Required denial gates landed:** lying `RECORD`, traversal, descriptor tampering, and staged-member tampering fail before installation effects, while exact controlled and real-distribution identity pins detect drift;
- **Nonclaim:** this is an adopter-ready repository contract for one Linux POSIX target model, not proof that a separately maintained project has adopted Sealr;
