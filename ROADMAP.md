...
| bzip2-wrapped portable ustar | Implemented on current main as an explicit policy v10 in-process preview; the third Gate B codec promotion adds exactly `bzip2` and `libbz2-rs-sys` |
| Copy-only 7z container | Implemented on current main as an explicit policy v11 in-process preview; the first Gate C structure step adds zero dependencies and proves cross-container content parity |
| xz-wrapped portable ustar | Implemented on current main as an explicit policy v9 in-process preview; the second Gate B codec promotion adds exactly `lzma-rust2` |
