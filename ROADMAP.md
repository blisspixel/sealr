...
1. **Alpha.4 target: measured semantic contract.** The first opaque verified capability, caller-bounded member reads, packaged consumer, two finite-domain property families, and independent identity-conformance verifier have landed. Finish the exact extra-field profile from compatibility evidence, bounded semantic-member retention, remaining pure property families, and first wheel compatibility report. The compatibility `apply()` facade remains available while the adopter-facing API is reviewed.
