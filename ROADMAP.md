...
3. **Alpha.6 target: supervised Linux worker.** The first repository-only, nonsemantic authority-bootstrap slice is implemented: it closes inherited authority, installs `no_new_privs` and a fixed Landlock ABI 3 policy before the child receives a source descriptor, reports readiness, runs bounded synthetic probes, exits, and is reaped without interpreting an archive. The [semantic-ownership decision](docs/decisions/0001-alpha6-semantic-ownership.md) accepts a private split-phase semantic-record experiment while keeping retained-content transfer, isolated non-retained reads, effect ownership, and helper packaging separate. It does not define protocol v2 or a public worker mode. Supervised execution follows only after writer quiescence, stage audit, permission-mutation control, and supervisor-owned publication are specified together.
