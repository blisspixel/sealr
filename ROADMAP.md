...
Do not add TAR, 7z, MCP, language bindings, accelerators, extra ZIP methods, a desktop CLI, or archive-decision signing first. Those are not substitutes for a dependent. The current implementation has a strict but incomplete ZIP32 parser, an executable ZipDiff gate, component-bound output, a private snapshot, a shared owning plan seam, and a dormant split-phase semantic contract with a plan-native inspect executor. Near-limit completion heap measurement, pre-parser no-descendant controls, raw ancillary validation, authority-epoch absolute deadlines, 500 repeated native worker iterations, bounded kernel-sealed handoffs, repository-only isolated Store-and-Deflate plan consumption, supervisor-owned exact-byte replay, immutable original-pass retained-content transfer, and one-shot isolated non-retained reads have landed. The next work is the materializing-writer lifecycle: prove writer quiescence, audit the exact stage after reap, and keep no-replace publication supervisor-owned. Helper packaging follows that executable lifecycle. Canonical Unicode paths and a real consuming boundary follow the reduced-authority gate. Broader semantic parity continues as an assurance lane driven by observed gaps. Layered adversarial assurance continues with every increment.
