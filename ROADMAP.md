...
- a dormant crate-private split-phase semantic-record experiment with independent magic, a 64 MiB encoded-length limit checked before buffer-growth requests and copies, exact request and plan correlation, source-bound structural and represented fixed ZIP fields, phase-specific finding validation, supervisor-bound path-depth allocation limits, complete pending structural IR, a plan-ordered completion vector, fallible hostile decode, pinned record vectors, and a dedicated deterministic-seed fuzz target with job-scoped manifest verification whose first scheduled Linux AddressSanitizer campaign remains pending.
...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, installs `no_new_privs` and a fixed Landlock ABI 3 policy before source transfer, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. The [private semantic-record](docs/semantic-record.md) slice makes the split-phase decision executable with exact invocation binding, source-backed structural and fixed-field checks, parser-equivalent metadata accounting, phase-specific finding validation, supervisor-bound path-depth allocation limits, source-ordered pending IR, a plan-digest-bound completion vector, hostile decoding, covered setup-failure parity, and a dedicated deterministic-seed fuzz target with parsed Cargo-target and complete scheduled-workflow verification. It remains dormant, and the new target's first scheduled Linux AddressSanitizer campaign is still pending. Broader shadow parity, validated-plan execution without structural reparse, retained-content authority, isolated later reads, sealed blob transport, helper packaging, writer quiescence, stage audit, permission-mutation control, and supervisor-owned publication remain separate gates.
