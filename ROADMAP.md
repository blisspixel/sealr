...
Do not add TAR, 7z, MCP, language bindings, accelerators, extra ZIP methods, a desktop CLI, or archive-decision signing first. Those are not substitutes for a dependent. The current implementation has a strict but incomplete ZIP32 parser, an executable ZipDiff gate, component-bound output, a private snapshot, a shared owning plan seam, and a dormant split-phase semantic contract with a plan-native inspect executor. Near-limit completion heap measurement, pre-parser no-descendant controls, raw ancillary validation, authority-epoch absolute deadlines, 500 repeated native worker iterations, bounded kernel-sealed handoffs, repository-only isolated Store-and-Deflate plan consumption, supervisor-owned exact-byte replay, and immutable original-pass retained-content transfer have landed. The next work is an isolated caller-bounded backend for non-retained reads and the remaining worker content gates, followed by canonical Unicode paths and a real consuming boundary. Broader semantic parity continues as an assurance lane driven by observed gaps. Layered adversarial assurance continues with every increment.
...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, applies a fixed Landlock floor and x86_64 seccomp no-descendant and permission-mutation controls before source transfer, validates every raw ancillary header while immediately owning received descriptors, enforces supervisor-owned absolute monotonic deadlines across all four authority rounds, exercises bounded lifecycle failures, and proves reap before checked cleanup. Eleven deterministic stalls cover changing authority from pre-bootstrap receive through post-ack exit. A required 500-iteration Linux campaign cycles the complete 44-case non-stall matrix while proving no child survives and no supervisor descriptor count grows. Bounded kernel-sealed plan, completion, and retained-content records prove required memfd seals, exact length, independently recomputed SHA-256, operation and source binding, live descriptor identity, malformed-plan rejection, and lifecycle behavior. The shared crate-private planner moves the exact snapshot, pending IR, planning findings, profile, policy identity, and compiled controls into ordinary `apply()` or the repository lab without cloning or record round-trips. The [private semantic-record](docs/semantic-record.md) slice has exact invocation and source binding, hostile bounded decode, a plan-native Store and Deflate executor without structural reparse, immutable original-pass retention, a required three-platform near-limit completion probe, deterministic fuzz infrastructure, and 24 named v1 plus v2 shadow observations with explicit v2 oracle ownership. A feature-gated repository bridge sends the canonical plan to the restricted worker, validates it against the exact file-backed snapshot, executes both Store and Deflate payload ranges without structural reparse, and returns sealed canonical completion and retained-content bundles. The supervisor retains lifecycle authority, classifies both as untrusted proposals, then replays the accepted plan against its retained exact source after worker reap and requires byte-for-byte canonical agreement. A forged canonical digest regression proves the authority boundary. Scheduled history, isolated later reads, helper packaging, writer quiescence, stage audit, and supervisor-owned publication remain separate gates.
