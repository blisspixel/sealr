...
Do not add TAR, 7z, MCP, language bindings, accelerators, extra ZIP methods, a desktop CLI, or archive-decision signing first. Those are not substitutes for a dependent. The current implementation has a strict but incomplete ZIP32 parser, an executable ZipDiff gate, component-bound output, a private snapshot, a shared owning plan seam, and a dormant split-phase semantic contract with a plan-native inspect executor. Near-limit completion heap measurement, pre-parser no-descendant controls, raw ancillary validation, authority-epoch absolute deadlines, 500 repeated native worker iterations, bounded kernel-sealed handoffs, repository-only isolated Store-and-Deflate plan consumption, and supervisor-owned exact-byte replay against the retained source have landed. The next work is immutable retained-content transfer and the remaining worker content gates, followed by canonical Unicode paths and a real consuming boundary. Broader semantic parity continues as an assurance lane driven by observed gaps. Layered adversarial assurance continues with every increment.
