...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, applies a fixed Landlock floor and x86_64 seccomp no-descendant and permission-mutation controls before source transfer, validates every raw ancillary header while immediately owning received descriptors, enforces supervisor-owned absolute monotonic deadlines across all four authority rounds, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. Nine deterministic stalls cover changing authority from pre-bootstrap receive through post-ack exit. A required 500-iteration Linux campaign cycles the complete 36-case non-stall matrix while proving no child survives and no supervisor descriptor count grows. The shared crate-private planner now moves the exact snapshot, pending IR, planning findings, profile, policy identity, and compiled controls into ordinary `apply()` or the in-crate conformance harness without cloning or record round-trips. The [private semantic-record](docs/semantic-record.md) slice has exact invocation and source binding, hostile bounded decode, a plan-native Store and Deflate executor without structural reparse, a required three-platform near-limit completion probe, deterministic fuzz infrastructure, and 24 named v1 plus v2 shadow observations with explicit v2 oracle ownership. This is still a dormant bounded projection, and a correlated completion remains an untrusted proposal that can carry an arbitrary file-content SHA-256. The next dependency is a sealed repository-only inspect bridge. Scheduled history, independent content authority, retained-content transfer, isolated later reads, helper packaging, writer quiescence, stage audit, and supervisor-owned publication remain separate gates.
