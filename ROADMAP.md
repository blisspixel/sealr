...
Do not add TAR, 7z, MCP, language bindings, accelerators, extra ZIP methods, a desktop CLI, or archive-decision signing first. Those are not substitutes for a dependent. The current implementation has a strict but incomplete ZIP32 parser, an executable ZipDiff gate, component-bound output, a private snapshot, a shared owning plan seam, and a dormant split-phase semantic contract with an in-process plan-native inspect executor. Near-limit completion heap measurement, pre-parser no-descendant controls, and raw ancillary validation have landed. The next work is authority-epoch deadline closure and repeated native stress, then sealed isolated plan consumption and the remaining worker content gates, followed by canonical Unicode paths and a real consuming boundary. Broader semantic parity continues as an assurance lane driven by observed gaps. Layered adversarial assurance continues with every increment.
...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, applies a fixed Landlock floor and x86_64 seccomp no-descendant and permission-mutation controls before source transfer, validates every raw ancillary header while immediately owning received descriptors, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. The shared crate-private planner now moves the exact snapshot, pending IR, planning findings, profile, policy identity, and compiled controls into ordinary `apply()` or the in-crate conformance harness without cloning or record round-trips. The [private semantic-record](docs/semantic-record.md) slice has exact invocation and source binding, hostile bounded decode, a plan-native Store and Deflate executor without structural reparse, a required three-platform near-limit completion probe, deterministic fuzz infrastructure, and 24 named v1 plus v2 shadow observations with explicit v2 oracle ownership. This is still a dormant bounded projection, and a correlated completion remains an untrusted proposal that can carry an arbitrary file-content SHA-256. The next dependencies are authority-epoch absolute deadlines and repeated native stress, followed by a sealed repository-only inspect bridge. Scheduled history, independent content authority, retained-content transfer, isolated later reads, helper packaging, writer quiescence, stage audit, and supervisor-owned publication remain separate gates.
...
Why this follows the snapshot slice: the Windows DACL removes inherited DACL grants to other principals but does not reduce a compromised parser's ambient authority. Windows descendants still receive the creating token's default owner, and a principal matching that SID remains outside this milestone. Protocol v1 binds retained snapshot and stage capability slots to a bounded reduced manifest, but it carries neither a complete `ArchiveIR` nor independent public outcome axes. The exact in-process plan seam and raw ancillary gate have landed. A repository-only inspect lab may cross a process boundary only after the remaining deadline and stress gates close, and the bridge itself must prove sealed transport and lifecycle. It cannot activate runtime or public behavior. Runtime activation still requires retained-content authority, isolated later reads, and helper packaging. Landlock confines pathname operations by the worker but does not revoke pre-opened descriptors. The bootstrap seccomp deny set prevents a new process or thread from retaining stage authority; descriptor closure, worker exit, reap, and writer quiescence remain separate gates. Same-principal containment requires a distinct service identity or equivalent mandatory-access-control boundary and remains outside this milestone.
