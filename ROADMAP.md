...
3. **Alpha.6 target: supervised Linux worker.** The repository-only authority bootstrap closes inherited authority, applies a fixed Landlock floor before source transfer, exercises bounded lifecycle failures, and proves reap before checked cleanup without interpreting an archive. The [private semantic-record](docs/semantic-record.md) slice now has exact invocation and source binding, hostile bounded decode, a plan-native Store and Deflate executor without structural reparse, a required three-platform near-limit completion probe, deterministic fuzz infrastructure, and 24 named v1 plus v2 shadow observations with explicit v2 oracle ownership. This is still a dormant bounded projection, and a correlated completion remains an untrusted proposal that can carry an arbitrary file-content SHA-256. The next dependencies are the real crate-private plan seam and remaining pre-parser authority controls, followed by a sealed repository-only inspect bridge. Scheduled history, independent content authority, retained-content transfer, isolated later reads, helper packaging, writer quiescence, stage audit, permission-mutation control, and supervisor-owned publication remain separate gates.
