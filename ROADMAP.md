...
7. Use targeted mutation testing and coverage reports to find weak assertions and blind branches. Add compiler-accurate public API comparison against an exact packaged release baseline. Do not use a global coverage percentage as an assurance claim. **Scheduled discovery landed:** exact tool versions, source and function scope, API baseline, authenticated tool artifact, known-warning debt, timeouts, report retention, failure semantics, and nonclaims are machine checked. Mutation and coverage are explicitly non-promotable scores. API compatibility becomes promotable only after a clean release baseline removes its warning debt, resets history, and then completes ten clean scheduled `main` runs plus explicit review.
