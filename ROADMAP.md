...
1. **Alpha.4 target: measured semantic contract.** The first opaque verified capability, caller-bounded member reads, packaged consumer, and compression-ratio oracle have landed. Finish the exact extra-field profile from compatibility evidence, bounded semantic-member retention, independent identity verifier and vectors, and the remaining pure property families. The compatibility `apply()` facade remains available while the adopter-facing API is reviewed.
