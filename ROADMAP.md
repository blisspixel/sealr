...
Do not add TAR, 7z, MCP, language bindings, accelerators, extra ZIP methods, a desktop CLI, or archive-decision signing first. Those are not substitutes for a dependent. The current implementation has a strict but incomplete ZIP32 parser, an executable ZipDiff gate, component-bound output, a private snapshot, a shared owning plan seam, and a split-phase semantic contract with plan-native inspect and materialize executors. Near-limit completion heap measurement, pre-parser no-descendant controls, raw ancillary validation, authority-epoch absolute deadlines, bounded kernel-sealed handoffs, Store-and-Deflate execution, supervisor-owned exact-byte replay, immutable inspect and materialize retained-content transfer, one-shot isolated non-retained reads, clean writer reap, exact stage audit, supervisor-only no-replace publication, an authenticated child-only helper, fixed Linux helper packaging, a self-bound generic semantic adapter, shared authenticated transport primitives, separate 500-iteration bootstrap and writer campaigns, actual Landlock ABI 2 fail-closed evidence, manifest-backed CLI, wheel-laboratory and packaged-consumer activation, and the scheduled assurance spine have landed. The supported fail-closed supervisor maps source-authorized inspect and materialize completion into the existing outcome axes, installs a hidden worker-backed `VerifiedArchive` backend, and keeps destination publication authority outside the worker. Alpha.6 releases that boundary. A repository-only wheel profile, bounded evaluator, hostile corpus, distinct identities, and external non-reopening installer bridge now prove the consumer shape. Portable general-purpose canonical names and promotion of that research into a supported boundary are next because one public tree identity cannot depend on platform decoding. Broader semantic parity, authenticated recovery, durability, and scheduled history continue as assurance lanes. Layered adversarial assurance continues with every increment.
...
The released Alpha.6 boundary is backed by the following implementation and repository evidence:
...
Items 5 through 8 are executable now so distribution mechanics cannot become a last-minute stable-release assumption. Items 1 through 4 still block 1.0. In particular, prerelease package readiness does not freeze the public API or convert the repository-only wheel proof into supported admission.
...
- **Repository research landed:** an exact wheel-oriented UTF-8 ZIP profile with exhaustive flag and extra-field dispositions;
- **Repository research landed:** a `python-wheel.v1-research` consumer profile that binds the outer artifact filename and validates wheel metadata, `RECORD`, normalized topology, relocation destinations, and actual verified content;
- **Repository research landed:** separate source, archive-tree, wheel-artifact, scheme-relative install-plan, and target-realization identities;
- **Repository research landed:** bounded verified metadata access from the admitted archive without reopening the source or invoking a second ZIP parser;
- **Repository research landed:** minimized hostile fixtures and a predecessor-bound benign compatibility report;
