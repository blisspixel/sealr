...
3. **Alpha.6 target: supervised Linux worker.** Begin with a nonsemantic authority bootstrap that validates and closes descriptor authority, installs `no_new_privs` and measured Landlock before any source read, reports readiness, exits, and is reaped without interpreting an archive. Then settle how a worker preserves the complete `ArchiveIR` and `VerifiedArchive` contract before operation protocol v2 or public worker mode exists. Supervised execution follows only after writer quiescence, stage audit, and supervisor-owned publication are specified together.
...
1. Prove a nonsemantic Linux authority bootstrap first. A same-binary child receives only a bounded control channel, read-only snapshot, and optional stage descriptors; validates their count, role, type, access, length, and non-aliasing; closes unrelated descriptors; installs `no_new_privs` and measured Landlock before any source read; reports readiness; then exits and is reaped without interpreting an archive.
