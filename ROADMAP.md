...
- an unreleased supported `LinuxWorker` plus `inspect_supervised` library path that authenticates one exact helper artifact, reports isolation infrastructure failures separately from archive outcomes, constructs complete public outcome axes from source-authorized execution, and backs non-retained public capability reads with fresh restricted workers without fallback;
