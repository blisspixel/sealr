...
Raw portable ustar, format-native archive and member evidence, explicit retained payload plans, one non-cloneable `ReadyArchive` execution authority, domain-zero raw snapshots, and the bounded private derived-snapshot constructor are landed. The active architectural gate replaces stringly transformation records with typed profile, parameter, input-range, and output-domain identity plus atomic graph insertion. ZIP64 is the zero-new-dependency structural increment on the original snapshot path. gzip-wrapped TAR then reuses existing `flate2` through one validated derived snapshot. PAX and selected GNU dialects, zstd, xz/LZMA, bzip2, LZ4 frame, 7z, cpio, ar/deb, CAB, RPM, and separate RAR5 and RAR4 research gates follow in the measured order documented in [format support architecture](docs/format-support.md). Targeted wheel evidence, authenticated recovery, durability, stable identities, and avoided-work performance continue in parallel.
