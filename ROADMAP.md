...
This is the current main baseline: the released Alpha.4 contract plus unreleased Alpha.5 access, private-spool, source-change, and resource-evidence increments. It is incomplete and is not a production security boundary. Path inputs no longer retain the complete archive in heap memory; caller byte inputs remain memory-backed by definition. The bounded, fuzzed worker-protocol gate remains open, along with broader native stress and open-handle measurement.
