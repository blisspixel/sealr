...
- a dormant crate-private split-phase semantic-record experiment with independent magic, bounded hostile decode, exact supervisor correlation and source binding, input-sized fallible validation allocations, single-materialization completion reconstruction, moved findings, pinned record vectors, a versioned 12-case shadow matrix against the current in-process semantics, a dedicated deterministic-seed target, and clean exact-main on-demand Linux AddressSanitizer evidence; scheduled-event history remains pending.
