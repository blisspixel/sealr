...
CI omits `--allow-dirty` because it runs from a clean checkout. The fixture exercises explicit strict-v2 profile selection, the opaque verified capability, bounded member reads, stable error categories, and capability ownership. Its static ZIP is field-grouped so the exact container structure remains reviewable without a ZIP-building dependency.
