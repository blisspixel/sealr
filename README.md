...
> Current main additionally implements three unreleased zero-new-dependency previews: the restricted raw old-GNU long-name profile under policy v6 with `sealrTreeV6`, and the gzip-wrapped restricted PAX and GNU long-name compositions under policy v7 with `sealrTreeV7` and `sealrTreeV8`. Each composition wraps only its frozen raw dialect behind the exact Alpha.10 single-member gzip transform, publishes a distinct transform-bound layout identity while preserving the format-neutral content root, and is refused by the authenticated worker without fallback.
...
- zstd, XZ, bzip2, and LZ4 TAR wrappers, 7z, RAR4, RAR5, cpio, ar, deb, RPM, and CAB are not yet supported selections, and broader GNU TAR extensions (long links, sparse maps, base-256 numbers) remain denied. Raw ustar, gzip-wrapped ustar, restricted raw PAX, restricted raw GNU long-name, and the gzip-wrapped PAX and GNU compositions are public only through separate explicit profile selections. Restricted PAX is an in-process Alpha.11 preview under policy v5; the GNU long-name profile and both gzip compositions are unreleased in-process previews on current main under policies v6 and v7; none is general PAX or GNU support. The authenticated Linux worker accepts only the supported ZIP32 records and fails closed without fallback for every TAR and ZIP64 selection until later semantic records exist.
...
1. Promote zstd, XZ/LZMA2, and bzip2 through separate dependency, memory, checksum, exact-consumption, and work gates. Each codec must justify the smallest practical pure-Rust graph and must not introduce a general extractor framework. The GNU long-name raw profile and both gzip compositions are already implemented on current main, so codec promotion is the first remaining format increment.
