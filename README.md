...
The next Alpha.6 increment follows the [semantic-ownership decision](docs/decisions/0001-alpha6-semantic-ownership.md): first prototype private split-phase planning and completion records while keeping retained-content transfer, isolated non-retained reads, and helper packaging as separate activation gates. No public worker mode or protocol v2 exists yet. Before any stage audit, the supervisor must also prove that no worker or descendant retains writable stage authority; stage equality is not stable while a writer remains live.
