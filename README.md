...
> Status: `v0.1.0-alpha.10` is the tenth development preview of the archive boundary. It is useful for evaluation, development, and adversarial testing. It is not ready to protect a production host from arbitrary hostile archives. The limitations below are security boundaries, not fine print.

> Release contents: Alpha.10 adds explicitly selected zero-new-dependency strict ZIP64 and strict single-member gzip-wrapped portable ustar, policy v3 and v4, ZIP64-native and two-domain transform evidence, independent `sealrTreeV3` and `sealrTreeV4` verification, packaged-client coverage, and dedicated bounded fuzz campaigns. It preserves Alpha.9 raw ustar, every earlier ZIP and wheel identity, and the Alpha.6 supervised Linux boundary. The [Alpha.10 release notes](docs/releases/v0.1.0-alpha.10.md) define the shipped delta and remaining limitations.
...
Alpha.10 supports classic ZIP32 archives with stored or Deflate members, strict ZIP64 under policy v3, raw portable POSIX ustar under policy v2, and strict single-member gzip-wrapped portable ustar under policy v4. Every nondefault format is explicit and in process. `apply()` and the `zip` CLI selection remain ZIP32 and never alias to another profile.
...
- PAX and GNU TAR extensions, zstd, XZ, bzip2, and LZ4 TAR wrappers, 7z, RAR4, RAR5, cpio, ar, deb, RPM, and CAB are not yet supported selections. Raw and gzip-wrapped portable ustar regular files and directories are public only through separate explicit profile selections. Strict ZIP64 and gzip-wrapped ustar are in-process Alpha.10 previews under policy v3 and v4 respectively; the authenticated Linux worker accepts the supported ZIP32 records and fails closed without fallback for both newer profiles until a later semantic record exists.
...
Download the native preview archives, `SHA256SUMS`, and provenance from the [`v0.1.0-alpha.10` release](https://github.com/blisspixel/sealr/releases/tag/v0.1.0-alpha.10). Runnable checksum and provenance commands are in [release verification](docs/release-verification.md). To build from source:
...
| [Near-term execution plan](docs/near-term.md) | Released Alpha.10 baseline and the next measurable format and assurance gates |
...
| [API contract](docs/api.md) | Published Alpha.10 Rust and JSON surface |
