...
The next milestone remains the Phase 0.1 ZIP trust gate, not another archive format. Alpha.4 closed the measured semantic and adopter-facing contract, and Alpha.5 closed bounded immutable input. Alpha.6 now starts with a nonsemantic Linux authority bootstrap: transfer and validate only the control, snapshot, and optional stage descriptors, close unrelated authority, install `no_new_privs` and measured Landlock before any archive read, report readiness, then exit and reap without interpreting an archive. This proves the confinement ordering without forcing the reduced protocol v1 manifest into the public API.
