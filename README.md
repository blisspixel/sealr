...
- A dormant crate-private [split-phase semantic-record experiment](docs/semantic-record.md). Its canonical bounded planning and completion records bind the complete invocation plus represented fixed ZIP fields, descriptors, names, extra fields, and structural ranges back to the supervisor-owned snapshot. It separately replays forbidden-signature checks over comments and stored descriptor payloads and checks member-kind agreement with source external attributes. Phase-specific finding allowlists, parser-equivalent metadata accounting, supervisor-bound path-depth allocation limits, source-ordered structural IR, a full completion vector, and a dedicated fuzz target harden hostile decoding without changing an `apply`, CLI, or shipped runtime path or adding a supported API. Required verification binds the complete Cargo manifest, parsed target graph, hidden driver, lock checksum, and registry-rooted crates.io libFuzzer resolution in a Cargo-config-free evidence environment, then binds the complete scheduled workflow, including the weekly trigger, permissions, concurrency, shell programs, and failure-artifact contracts. Executable negative fixtures reject inert TOML remapping, local, patched, or vendored fuzz-engine substitution, manual-only drift, inactive or appended commands, inert evidence, and duplicate last-wins resource flags. The first scheduled Linux AddressSanitizer campaign for the semantic target remains pending.
...
- The private semantic-record experiment establishes parity only for its named deterministic record, source-binding, and codec cases. Its dedicated target is committed with deterministic seeds, but it has not yet completed its first scheduled Linux AddressSanitizer campaign. The experiment does not transfer retained bytes, execute a validated plan without structural reparse, provide isolated later member reads, construct `VerifiedArchive`, launch a worker, change `kernel_jail`, or prove that a worker proposal is the source's unique meaning.
...
The next Alpha.6 increment runs and expands the landed [private semantic-record fuzz surface](docs/semantic-record.md), broadens shadow parity, and proves that execution consumes the validated plan without a structural reparse. Retained-content transfer, isolated non-retained reads, sealed blob transport, and helper packaging remain separate activation gates. No public worker mode or protocol v2 exists yet. Before any stage audit, the supervisor must also prove that no worker or descendant retains writable stage authority; stage equality is not stable while a writer remains live.
...
| [Private semantic record](docs/semantic-record.md) | Dormant Alpha.6 planning/completion codec, validation rules, evidence, and remaining gates |
