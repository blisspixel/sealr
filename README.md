...
- **Not chasing format breadth.** Parser breadth is currently frozen. The active sequence is a copyable wheel handoff, an exact Poetry 2.4.2 repository fixture, independent adoption, measured compatibility, and review. See the [near-term execution plan](docs/near-term.md).
...
- An adversarial unit suite, an external-crate API fixture, and two separately locked consumers that run against the extracted packaged crate. One exercises supervised ZIP and every in-process format through Copy-only 7z. The other completes a real PyPA installation from bounded `VerifiedArchive` reads after source deletion. Strict Clippy, rustfmt, documentation checks, cross-platform tests, and cargo-deny policy run in CI.
...
- A required [packaged PyPA adopter conformance kit](tests/pypa-installer-consumer/README.md) for exact `installer` 1.0.1. It compiles only against Cargo's extracted crate, verifies canonical evidence independently, removes the source before Python starts, passes only verified member blobs, denies every `.whl` open, performs the real filesystem installation, and audits the resulting realization identity. This is repository mechanism evidence, not a claim that an independent project has adopted Sealr.
...
1. **Packaged wheel handoff.** Extract the current repository bridge into a compact, copyable public-API-only `WheelSource` example. It must accept only independently checked canonical evidence, an exact plan, and bounded verified member blobs, and it must never receive or reopen a wheel path.
2. **Poetry 2.4.2 repository fixture.** Pin the exact upstream artifact and dependency closure, prepare and admit the wheel before update uninstall or destination writes, consume the packaged handoff, remove the source before Python starts, deny every `.whl` open, and compare realized outputs with the Sealr plan and realization identity. This remains repository-owned mechanism evidence.
3. **External usefulness.** Seek one separately maintained publisher, registry, build backend, or installer that adopts the capability and independently verified evidence as authoritative only after the packaged contract is copyable and the repository fixture is complete.
