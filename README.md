...
- The Alpha.5 library and CLI do not use Landlock, seccomp, AppContainer, or another process sandbox. Their receipts report `kernel_jail: unavailable`. Current source adds an explicit Linux-only supervised request API whose successful inspect or materialize receipt reports `landlock-abi3+seccomp-v1`; macOS and Windows activation fails closed as isolation unavailable. The default `apply`, `apply_with_options`, and CLI remain in-process. The worker path confines payload verification, stage writes, and later non-retained reads, not structural planning, and it does not provide a general network, IPC, same-user, or production-containment claim.
