...
- The library now exposes an unreleased, explicit `LinuxWorker::load` plus `inspect_supervised` path. It requires the packaged helper's absolute path, exact length, and SHA-256; retains an authenticated sealed executable; and never searches `PATH` or silently falls back to in-process verification. Archive rejection remains an `Outcome`, while helper, restriction, protocol, timeout, exit, reap, source, and integrity-boundary failures are typed `SupervisionError` values. A complete result constructs the ordinary public outcome axes and a `VerifiedArchive` whose retained bytes stay local and whose non-retained reads each use a fresh restricted worker.
...
- The Alpha.5 library and CLI do not use Landlock, seccomp, AppContainer, or another process sandbox. Their receipts report `kernel_jail: unavailable`. Current source adds an explicit Linux-only supervised inspect API whose successful receipt reports `landlock-abi3+seccomp-v1`; macOS and Windows activation fails closed as isolation unavailable. The default `apply`, `apply_with_options`, and CLI remain in-process. The worker path confines payload verification and later non-retained reads, not structural planning, and it does not provide a general network, IPC, same-user, or production-containment claim.
...
The landed [private semantic-record assurance](docs/semantic-record.md) now includes an immutable 12-case v1 baseline, 12 additive v2 cases with explicit apply, backend, and supervisor-reproduction oracles, an in-process plan-native inspect executor, a shared owning plan seam, and a required near-limit completion heap probe. The Linux bootstrap has closed no-descendant and permission-mutation authority before source transfer, parses every raw ancillary header while closing all installed descriptors on rejection, and enforces supervisor-owned absolute monotonic deadlines across every authority round. Eleven deterministic stalls prove pidfd kill, reap, and cleanup ordering from pre-bootstrap receive through post-ack exit, including sealed-plan receipt and acceptance. A required 500-iteration Linux campaign repeats the complete non-stall case matrix while checking child reap, supervisor descriptor stability, retained source and outside-sentinel integrity, and checked cleanup after every iteration. Bounded `SLRBLOB1` memfds carry the canonical semantic plan, completion, and retained-content bundle for deterministic Store-and-Deflate inspect and materialize fixtures. The restricted worker binds the plan to the exact file-backed snapshot, invokes no structural parser during execution, reads only planned payload ranges, and captures selected members during the same verification pass that produces completion and, for materialization, the staged files. Its generic semantic adapter is self-bound to the actual sealed plan rather than a reconstructed fixture policy, profile, budget, or retention request. After worker exit and reap, the supervisor treats both sealed outputs as untrusted plan-bound proposals, independently replays the accepted plan against its retained exact source descriptor, and requires byte-for-byte canonical agreement. Replay reconstructs the exact private outcome axes and retention state for both complete and stopped verification. A separate private one-shot read boundary gives each caller-bounded non-retained read a fresh operation and restricted worker, transfers no stage or destination authority, and withholds bytes until exact output, correlated success, integrity, clean exit, and reap all agree. Tests cover clone serialization, pre-spawn, queued, and active cancellation, post-result crash isolation, next-call recovery, 64 alternating Store and Deflate reads, and last-owner cleanup. A private materializing path gives the restricted worker only the supervisor-created stage root and sealed plan, closes writer authority on exit, requires exact reap before validating completion and retained content, audits the exact stage, and leaves no-replace publication solely to the supervisor. Normal repository conformance executes a distinct child-only helper whose explicitly supplied bytes, length, SHA-256, immutable memfd copy, private hello, process identity, and reap are verified without fallback before archive authority is sent. The frame codec, raw descriptor transport, sealed-blob envelope, and helper-artifact authenticator now live in one shared library implementation used by both sides of that boundary, so the public supervisor can reuse the proven bytes and checks without a dependency cycle. The [Linux helper packaging contract](docs/helper-packaging.md) now fixes its release location, manifest, modes, helper-aware license closure, and extracted-package proof while requiring its absence from macOS and Windows archives. The next dependency is an explicit fail-closed public supervisor path and hidden `VerifiedArchive` backend integration. Real-kernel setup-failure evidence follows as a release gate. No public worker mode or protocol v2 exists yet.
