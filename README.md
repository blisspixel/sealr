...
- A repository-only [Linux authority-bootstrap conformance lab](docs/sandbox.md#current-bootstrap-evidence). It proves the ordering of descriptor closure, runtime-probed Landlock setup before source transfer, bounded failure handling, reap, and checked cleanup. The lab interprets no archive and is not included in native release archives.
- A dormant crate-private [split-phase semantic-record experiment](docs/semantic-record.md). Its bounded planning and completion records bind the complete invocation and represented ZIP evidence back to the supervisor-owned snapshot, enforce phase-specific semantics, use typed failure for input-sized validation and reconstruction allocations, and reconstruct accepted completion IR exactly once without cloning findings. A source-owning, non-cloneable inspect executor now consumes each validated Ready plan value, verifies only its recorded Store or Deflate payload ranges through the same bounded verifier as `apply` and later member reads, and emits the canonical completion without structural ZIP parsing. It remains absent from `apply`, CLI, default-feature, worker, and shipped runtime paths. A manifest-pinned semantic fuzz target has clean exact-main on-demand campaign evidence; the first scheduled-event run and accumulated history remain open. See the [current assurance evidence](docs/assurance.md#current-evidence) for measurements and the detailed build and workflow contract.
...
- The shipped library and CLI do not use Landlock, seccomp, AppContainer, or another process sandbox. Receipts report `kernel_jail: unavailable`. The repository-only Linux bootstrap lab tests a narrow Landlock and descriptor-lifecycle mechanism, but it does not run the parser, deny process creation, constrain network access, audit a semantic stage, or establish a production containment claim.
...
- The private semantic-record experiment has a versioned, manifest-pinned shadow matrix for 12 named strict-ASCII-v1 cases across planning, terminal, complete, stopped, and setup-failure outcomes. The dormant in-process inspect executor now produces the completion frames for executable matrix cases without structural reparse; focused tests also cover strict-v2 directory reconstruction, a private-file snapshot after caller-path removal, source I/O after a verified prefix, and impossible completion causes. This bounded evidence does not establish parity for other policies or lifecycle states, process isolation, transport safety, or runtime-worker behavior. Bounded on-demand campaigns do not establish scheduled stability or decoder safety. The experiment does not transfer retained bytes, provide isolated later member reads, construct `VerifiedArchive`, launch a worker, change `kernel_jail`, or prove that a worker proposal is the source's unique meaning.
...
The next milestone remains the Phase 0.1 ZIP trust gate, not another archive format. Alpha.4 closed the measured semantic and adopter-facing contract, and Alpha.5 closed bounded immutable input. The first Alpha.6 slice is now a repository-only nonsemantic Linux authority-bootstrap lab: it removes unrelated inherited authority, validates a synthetic stage, installs `no_new_privs` and fixed Landlock ABI 3 handling, reports readiness before receiving a synthetic source, then exits and is reaped without interpreting an archive. This proves a narrow confinement ordering without forcing the reduced protocol v1 manifest into the public API or changing shipped execution.

The landed [private semantic-record assurance](docs/semantic-record.md) now includes a zero-difference, manifest-pinned 12-case shadow matrix over ready, terminal, complete, stopped, Store, Deflate, descriptor, quota, codec, source-I/O, and setup-failure outcomes. Its crate-private inspect-only executor consumes accepted Ready plans without structural reparse, but remains an in-process test path. The next semantic increment is a sealed inspect-only worker bridge that preserves exact snapshot ownership, typed infrastructure failures, bounded lifecycle, clean exit, and reap. Retained-content transfer, isolated non-retained reads, helper packaging, writer quiescence, stage audit, and supervisor-owned publication remain separate activation gates. No public worker mode or protocol v2 exists yet.
