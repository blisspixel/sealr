...
- Receipts are unsigned, and their JSON digest is deterministic for the current Rust structs but is not yet RFC 8785 JCS. The exact declaration-order encoding behind every shipped digest — and the honest statement that emitted pretty bytes are presentation, not the digested bytes — is now the normative [evidence encoding contract](docs/evidence-encoding.md).
...
| [Evidence encoding contract](docs/evidence-encoding.md) | The normative declaration-order canonical JSON behind every policy digest and `view_digest` |
