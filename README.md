...
- A dormant crate-private [split-phase semantic-record experiment](docs/semantic-record.md). Its bounded planning and completion records bind the complete invocation and represented ZIP evidence back to the supervisor-owned snapshot, enforce phase-specific semantics, and reconstruct accepted completion IR exactly once. A source-owning inspect executor consumes each validated Ready plan value without structural ZIP parsing. Required 64-bit Linux, macOS, and Windows CI now measures Complete and last-member Stopped reconstruction from a plan only 67,760 bytes below the private 64 MiB limit and rejects a second full-IR-sized live allocation. The experiment remains absent from `apply`, CLI, workers, and every shipped invocation path. See the [detailed evidence and limits](docs/semantic-record.md#near-limit-completion-heap-evidence).
...
- The private semantic-record experiment has a versioned, manifest-pinned 12-case strict-v1 shadow matrix, focused strict-v2 and private-file regressions, and a near-limit requested-heap measurement for completion reconstruction. The measurement excludes planning, RSS, allocator internals, transport, executor memory, isolation, materialization, and retained-content behavior. This evidence does not establish parity for other policies or lifecycle states, process isolation, transport safety, or runtime-worker behavior. Bounded on-demand campaigns do not establish scheduled stability or decoder safety. A correlated completion may carry an arbitrary non-directory content digest because binding and canonical decode do not prove that payload verification ran. The result is therefore an untrusted worker proposal and cannot shape public `Outcome` or `VerifiedArchive` state until a separate content-authority gate verifies the exact bytes. The experiment does not transfer retained bytes, provide isolated later member reads, construct `VerifiedArchive`, launch a worker, change `kernel_jail`, or prove that a worker proposal is the source's unique meaning.
...
The landed [private semantic-record assurance](docs/semantic-record.md) now includes a zero-difference, manifest-pinned 12-case shadow matrix, an in-process plan-native inspect executor, and a required near-limit completion heap probe. The next sequence expands strict-v2 and backend parity while the Linux bootstrap closes its pre-parser authority controls. A sealed inspect-only worker lab follows only after one real plan-only seam exists. Retained-content transfer, isolated non-retained reads, helper packaging, writer quiescence, stage audit, and supervisor-owned publication remain separate activation gates. No public worker mode or protocol v2 exists yet.
