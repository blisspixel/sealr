...
> Current main additionally implements six unreleased previews: the restricted raw old-GNU long-name profile under policy v6 with `sealrTreeV6`, the gzip-wrapped restricted PAX and GNU long-name compositions under policy v7 with `sealrTreeV7` and `sealrTreeV8` (all zero-new-dependency), and three executed Gate B codec promotions — the zstd-wrapped portable ustar profile under policy v8 with `sealrTreeV9` (`ruzstd` + `twox-hash`), the xz-wrapped portable ustar profile under policy v9 with `sealrTreeV10` (`lzma-rust2`, compiled under `forbid(unsafe_code)`), and the bzip2-wrapped portable ustar profile under policy v10 with `sealrTreeV11` (`bzip2` + the audited pure-Rust `libbz2-rs-sys`). Each wrapper composes only a frozen raw dialect, publishes a distinct transform-bound layout identity while preserving the format-neutral content root, and is refused by the authenticated worker without fallback.
...
- The LZ4 TAR wrapper, ZIP methods 12, 93, and 95, 7z, RAR4, RAR5, cpio, ar, deb, RPM, and CAB are not yet supported selections, and broader GNU TAR extensions (long links, sparse maps, base-256 numbers) remain denied. Raw ustar, gzip-wrapped ustar, restricted raw PAX, restricted raw GNU long-name, the gzip-wrapped PAX and GNU compositions, and the zstd-, xz-, and bzip2-wrapped ustar profiles are public only through separate explicit profile selections. Restricted PAX is an in-process Alpha.11 preview under policy v5; the GNU long-name profile, both gzip compositions, and the three codec wrappers are unreleased in-process previews on current main under policies v6 through v10; none is general PAX, GNU, or codec-ecosystem support. The authenticated Linux worker accepts only the supported ZIP32 records and fails closed without fallback for every TAR and ZIP64 selection until later semantic records exist.
...
1. Build a local bounded 7z interpreter with Copy first, then reuse the reviewed LZMA layer. The zstd-, xz-, and bzip2-wrapped ustar profiles already ship on current main through the reviewed ruzstd, lzma-rust2, and libbz2-rs-sys boundaries, completing the first three Gate B codec promotions.
2. Copy-first 7z separates container correctness from decoder complexity and keeps packed headers, coder graphs, solid routing, encryption, and volume behavior explicitly gated.
