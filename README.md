...
- GNU TAR extensions, gzip-wrapped PAX, zstd, XZ, bzip2, and LZ4 TAR wrappers, 7z, RAR4, RAR5, cpio, ar, deb, RPM, and CAB are not yet supported selections. Raw ustar, gzip-wrapped ustar, and restricted raw PAX regular files and directories are public only through separate explicit profile selections. Restricted PAX is an in-process Alpha.11 preview under policy v5; it is not general PAX support. The authenticated Linux worker accepts only the supported ZIP32 records and fails closed without fallback for every TAR and ZIP64 selection until later semantic records exist.
...
1. Add a separate GNU long-name-only raw TAR profile. GNU `L` carrier state is common enough to matter, but combining it with PAX, long links, sparse maps, or base-256 numbers would erase the exact language Alpha.11 established.
2. Compose the frozen PAX and GNU raw profiles with the existing exact gzip transform. Composition comes after each raw dialect so wrapper evidence cannot hide an unsettled inner interpretation.
3. Promote zstd, XZ/LZMA2, and bzip2 through separate dependency, memory, checksum, exact-consumption, and work gates. Each codec must justify the smallest practical pure-Rust graph and must not introduce a general extractor framework.
4. Build a local bounded 7z interpreter with Copy first, then reuse the reviewed LZMA layer. Copy-first separates container correctness from decoder complexity and keeps packed headers, coder graphs, solid routing, encryption, and volume behavior explicitly gated.
5. Add cpio, ar/deb, CAB, RPM, and restricted RAR5 through format-specific threat models, identities, package evidence, and dependency gates. ISO 9660, UDF, and other filesystem images remain a separate program because they expose a different namespace and authority model.
