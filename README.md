...
- A dormant crate-private [split-phase semantic-record experiment](docs/semantic-record.md). Its bounded planning and completion records bind the complete invocation and represented ZIP evidence back to the supervisor-owned snapshot, enforce phase-specific semantics, and remain absent from `apply`, CLI, default-feature, and shipped runtime paths. A manifest-pinned semantic fuzz target now has one clean exact-main on-demand campaign; the first scheduled-event run and accumulated history remain open. See the [current assurance evidence](docs/assurance.md#current-evidence) for measurements and the detailed build and workflow contract.
