...
- Receipts are unsigned. The default lineage's digests are deterministic for the current Rust structs under the normative declaration-order [evidence encoding contract](docs/evidence-encoding.md), where emitted pretty bytes are presentation rather than the digested bytes; the opt-in `--canonical` RFC 8785 lineage removes that split by making the emitted file bytes exactly the digested bytes.
