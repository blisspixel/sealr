...
- A repository-only [Linux authority-bootstrap conformance lab](docs/sandbox.md#current-bootstrap-evidence). Its same-binary child closes inherited descriptors, validates optional private-stage authority, hard-requires `no_new_privs` and a fixed Landlock ABI 3 filesystem policy, and receives a read-only synthetic source only after the supervisor observes restriction readiness. The lab interprets no archive and is not included in native release archives.
...
The following Alpha.6 increment must preserve the existing `ArchiveIR` and `VerifiedArchive` contract by choosing either a complete bounded IR or certificate returned to the supervisor, or an explicit worker-backed session with bounded reads, lifetime, crash, cancellation, and retention semantics. Only then does supervised archive execution begin. Before any stage audit, the supervisor must also prove that no worker or descendant retains writable stage authority; stage equality is not stable while a writer remains live.
