...
> Current main additionally implements four unreleased previews: the restricted raw old-GNU long-name profile under policy v6 with `sealrTreeV6`, the gzip-wrapped restricted PAX and GNU long-name compositions under policy v7 with `sealrTreeV7` and `sealrTreeV8` (all zero-new-dependency), and the zstd-wrapped portable ustar profile under policy v8 with `sealrTreeV9` — the first promoted codec adapter, which adds exactly the two reviewed runtime packages (`ruzstd`, `twox-hash`) the codec dependency gates budget allows. Each wrapper composes only a frozen raw dialect, publishes a distinct transform-bound layout identity while preserving the format-neutral content root, and is refused by the authenticated worker without fallback.
...
- XZ, bzip2, and LZ4 TAR wrappers, ZIP method 93, 7z, RAR4, RAR5, cpio, ar, deb, RPM, and CAB are not yet supported selections, and broader GNU TAR extensions (long links, sparse maps, base-256 numbers) remain denied. Raw ustar, gzip-wrapped ustar, restricted raw PAX, restricted raw GNU long-name, the gzip-wrapped PAX and GNU compositions, and the zstd-wrapped ustar profile are public only through separate explicit profile selections. Restricted PAX is an in-process Alpha.11 preview under policy v5; the GNU long-name profile, both gzip compositions, and the zstd wrapper are unreleased in-process previews on current main under policies v6 through v8; none is general PAX, GNU, or zstd-ecosystem support. The authenticated Linux worker accepts only the supported ZIP32 records and fails closed without fallback for every TAR and ZIP64 selection until later semantic records exist.
...
1. Promote XZ/LZMA2 and bzip2 through separate dependency, memory, checksum, exact-consumption, and work gates, following the executed zstd promotion. Each codec must justify the smallest practical pure-Rust graph and must not introduce a general extractor framework. The zstd-wrapped ustar profile already ships on current main through the reviewed two-package ruzstd boundary.
