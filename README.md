...
## Released Alpha.6 boundary

The released Alpha.6 Rust implementation supports classic ZIP32 archives with stored or Deflate members.
...
- A repository-only [wheel semantic inventory](docs/wheel-compatibility-v2.md) and consumer laboratory. The lab adds an exact exhaustive UTF-8 and NFC wheel profile, a bounded four-way artifact and install-plan evaluator, minimized hostile fixtures, distinct source/tree/artifact/plan/realization identities, and an external PyPA `installer` 0.7.0 bridge that never receives or reopens the original wheel. This is research evidence, not shipped wheel support.
...
- The released compatibility and strict ASCII profiles still reject non-ASCII member paths, and CP437 decoding plus a general-purpose Unicode profile remain unimplemented. The repository-only wheel profile accepts only strict UTF-8 NFC names under its separate exhaustive rule table. That research profile does not widen the released default.
...
The next milestone remains the Phase 0.1 ZIP trust gate, not another archive format. Alpha.4 closed the measured semantic and adopter-facing contract, Alpha.5 closed bounded immutable input, and Alpha.6 released the explicit supervised Linux boundary through the CLI, wheel laboratory, and extracted package. The next product increment is canonical Unicode naming followed by a real wheel consumer that accepts Sealr's verified capability without reparsing the ZIP. This comes next because inconsistent names are the largest remaining obstacle to a portable tree identity, and a non-reparsing consumer is the direct usefulness test for the boundary already built.
...
Assurance now advances with each increment rather than waiting for a late phase. Three scalar Kani harnesses, targeted mutation discovery, source-coverage discovery, fuzzing, native resource evidence, and required deterministic gates remain distinct in the [promotion ledger](docs/assurance-promotion.md). The repository-only wheel laboratory now proves that a real external consumer can operate on verified members after the original wheel becomes unavailable. The next product gate is to promote portable canonical naming and the wheel result from research into a supported boundary while authenticated recovery, durability, and scheduled assurance history continue in parallel.

See the [near-term execution plan](docs/near-term.md) for release-sized work and acceptance gates, the [assurance promotion contract](docs/assurance-promotion.md) for exact claims and promotion rules, the [identity-conformance contract](docs/identity-conformance.md) for the independent root checks, the [wheel inventories](docs/wheel-compatibility-v2.md) for bounded measurement, the [distribution contract](docs/distribution-contract.md) for separate source and native promises, the [roadmap](ROADMAP.md) for the full trust gate, and the [wheel profile draft](docs/profiles/python-wheel-v1.md) for the repository-only first-consumer implementation. No supported wheel API or CLI mode exists yet.
...
| [API contract](docs/api.md) | Current Alpha.6 Rust and JSON surface |
