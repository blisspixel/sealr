...
sealr is an early attempt to make archive ingestion easier to reason about. Current main implements deliberately narrow ZIP32, explicit strict ZIP64, raw portable ustar, and strict single-member gzip-wrapped portable ustar paths. The latest published release, Alpha.9, contains ZIP32 and raw portable ustar; ZIP64 and gzip-wrapped ustar are in-process alpha previews on current main. Every nondefault format is selected explicitly and does not widen or alias the ZIP32 compatibility default. Every supported format builds one versioned interpretation from an immutable source snapshot, verifies accepted members, and either publishes the requested tree without replacement or publishes no destination. An explicit x86_64 Linux mode moves supported ZIP32 payload verification, stage writes, and later non-retained reads into an authenticated worker restricted with Landlock ABI 3 and seccomp, while the supervisor retains structural planning and publication authority. ZIP64 and gzip-wrapped ustar fail closed at that boundary until a later semantic record can represent their evidence; unsupported worker selections never fall back to in-process execution. Sealr does not yet provide a general process sandbox or production security claim.
...
The latest published Alpha.9 release supports classic ZIP32 archives with stored or Deflate members and raw portable POSIX ustar regular files and directories through explicit selection and policy v2 authorization. Current main additionally exposes `Zip64StrictAsciiV1` under policy v3 and strict single-member gzip-wrapped portable ustar under policy v4 as explicit in-process previews. `apply()` and the `zip` CLI selection remain ZIP32 and never alias to either profile.
...
- An explicitly selected [gzip-wrapped portable ustar profile](docs/profiles/tar-gzip-ustar-portable-v1.md) on current main. It reuses the existing `flate2` boundary and adds no dependency. The exact caller-supplied gzip bytes remain source domain 0; one bounded, immutable decoded TAR is retained as domain 1. Policy v4 separately caps the original and derived archives. Wrapper and TAR coverings, one exact transform record, CRC32, ISIZE, SHA-256, payload plans, and `sealrTreeV4` layout evidence bind the two domains before any destination stage is created.
...
- An explicit [strict ASCII ZIP64 v1 profile](docs/profiles/zip64-strict-ascii-v1.md) on current main. It uses policy v3, ZIP64-native IR evidence, an independent covering audit, and `sealrTreeV3` layout identity. It is available only in process: an authenticated worker request fails closed until semantic-record v3 can represent this evidence.
...
- Versioned identity-conformance bundles for ZIP32, strict ZIP64, raw portable ustar, and gzip-wrapped portable ustar. Production APIs pin every family. The standalone workspace verifier independently reconstructs the profile and tree encodings without depending on the Sealr crate. It validates source-bound ZIP and gzip wrapper geometry without record discovery or inflation, validates declared TAR geometry and derived-byte integrity, and reconstructs the published layout and content roots.
...
- PAX and GNU TAR extensions, zstd, XZ, bzip2, and LZ4 TAR wrappers, 7z, RAR4, RAR5, cpio, ar, deb, RPM, and CAB are not yet supported selections. Raw and gzip-wrapped portable ustar regular files and directories are public only through separate explicit profile selections. Strict ZIP64 and gzip-wrapped ustar are in-process previews under policy v3 and v4 respectively; the authenticated Linux worker accepts the supported ZIP32 records and fails closed without fallback for both newer profiles until a later semantic record exists.
...
1. Finish strict ZIP64 assurance and add semantic-record v3 worker parity. The public in-process profile must not be mistaken for sandbox support, and ZIP32 must remain an immutable non-aliasing default.
2. Finish the public gzip-wrapped ustar assurance increment with bounded public fuzzing, native package evidence, and the fail-closed worker-record boundary. Independent `sealrTreeV4` vectors are already present. The profile reuses `flate2` and adds no decoder dependency.
3. Add raw PAX and a selected GNU long-name TAR profile as separate structural languages, then compose them with gzip before promoting additional wrappers one codec at a time.
4. Promote zstd, XZ, and bzip2 wrappers through their separate dependency, memory, checksum, exact-consumption, and work gates, then build 7z on the reviewed LZMA layer.
5. Evaluate cpio, ar/deb, CAB, RPM, and RAR5 through format-specific threat models, evidence, and dependency gates. ISO 9660, UDF, and other filesystem images remain a separate program because they expose a different namespace and authority model.
...
| [Near-term execution plan](docs/near-term.md) | Released Alpha.9 baseline, current ZIP64 preview, and the next measurable gates |
