...
> Current main additionally implements five unreleased previews: the restricted raw old-GNU long-name profile under policy v6 with `sealrTreeV6`, the gzip-wrapped restricted PAX and GNU long-name compositions under policy v7 with `sealrTreeV7` and `sealrTreeV8` (all zero-new-dependency), the zstd-wrapped portable ustar profile under policy v8 with `sealrTreeV9` — the first promoted codec adapter, which adds exactly the two reviewed runtime packages (`ruzstd`, `twox-hash`) the codec dependency gates budget allows — and the xz-wrapped portable ustar profile under policy v9 with `sealrTreeV10`, the second promoted codec, which adds exactly one reviewed runtime package (`lzma-rust2`) compiled under `forbid(unsafe_code)`. Each wrapper composes only a frozen raw dialect, publishes a distinct transform-bound layout identity while preserving the format-neutral content root, and is refused by the authenticated worker without fallback.
...
- bzip2 and LZ4 TAR wrappers, ZIP methods 93 and 95, 7z, RAR4, RAR5, cpio, ar, deb, RPM, and CAB are not yet supported selections, and broader GNU TAR extensions (long links, sparse maps, base-256 numbers) remain denied. Raw ustar, gzip-wrapped ustar, restricted raw PAX, restricted raw GNU long-name, the gzip-wrapped PAX and GNU compositions, and the zstd- and xz-wrapped ustar profiles are public only through separate explicit profile selections. Restricted PAX is an in-process Alpha.11 preview under policy v5; the GNU long-name profile, both gzip compositions, and the zstd and xz wrappers are unreleased in-process previews on current main under policies v6 through v9; none is general PAX, GNU, zstd-, or xz-ecosystem support. The authenticated Linux worker accepts only the supported ZIP32 records and fails closed without fallback for every TAR and ZIP64 selection until later semantic records exist.
...
1. Promote bzip2 through separate dependency, memory, checksum, exact-consumption, and work gates, following the executed zstd and XZ/LZMA2 promotions. Each codec must justify the smallest practical pure-Rust graph and must not introduce a general extractor framework. The zstd- and xz-wrapped ustar profiles already ship on current main through the reviewed ruzstd and lzma-rust2 boundaries.
2. Build a local bounded 7z interpreter with Copy first, then reuse the reviewed LZMA layer. Copy-first separates container correctness from decoder complexity and keeps packed headers, coder graphs, solid routing, encryption, and volume behavior explicitly gated.
