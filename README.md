...
- A dormant crate-private [split-phase semantic-record experiment](docs/semantic-record.md). Its canonical bounded planning and completion records bind the complete invocation, reproduce accepted covering claims against the supervisor-owned snapshot, enforce parser-equivalent metadata accounting, preserve source-ordered structural IR, derive completion axes from one full verification vector, and reject hostile bytes without changing a shipped path or public contract.
...
The next Alpha.6 increment extends the landed [private semantic-record experiment](docs/semantic-record.md) with broader shadow parity and hostile fuzzing, then closes retained-content transfer, isolated non-retained reads, and helper packaging as separate activation gates. No public worker mode or protocol v2 exists yet. Before any stage audit, the supervisor must also prove that no worker or descendant retains writable stage authority; stage equality is not stable while a writer remains live.
