...
- **Not chasing format breadth.** Parser breadth is currently frozen. Supervised prefix parity and the copyable wheel handoff are implemented. The active sequence is an exact Poetry 2.4.2 repository fixture, independent adoption, measured compatibility, and review. See the [near-term execution plan](docs/near-term.md).
...
2. **Poetry 2.4.2 repository fixture: next.** Pin the exact upstream artifact and dependency closure, prepare and admit the wheel before update uninstall or destination writes, consume the packaged handoff, remove the source before Python starts, deny every `.whl` open, and compare realized outputs with the Sealr plan and realization identity. This remains repository-owned mechanism evidence.
3. **External usefulness: afterward.** Seek one separately maintained publisher, registry, build backend, or installer that adopts the capability and independently verified evidence as authoritative only after the repository fixture is complete.
