...
The landed [private semantic-record assurance](docs/semantic-record.md) now includes an immutable 12-case v1 baseline, 12 additive v2 cases with explicit apply, backend, and supervisor-reproduction oracles, an in-process plan-native inspect executor, a shared owning plan seam, and a required near-limit completion heap probe. The Linux bootstrap has closed no-descendant and permission-mutation authority before source transfer and now parses every raw ancillary header while closing all installed descriptors on rejection. Per-epoch absolute deadlines and repeated native stress are next because confinement is not useful if any authority exchange can stall indefinitely or if failure cleanup is only proved once. Those gates precede a sealed inspect-only worker lab that consumes the plan across a process boundary. Independent content authority, retained-content transfer, isolated non-retained reads, helper packaging, writer quiescence, stage audit, and supervisor-owned publication remain separate activation gates. No public worker mode or protocol v2 exists yet.
