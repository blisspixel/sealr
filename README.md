...
sealr is an early attempt to make archive ingestion easier to reason about. Current main implements deliberately narrow ZIP32, explicit strict ZIP64, and raw portable ustar paths. The latest published release, Alpha.9, contains ZIP32 and raw portable ustar; the ZIP64 integration is an in-process alpha preview on current main. Portable ustar and ZIP64 are explicitly selected and do not widen or alias the ZIP32 compatibility default. Every supported format builds one versioned interpretation from an immutable source snapshot, verifies accepted members, and either publishes the requested tree without replacement or publishes no destination. An explicit x86_64 Linux mode moves supported ZIP32 payload verification, stage writes, and later non-retained reads into an authenticated worker restricted with Landlock ABI 3 and seccomp, while the supervisor retains structural planning and publication authority. ZIP64 fails closed at that boundary until semantic-record v3 exists; unsupported worker selections never fall back to in-process execution. Sealr does not yet provide a general process sandbox or production security claim.
...
- A versioned four-case ZIP32 identity-conformance bundle with four exact ZIP32 profile vectors, a separate two-case ZIP64 profile and layout bundle on current main, and separate portable ustar profile and layout vectors. Production APIs pin all three families. The standalone workspace verifier independently checks the ZIP32 and TAR bundles without depending on the Sealr crate; ZIP64 independent-verifier parity remains an assurance gate. For ZIP32 it hashes embedded source and profile bytes and checks the claimed covering without searching or inflating. For TAR it independently validates the declared geometry and reconstructs the published layout and content roots, while producer fixtures separately reconstruct, hash, and apply exact TAR bytes.
...
- PAX and GNU TAR extensions, compressed TAR wrappers, 7z, RAR4, RAR5, cpio, ar, deb, RPM, and CAB are not yet supported selections. gzip currently exists only as bounded internal transform and fuzz infrastructure, not as a public format. Uncompressed portable ustar regular files and directories are public only through explicit profile selection. Strict ZIP64 is an explicit in-process preview under policy v3; the authenticated Linux worker accepts the supported ZIP32 records and fails closed without fallback for ZIP64 until semantic-record v3 exists.
...
2. Promote a bounded gzip-wrapped ustar profile from the existing internal transform and fuzz infrastructure. It reuses `flate2` and tests derived-snapshot identity without adding a decoder dependency.
3. Add PAX and selected GNU TAR as separate structural profiles, then promote additional compressed TAR wrappers one codec at a time.
4. Evaluate 7z, cpio, ar/deb, CAB, RPM, and RAR5 through format-specific threat models, evidence, and dependency gates. ISO 9660, UDF, and other filesystem images remain a separate program because they expose a different namespace and authority model.
